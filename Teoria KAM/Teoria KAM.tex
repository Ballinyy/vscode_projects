...
Consideriamo una funzione liscia $\mathcal{H}$ definita su un aperto $M$ di $\mathbb{T}^n \times \mathbb{R}^n$ = $\{(\theta, r)\}$ (dove $\mathbb{T}^n = \mathbb{R}^n/\mathbb{Z}^n$ è il toro $n$-dimensionale e $(\theta, r) = (\theta_1, \dots, \theta_n, r_1, \dots, r_n)$). \\
Il \textit{campo vettoriale hamiltoniano} $X_\mathcal{H}$ indotto da $\mathcal{H}$ tramite le equazioni di Hamilton è dato da 
\begin{align} X_\mathcal{H} =
    \begin{cases}
        \dot{\theta}_j &= \frac{\partial \mathcal{H}}{\partial r_j} \\
        \dot{r}_j &= -\frac{\partial \mathcal{H}}{\partial \theta_j}
    \end{cases} \tag{2.1}
\end{align}
...
\begin{align}
    \nabla \mathcal{H} \cdot X_\mathcal{H} &= \sum_{j = 1}^n \left(\frac{\partial \mathcal{H}}{\partial \theta_j}\dot{\theta}_j + \frac{\partial \mathcal{H}}{\partial r_j}\dot{r}_j\right) \notag \\
    &= \sum_{j = 1}^n \left(\frac{\partial \mathcal{H}}{\partial \theta_j}\frac{\partial \mathcal{H}}{\partial r_j} - \frac{\partial \mathcal{H}}{\partial r_j}\frac{\partial \mathcal{H}}{\partial \theta_j}\right) = 0 \tag{2.2}
\end{align}
...
\begin{align}
    \varphi_t(\theta, r) = \left(\theta + t\nabla_r\mathcal{H}, r\right) \tag{3.2}
\end{align}
...
\begin{align}
    \lim_{T \rightarrow \infty}\frac{1}{T}\int_0^T f(\theta + t\alpha) \, dt = \frac{1}{\Omega}\int_{\mathbb{T}^n} f(\theta) \, d\theta \tag{3.19}
\end{align}
...
Tutta l'informazione riguardante un determinato sistema dinamico è contenuta nella funzione di Hamilton $\mathcal{H}$. $\mathcal{H}$ è una funzione scalare regolare definita sullo spazio delle fasi, da $\mathcal{H}$ possiamo ricavare, tramite le equazioni del moto di Hamilton, il campo di velocità hamiltoniano $X_\mathcal{H}$ associato al sistema in esame e quindi anche il flusso hamiltoniano $\varphi$. \\
Il vantaggio di partire da una funzione scalare risiede nel fatto che è molto più semplice eseguire un cambio di coordinate per una funzione scalare che per campi vettoriali. Tuttavia, solamente una particolare classe di cambi di coordinate è ammessa, ovvero i diffeomorfismi $\phi$ che lasciano invariato $X_\mathcal{H}$ (e quindi le equazioni del moto), cioè
\begin{align}
    \phi^*X_{\mathcal{H} \circ \phi} = X_\mathcal{H} \tag{4.1}
\end{align}
Per caratterizzare tali diffeomorfismi diamo una defizione di $X_\mathcal{H}$ che non necessita di un sistema di coordinate. Sia
\begin{align}
...
\end{align}
Segue che $X_\mathcal{H}$ è definito implicitamente dall'equazione
\begin{align}
    \omega(X_\mathcal{H}, \cdot) = d\mathcal{H} \tag{4.4}
\end{align}
...
\end{align}
che garantisce l'invarianza del differenziali di $\mathcal{H}$ rispetto a $\phi$. Tali trasformazioni sono dette \textit{simplettiche} o \textit{canoniche}. \\ \\
...
\begin{align}
    \phi: \quad (\theta, r) \mapsto (\varphi(\theta), (\varphi'(\theta)^{-1})r) \tag{4.10}
\end{align}
...
\begin{align}
    \phi^*\lambda = \tilde{r}d\tilde{\theta} = r \cdot (\varphi'(\theta))^{-1}(\varphi'(\theta)) \cdot d\theta = \lambda \tag{4.11}
\end{align}
Concludiamo che $\phi$ è simplettico. \\ \\
\textbf{Proposizione 4.1:} sia $\phi_t$ è il flusso associato ad un campo vettoriale hamiltoniano $X_\mathcal{H}$, allora $\phi^*_t\omega = \omega$ per ogni $t$ nell'intervallo di definizione del flusso. \\
...
\begin{align}
    ...
    &= \int_0^t \phi^*_s(\mathcal{L}_{X_\mathcal{H}}\omega) \, ds \tag{4.12}
\end{align}
per definizione di derivata di Lie. Per la \textit{formula d'omotopia di Cartan} si ha
\begin{align}
    \mathcal{L}_{X_\mathcal{H}}\omega = di_{X_\mathcal{H}}\omega + i_{X_\mathcal{H}}d\omega \tag{4.13}
\end{align}
In particolare, $d\omega = 0$, dato che che $\omega$ ha tutti coefficienti costanti, mentre $i_{X_\mathcal{H}}\omega = d\mathcal{H}$, pertanto $di_{X_\mathcal{H}}\omega = d^2\mathcal{H} = 0$ (dato che $d^2 = 0$, operatore di coomologia). \qed

\newpage
\subsection{Serie perturbative}
