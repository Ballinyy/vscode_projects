...
\end{align*}
Per ipotesi $g$ è olomorfa in $\mathbb{T}^n_{s + \sigma}$, pertanto, per ogni $k \in \mathbb{Z}\setminus\{0\}$, trasliamo il toro di integrazione attraverso il diffeomorfismo
\begin{align*}
...
\end{align}
Siamo ora interessati a ricondurre la sommatoria ad una sommatoria su $\mathbb{N}$, pertanto, fissato $m \in \mathbb{N}$, dobbiamo contare quanti $k \in \mathbb{Z}^n\setminus\{0\}$ soddisfano $|k| = m$. Notiamo subito che se un dato $k$ soddisfa $|k| = m$, anche $-k$ soddisfa $|-k| = m$, pertanto ottieniamo un fattore $2^n$ a moltiplicare. Pertanto, fissata una base ortonormale destra $\{e_i\}_{i = 1, \dots, n}$ di $\mathbb{Z}^n$, ci resta da contare quanti $k$ nell'insieme
\begin{align*}
    \left\{k \in \mathbb{Z}^n\setminus\{0\} : \sum_{i = 1}^n k \cdot e_i > 0\right\} \tag{4.38}
\end{align*}
soddisfano contemporaneamente $|k| = m$, fissato $m \in \mathbb{N}$.
...
\textbf{1.} $d$ è lineare. \\
\textbf{2.} se $\omega^{(k)}$ è una $k$-forma differenziabile e $\eta$ una $1$-forma differenziabile in un aperto $U \subset \mathbb{R}^n$ si ha che
\begin{align}
    d(\omega^{(k)} \wedge \eta) = d\omega^{(k)} \wedge \eta + (-1)^k\omega^{(k)} \wedge d\eta \tag{B.10.6}
\end{align}
...
\begin{align}
    \sum_{i, j}\frac{\partial}{\partial x_i}\left(\frac{\partial \omega_{i_1, \dots, i_k}}{\partial x_j}\right)dx_i \wedge dx_j &= \sum_{i < j}\overbrace{\left(\frac{\partial \omega_{i_1, \dots, i_k}}{\partial x_i\partial x_j} - \frac{\partial \omega_{i_1, \dots, i_k}}{\partial x_j \partial x_i}\right)}^{0}dx_i \wedge dx_j \tag{B.10.?}
\end{align}
...
\begin{align}
    d(F^*\omega^{(k)}) = F^*d\omega^{(k)} \tag{B.10.?}
\end{align}
...
\begin{flalign}
    \|f_n - f_m\|_\infty < \frac{\varepsilon}{2} \tag{D.2}
\end{flalign}
per ogni $n, m > N_\varepsilon$. Allora, per ogni $t \in S$ si ha
\begin{flalign}
    |f_n(t) - f_m(t)| \leq \|f_n - f_m\|_\infty < \frac{\varepsilon}{2} \tag{D.3}
\end{flalign}
...
\begin{flalign}
    f(t) = \lim_{n \rightarrow \infty} f_n(t) \tag{D.4}
\end{flalign}
...
\begin{flalign}
    |f_n(t) - f(t)| &= \lim_{m \rightarrow \infty} |f_n(t) - f_m(t)| \leq \frac{\varepsilon}{2} < \varepsilon \tag{D.5}
\end{flalign}
per continuità della norma, per ogni $n > N_\varepsilon$ e ogni $t \in S$. Prendendo il $\sup$ si ottiene
\begin{align*}
    \sup_{t \in S} |f_n(t) - f(t)| = \|f_n - f\|_\infty < \varepsilon \tag{D.6}
\end{align*}
...
\begin{flalign}
    ...
    &\leq 2\int_{\delta}^1 q_n(x)\|f\|_\infty \, dt + \int_{-\delta}^\delta q_n(x)|f(x + t) - f(x)| \, dt \tag{D.14}
\end{flalign}
...
\begin{flalign}
    |p_n(x) - f(x)| \leq 2\|f\|_\infty\sqrt{n}(1 - \delta^2)^n(1 - \delta) + 2\varepsilon\delta\sqrt{n}(1 - \delta^2)^n \rightarrow 0 \tag{D.15}
\end{flalign}
...
\begin{flalign}
    \|p_n - f\|_{\infty} \rightarrow 0 \tag{D.16}
\end{flalign}
