...
\end{align}
Per ogni $i,j = 1, \dots, n$ definiamo $\omega_{ij} = \omega^{(2)}(e_i, e_j)$. Sappiamo che $\omega_{ii} = 0$ e $\omega_{ij} = -\omega_{ji}$ per ogni $i, j = 1, \dots, n$. Inoltre, per ogni $0 \leq i < j \leq n$, definiamo la $2$-forma 
\begin{align}
    dx_{ij}(e_k, e_l) = \det(0, \dots, \overbrace{e_k}^{i-\text{esima riga}}, \dots, \overbrace{e_l}^{j-\text{esima riga}}, \dots, 0) \tag{1.1.12}
\end{align}
ed estensione bilineare, con $k, l = 1, \dots, n$. Allora 
\begin{align}
    \omega^{(2)}(x, y) = \sum_{0 \leq i < j \leq n}\omega_{ij}dx_{ij}(x, y) \tag{1.1.13}
\end{align}
e per l'arbitrarietà della coppia $(x, y)$ segue che 
\begin{align}
    \omega^{(2)} = \sum_{0 \leq i < j \leq n} \omega_{ij}dx_{ij} \tag{1.1.14}
\end{align}
Inoltre 
\begin{align}
    \sum_{0 \leq i < j \leq n} 1 = \frac{n(n - 1)}{2} \tag{1.1.15}
\end{align}
ovvero la dimensione del duale è $\frac{n(n - 1)}{2}$. \qed \\ \\
La dimensione di $(\mathbb{R}^n \times \mathbb{R}^n)^*$ è $\frac{n(n - 1)}{2}$ proprio per le proprietà di antisimmetricità delle $2$-forme. \\ \\
...
Introduciamo l'operazione di \textit{prodotto esterno} $\wedge$ per le forme differenziali. Date $\omega^{(k)}$ una $k$-forma e $\omega^{(l)}$ una $l$-forma, il prodotto esterno $\omega^{(k)} \wedge \omega^{(l)}$ sarà una $k + l$-forma. \\ \\
Prima di tutto, definiamo il prodotto esterno per le $1$-forme. Date $\omega_1, \omega_2$ $1$-forme su $\mathbb{R}^n$, poniamo 
\begin{align}
