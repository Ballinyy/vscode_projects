...
\begin{align}
    v \in \mathbb{R}^3 \mapsto \omega^{(1)}_v \in \Omega^{(1)}(\mathbb{R}^3) \tag{1.4.13} \\
    v \in \mathbb{R}^3 \mapsto \omega^{(2)}_v \in \Omega^{(2)}(\mathbb{R}^3) \tag{1.4.14}
\end{align}
...
\textbf{Proposizione 1.5.1:} $f^*$ è un operatore lineare da $\Omega^{(k)}(\mathbb{R}^n)$ a $\Omega^{(k)}(\mathbb{R}^m)$. \\
...
\textbf{Proposizione 1.7.1:} l'insieme $\Omega^{(k)}(M)$ delle $k$-forme differenziali sulla varietà $M$ è uno spazio vettoriale (di dimensione infinita se $k \leq \dim M$). \\
...
\textbf{Proposizione 1.8.1:} sia $g : L \longrightarrow M$ un'applicazione differenziabile di varietà differenziabili. Allora $(f \circ g)^* = g^* \circ f^*$. \\

\textbf{Dimostrazione:} data $\omega^{(k)} \in \Omega^{(k)}(N)$ si ha
\begin{align}
    (f \circ g)^*\omega = \omega \circ (f \circ g)_* \tag{1.8.6}
\end{align}
...
\begin{align}
    ...
    &= g^*f^*\omega(v_1, \dots, v_s) \tag{1.8.7}
\end{align}
...
\begin{align}
   \int_\gamma \omega = \lim_{\Delta \rightarrow 0}\sum_i \omega(v_i) \tag{1.9.1}
\end{align}
dove $v_i$ è il vettore tangente a $\gamma$ in $t_i$, ovvero
\begin{align}
   v_i = \dot{\gamma}(t_i) \tag{1.9.2}
\end{align}
$0 \leq i \leq s - 1$. Osserviamo che il limite $\Delta \rightarrow 0$ corrisponde al limite $s\rightarrow \infty$. \\ \\
In particolare, se $\omega = a_1dx_1 + \dots + a_ndx_n$, con $a_1, \dots, a_n : M \rightarrow \mathbb{R}$ differenziabili, $(1.9.1)$ è equivalente a
\begin{align}
    \int_0^1 \sum_{i = 1}^n a_i(\gamma(t))\dot{\gamma}_i(t) \, dt \tag{1.9.3}
\end{align}
...
\begin{align}
    \omega^{(k)} = adx_1 \wedge \dots \wedge dx_k \tag{1.9.4}
\end{align}
Definiamo
\begin{align}
    \int_D \omega^{(k)} = \int_D a(x) \, dx_1 \dots dx_k \tag{1.9.5}
\end{align}
dato che identifichiamo $T\mathbb{R}^k$ con $\mathbb{R}^k$ stesso. \\ \\
Da $(1.9.5)$ segue che anche $\int_D \omega^{(k)}$ dipende da $\omega^{(k)}$ e da $D$ linearmente (per le proprietà dell'integrale di Riemann), ovvero
\begin{align}
    \int_{D_1 + D_2} \omega^{(k)}_1 + \omega^{(k)}_2 = \int_{D_1}\omega^{(k)}_1 + \int_{D_2}\omega^{(k)}_2 + \int_{D_1}\omega^{(k)}_2 + \int_{D_2}\omega^{(k)}_1 \tag{1.9.6}
\end{align}
per ogni $\omega^{(k)}_1, \omega^{(k)}_2 \in \Omega^{(k)}(\mathbb{R}^k)$ e ogni $D_1, D_2$ poliedri convessi limitati in $\mathbb{R}^k$. \\ \\
\textbf{Proposizione 1.9.1:} siano $D_1, D_2$ poliedri convessi limitati di $\mathbb{R}^k$ e sia $f : D_1 \longrightarrow D_2$ un diffeomorfismo che conserva l'orientazione dell'interno di $D_1$ nell'interno di $D_2$. Data $\omega^{(k)} \in \Omega^{(k)}(\mathbb{R}^k)$ si ha
\begin{align}
    \int_{D_1} f^*\omega^{(k)} = \int_{D_2} \omega^{(k) \tag{1.9.7}}
\end{align} \\

\textbf{Dimostrazione:} nel sistema di coordinate $y = f(x)$ identificato da $f$ si ha
\begin{align}
    \omega^{(k)} = a(y)dy_1 \wedge \dots \wedge dy_k \tag{1.9.8}
\end{align}
...
\begin{align}
    dy_i = \sum_{j = 1}^k \frac{\partial y_i}{\partial x_j}dx_j \tag{1.9.9}
\end{align}
Allora
\begin{align}
    ...
    &= \int_{D_1} f^*\omega^{(k)} \tag{1.9.10}
\end{align}
ovvero la tesi. \qed \\ \\
Vogliamo estendere la definizione di integrazione di $k$-forme differenziali su $\mathbb{R}^k$ per integrali di $k$-forme differenziali su varietà $n$-dimensionali. \\ \\
Siano $\omega^{(k)}$ una $k$-forma differenziale su una varietà $n$-dimensionale $M$, $D$ un poliedro convesso limitato in $\mathbb{R}^k$ e $f : D \longrightarrow M$ un'applicazione differenziabile.
...
\begin{align}
    \varphi_t(\theta, r) = \left(\theta + t\partial_rH, r\right) \tag{3.2}
\end{align}
...
\begin{align}
    h(\theta + t\alpha) = h(\theta) + t\beta \tag{3.4}
\end{align}
...
\begin{align}
    \pi(H(\theta + t\alpha)) = \pi(H(\theta) + t\beta) \tag{3.6}
\end{align}
 per ogni $\theta \in \mathbb{R}^n$ e ogni $t \in \mathbb{R}$. Concludiamo che 
\begin{align}
    H(\theta + t\alpha) - H(\theta) - t\beta \in \mathbb{Z}^n \tag{3.7}
\end{align}
...
\begin{align}
    \theta \mapsto H(\theta + t\alpha) - H(\theta) - t\beta \tag{3.8}
\end{align}
Notiamo immediatamente che $\Delta_H$ è continua, infatti $H$ è continua. Inoltre, dato che $\mathbb{R}^n$ è connesso, $\Delta_H$ continua e $\mathbb{Z}^n$ uno spazio con topologia discreta, concludiamo che $\Delta_H$ è costante. Pertanto $(3.5)$ è soddisfatta per ogni $\theta \in \mathbb{R}^n$ (e ogni $t \in \mathbb{R}^n$). \\ \\
Notiamo che esiste $A \in GL_n(\mathbb{Z})$ tale che
 \begin{align}
    H(\theta + k) = H(\theta) + Ak \tag{3.9}
 \end{align}
per ogni $k \in \mathbb{Z}^n$. Infatti, per definizione di sollevamento, si ha
\begin{align}
    \pi(H(\theta + k)) = h(\pi(\theta + k)) = h(\pi(\theta)) = \pi(H(\theta)) \tag{3.10}
 \end{align}
ovvero
\begin{align}
    H(\theta + m) - H(\theta) \in \mathbb{Z}^n \tag{3.11}
 \end{align}
...
\begin{align}
    V := A^{-1}H - \mathbf{1} : \mathbb{R}^n \longrightarrow \mathbb{R}^n \tag{3.12}
\end{align}
...
\begin{align}
    ...
    &= V(\theta) \tag{3.13}
\end{align}
per ogni $m \in \mathbb{Z}^n$. Riscrivendo $(3.5)$ in termini di $V$ si ottiene
\begin{align}
    (AV + A)(\theta + t\alpha) = (AV + A)(\theta) + t\beta \tag{3.14}
\end{align}
ovvero
\begin{align}
    AV(\theta + t\alpha) - AV(\theta) = t(\beta - A\alpha) \tag{3.15}
\end{align}
In particolare, per $\theta = 0$, si ha
\begin{align}
    A(V(t\alpha) - V(0)) = t(\beta - A\alpha) \tag{3.16}
\end{align}
...
\begin{align}
    \lim_{T \rightarrow \infty}\frac{1}{T}\int_0^T f(\theta + t\alpha) \, dt = \int_{\mathbb{T}^n} f(\theta) \, d\theta \tag{3.18}
\end{align}
...
\begin{align}
    \widehat{f \circ \varphi_t}(k) = \int_{\mathbb{T}^n} e^{-i2\pi k \cdot \theta}f(\theta + t\alpha) \, d\theta \tag{3.19}
\end{align}
Poniamo $\theta' = \theta + t\alpha$, allora $\theta = \theta' - t\alpha$ e $d\theta = d\theta'$. Sostituendo in $(3.19)$ si ottiene
\begin{align}
    \widehat{f \circ \varphi_t}(k) = e^{i2\pi k \cdot \alpha t}\int_{\mathbb{T}^n} e^{-i2\pi k \cdot \theta'}f(\theta') \, d\theta' = e^{i2\pi k \cdot \alpha t}\widehat{f}(k) \tag{3.20}
\end{align}
...
\begin{align}
    f(\theta) = e^{i2\pi k \cdot \theta} \tag{3.21}
\end{align}
per ogni $\theta \in \mathbb{R}^n$, è invariante ma non costante, pertanto si ottiene un assurdo. \\ \\
$(1 \rightarrow 3)$ Se $f$ è costante $(3.18)$ è immediatamente verificata. Se $f$ è della forma
\begin{align}
...
\begin{align}
    ...
    &= \frac{1}{T}e^{i2\pi k \cdot \theta}\frac{e^{i2\pi k \cdot aT} - 1}{ik \cdot \alpha} \rightarrow 0 \tag{3.23}
\end{align}
...
Se $f$ è un polinomio trigonometrico può essere espressa come somma finita di termini della forma $e^{i2\pi m \cdot \theta}$, per $m \in \mathbb{Z}^n$. Pertanto $(3.18)$ è verificata grazie a $(3.23)$ per linearità dell'integrale. \\
Per il \textit{teorema di Stone-Weierstrass}\footnote{In appendice B}, se $f$ è continua, fissato $\varepsilon > 0$, esiste $P$ polinomio trigonometrico tale che
\begin{align}
    \max_{\theta \in \mathbb{T}^n} |f(\theta) - P(\theta)| < \varepsilon \tag{3.24}
\end{align}
In particolare, per polinomi trigonometrici abbiamo appena dimostrato che 
\begin{align}
    \left|\frac{1}{T}\int_0^T P(\theta + t\alpha) \, dt\right| \rightarrow 0 \tag{3.25}
\end{align}
...
\begin{align}
    ...
    &+ \left|\overline{P} - \overline{f}\right| < 3\varepsilon \tag{3.26}
\end{align}
per $T$ sufficientemente grande. \\ \\
$(3 \rightarrow 1)$ Supponiamo che esista $k \in \mathbb{Z}^n \setminus \{0\}$ tale che $k \cdot \alpha = 0$. Allora $f$ definita in $(3.21)$ è una funzione invariante e continua che ha media temporale diversa dalla media spaziale, infatti
\begin{align}
    \frac{1}{T}\int_0^T e^{i2\pi k \cdot (\theta + \alpha t)} \, dt = e^{i2\pi k \cdot \theta} \tag{3.27}
\end{align}
mentre $\overline{f} = 0$. \\ \\
...
\begin{align}
    0 = k \cdot \theta + k \cdot l \tag{3.28}
\end{align}
...
\begin{align}
    \phi: \quad (\theta, r) \mapsto \underbrace{(\varphi(\theta), r(\varphi_*(\theta)^{-1}))}_{(\tilde{\theta}, \tilde{r})} \tag{4.10}
\end{align}
...
\begin{align}
    \phi^*\lambda = \tilde{r} \cdot d\tilde{\theta} = r \cdot (\varphi_*(\theta))^{-1}(\varphi_*(\theta)) \cdot d\theta = \lambda \tag{4.11}
\end{align}
...
In particolare, $\mathbb{T}^n_s$ è compatto e ogni funzioni olomorfa su un insieme è ivi continua, pertanto se $f \in \mathcal{O}(\mathbb{T}^n_s)$ segue che $\|f\|_\infty < \infty$. Concludiamo che la coppia $(\mathcal{O}(\mathbb{T}^n_s), \|\cdot\|_\infty)$ è uno spazio di Banach\footnote{Teorema B.$1$ in appendice B.}. \\ \\
Invertendo $(5.10)$, $F$ è univocamente determinato dall'azione di $\mathcal{L}^{-1}_\alpha$ su $\tilde{H}_1$. Tuttavia, affinché $\mathcal{L}^{-1}_\alpha$ mappi funzioni olomorfe a funzioni olomorfe, necessitiamo di condizioni aritmetiche sul vettore frequenza $\alpha$ che garantiscano sufficiente 'lontananza' dalla risonanza. \\ \\
...
Dividiamo l'intervallo $[0, 1)$ in $q^n$ sottointervalli di lunghezza $1/q^n$. Avendo considerato solamente i vettori interi con tutte componenti positive, per ogni $a, b \in \mathbb{Z}^n$ distinti che soddisfano $(5.20)$ si ha $x_a \neq x_b$. Pertanto l'insieme $\{x_\kappa\}_\kappa$ è costituito da $(q + 1)^n$ elementi distinti che si dispongono nell'intervallo $[0, 1)$. Dato che $(q + 1)^n > q^n$, esistono $x_a, x_b \in \{x_\kappa\}_\kappa$ tali che $|x_a - x_b| \leq 1/q^n$, ovvero
\begin{align}
...
\textbf{Osservazione:} se poniamo $q = \|k\|_\infty$, $(5.22)$ è valida per ogni $k \in \mathbb{Z}^n \setminus \{0\}$. \\ \\
...
\newpage
\section{Risultati di analisi funzionale}
\textbf{Teorema B.1:} sia $S$ un insieme, allora
\begin{align}
    l^\infty(S) := \{f : S \longrightarrow \mathbb{C} : \|f\|_\infty < \infty\} \tag{B.1}
\end{align}
...
\begin{align}
    \|f_n - f_m\|_\infty < \varepsilon \tag{B.2}
\end{align}
per ogni $n, m > N_\varepsilon$. Allora, per ogni $t \in S$ si ha
\begin{align}
    |f_n(t) - f_m(t)| \leq \|f_n - f_m\|_\infty < \varepsilon \tag{B.3}
\end{align}
...
\begin{align}
    ...
    &\leq 2\int_{\delta}^1 q_n(x)\|f\|_\infty \, dt + \int_{-\delta}^\delta q_n(x)|f(x + t) - f(x)| \, dt \tag{B.13}
\end{align}
...
\begin{align}
    |p_n(x) - f(x)| \leq 2\|f\|_\infty\sqrt{n}(1 - \delta^2)^n(1 - \delta) + 2\varepsilon\delta\sqrt{n}(1 - \delta^2)^n \rightarrow 0 \tag{B.14}
\end{align}
...
\begin{align}
    \|p_n - f\|_{\infty} \rightarrow 0 \tag{B.15}
\end{align}
