...
\subsection{Statistica dei sistemi quantistici}
...
Consideriamo un stato quantistico $\ket{\psi(t)}$ e scriviamolo come combinazione lineare degli autostati dell'hamiltoniano
\begin{align}
    \ket{\psi(t)} = \sum_n \braket{\phi_n|\psi(t)}\ket{\phi_n} = \sum_n a_n(t)\ket{\phi_n} \tag{4.2.1}
\end{align}
dove
\begin{align}
    \sum_n |a_n(t)|^2 = 1 \notag
\end{align}
Sia $A$ un'osservabile, il valore medio di $A$ sullo stato $\ket{\psi}$ è dato da
\begin{align}
    \braket{A} = \braket{\psi(t)|A|\psi(t)} = \sum_{m, n}a_m(t)^*a_n(t)\braket{\phi_m|A|\phi_n} \tag{4.2.2}
\end{align}
A questo punto, possiamo definire la media temporale di $A$ come segue 
\begin{align}
    \overline{\braket{A}} = \lim_{T \rightarrow \infty}\frac{1}{T}\int_0^T dt \, \braket{A} \tag{4.2.3}
\end{align}
Nel contesto dell'ensemble microcanonico, fissata l'energia $E$ del sistema, poniamo
\begin{align}
    \overline{|a_n(t)|^2} = \begin{cases}
        \frac{1}{\Gamma(E)} \qquad &E \leq E_n\ leq E + \Delta \\
        0 \qquad &\text{altrimenti}
    \end{cases} \tag{4.2.4}
\end{align}
Mentre ha senso aspettarsi che 
\begin{align}
    \overline{a_m(t)^*a_n(t)} = 0 \tag{4.2.5}
\end{align}
dato che si tratta di sovrapposizione di stati incoerenti. $(4.2.5)$ è detto \textit{postulato delle fasi random}.
