...
\end{align}
infatti $Z = e^{\beta\mu}$. Poniamo $\eta = \log Z$ (fissato) e supponiamo $\eta >> 1$ (limite classico di bassa densità e alta temperatura), allora
\begin{align}
...
\begin{align}
    f_{3/2}(Z) = \frac{4}{3\sqrt{\pi}}\left(\eta^{3/2} + \frac{\pi^2}{8}\eta^{1/2} + \dots\right) \tag{4.4.26}
\end{align}
Sostituendo da $(4.4.14)$ si ha
\begin{align}
    f_{3/2}(Z) = \frac{\lambda^3\mathcal{N}}{g_sV} \notag
\end{align}
quindi, sostituendo il risultato ottenuto in $(4.4.26)$, si ha
\begin{align}
    \frac{4}{3\sqrt{\pi}}\overbrace{(\beta\mu)^{3/2}}^{\eta^{3/2}}\left(1 + \frac{\pi^2}{8}(\beta\mu)^{-2} + \dots\right) = \frac{1}{g_s}\frac{\mathcal{N}}{V}\left(\frac{\beta h^2}{2\pi m}\right)^{3/2} \tag{4.4.27}
\end{align}
ovvero
\begin{align}
    \mu\left(1 + \frac{\pi^2}{8}(\beta\mu)^{-2} + \dots\right)^{2/3} &= \frac{h^2}{2m}\left(\frac{3}{4\pi g_s}\frac{\mathcal{N}}{V}\right)^{2/3} =: \varepsilon_F \tag{4.4.28}
\end{align}
dove $\varepsilon_F$ è detta \textit{energia di Fermi} del sistema. Notiamo che
\begin{align}
    \mu = \begin{cases}
        \varepsilon_F \qquad &T \rightarrow 0 \\
        \varepsilon_F\left(1 - \frac{\pi^2}{12}(\beta\varepsilon_F)^{-2} + \dots\right) \qquad &T \sim 0
    \end{cases} \tag{4.4.29}
\end{align}
...
\end{align}
Notiamo che nel caso $Ze^{-\beta\varepsilon_p} \rightarrow 1$, il passaggio da sommatoria a integrale non è scontato, questo perché si potrebbe avere una frazione di particelle nello stato fondamentale non infinitesima. \\ \\
A tal proposito, ha senso isolare il termine dovuto all'energia dello stato fondamentale (in questo caso $\varepsilon_p = 0$) e riscrivere $(4.4.1)$ come segue
\begin{align}
...
\end{align}
come fatto in $(4.4.16)$, possiamo riscrivere le funzioni di Bose in termine di serie note, in particolare in termini della funzione $\zeta$ di Riemann
\begin{align}
...
che porta a 
\begin{align}
    \frac{\lambda^3}{v} = g_{3/2}(Z) + \frac{\lambda^3}{V}\left(\frac{Z}{1 - Z}\right) \tag{4.4.36}
\end{align}
Il polt della fugacità $Z$ che risolve $(4.4.36)$ è il seguente
\begin{center}
    \begin{figure}[h]
        \centering
        \includegraphics[width=0.65\linewidth]{immagini/Screenshot 2026-07-09 183550.png}
        \caption{Plot: $Z$ vs $v/\lambda^3$}
        \label{}
    \end{figure}
\end{center}
Pertanto, notiamo che la fugacità è vincolata a $0 < Z < 1$. Dunque, al limite termodinamico $V \rightarrow \infty$, si ottiene
\begin{align}
    Z = \begin{cases}
    ...
    \end{cases} \tag{4.4.37}
\end{align}
Vediamo ora cosa emerge dall'equazione $(4.4.36)$ se facciamo variare la temperatura $T$ del sistema a $v$ fissato. L'equazione
\begin{align}
    \frac{\lambda^3}{v} = g_{3/2}(Z \rightarrow 1) \tag{4.4.38}
\end{align}
...
\end{align}
Per $T > T_c$ si ha $\frac{\lambda^3}{v} = g_{3/2}(Z)$, pertanto
\begin{align}
    \frac{\mathcal{N}_0}{\mathcal{N}} = 1 - \frac{v}{\lambda^3}g_{3/2}(Z) = 0 \tag{4.4.40}
\end{align}
ovvero non ci sono particelle che occupano lo stato fondamentale (o sono una frazione infinitesima). Mentre, per $T < T_c$, se $T_c > 0$, si ha
\begin{align}
    \frac{\mathcal{N}_0}{\mathcal{N}} = 1 - \left(\frac{T}{T_c}\right)^{2/3} \tag{4.4.41}
\end{align}
pertanto si osserva una frazione \textit{non} infinitesima di particelle che occupano lo stato fondamentale. \\ \\
