...
Vediamo, in generale, come $\rho$ evolve nel tempo. Consideriamo una regione $\Omega$ dello spazio delle fasi e sia $\Sigma = \partial \Omega$. La probabilità $P(t)$ di trovare il sistema in un stato contenuto in $\Omega$ in un dato istante $t$ è data da
\begin{gather}
    P(t) = \int_{\Omega}\rho(\mathbf{p}, \mathbf{q}, t) \, d\mathbf{p}d\mathbf{q} \tag{1.1.6}
\end{gather}
Posto $\mathbf{v} = (\dot{\mathbf{p}}, \dot{\mathbf{q}})$, calcolando la derivata totale rispetto al tempo, si ottiene
\begin{gather}
    \frac{dP}{dt} = \int_{\Omega}\frac{\partial \rho}{\partial t} \, d\mathbf{p}d\mathbf{q} \tag{1.1.7}
\end{gather}
Dato che $\mathbf{v}$ è il campo di velocità associato alle equazioni di Hamilton, per il \textit{teorema di Liouville} (appendice) si ottiene
\begin{align}
    \frac{dP}{dt} = -\int_{\Omega}\nabla \cdot (\rho\mathbf{v}) \, d\mathbf{p}d\mathbf{q} \tag{1.1.8}
\end{align}
Prendendo come positivo il flusso uscente da $\Sigma$. Confrontando $(1.1.8)$ e $(1.1.7)$ si ottiene
\begin{gather}
    \int_\Omega \frac{\partial \rho}{\partial t} \, d\mathbf{p}d\mathbf{q} = -\int_{\Omega}\nabla \cdot (\rho\mathbf{v}) \, d\mathbf{p}d\mathbf{q} \tag{1.1.9}
\end{gather}
ovvero
\begin{gather}
    \int_\Omega \left(\frac{\partial \rho}{\partial t} + \nabla \cdot (\rho\mathbf{v})\right) \, d\mathbf{p}d\mathbf{q} = 0 \tag{1.1.10}
\end{gather}
dall'arbitrarietà di $\Omega$ (e quindi $\Sigma$), segue che la funzione integranda in $(1.1.10)$ è identicamente nulla (ovunque per continuità, ovvero anche su insiemi di misura nulla), pertanto concludiamo
\begin{gather}
...
\begin{align}
    ...
    &= \frac{\partial \rho}{\partial t} + \{\mathcal{H}, \rho\} = 0 \tag{1.1.12}
\end{align}
Come detto in precedenza, se il sistema si trova all'equilibrio termodinamico deve per forza valere $\frac{\partial\rho}{\partial t} = 0$, pertanto $\rho$ soddisfa $\{\mathcal{H}, \rho\} = 0$. \\
...
\begin{align}
    \{\mathcal{H}, \rho\} &= \frac{\partial \rho}{\partial \mathcal{H}}\sum_{i = 1}^{3N}\cancel{\left(-\frac{\partial \mathcal{H}}{\partial q_i}\frac{\partial \mathcal{H}}{\partial p_i} + \frac{\partial \mathcal{H}}{\partial p_i}\frac{\partial \mathcal{H}}{\partial q_i}\right)} \tag{1.1.13}
\end{align}
...
\newpage
\subsection{Entropia}
Nel contesto dell'ensemble microcanonico, consideriamo un sistema con energia fissata $E$, definiamo l'entropia del sistema come segue
\begin{gather}
...
\textbf{Dimostrazione:} nel contesto dell'ensemble microcanico la media di una qualsiasi osservabile $A$ è data da 
\begin{align}
...
\begin{align}
    ...
    &\sim \frac{\Delta}{\Gamma(E)}\frac{\partial}{\partial E}\int_{\mathcal{H} \leq E}x_i\frac{\partial \mathcal{H}}{\partial x_j} \, d\mathbf{p}d\mathbf{q} \notag \\
    &= \left(\frac{\partial \Omega}{\partial E}\right)^{-1}\frac{\partial}{\partial E}\int_{\mathcal{H} \leq E}x_i\frac{\partial \mathcal{H}}{\partial x_j} \, d\mathbf{p}d\mathbf{q} \notag \\
    &= \left(\frac{\partial \Omega}{\partial E}\right)^{-1}\frac{\partial}{\partial E}\int_{\mathcal{H} \leq E}x_i\frac{\partial}{\partial x_j}(\mathcal{H} - E) \, d\mathbf{p}d\mathbf{q} \notag \\
    &= \left(\frac{\partial \Omega}{\partial E}\right)^{-1}\frac{\partial}{\partial E}\left(\cancel{\int_{\mathcal{H} \leq E}\frac{\partial}{\partial x_j}(x_i(\mathcal{H} - E)) \, d\mathbf{p}d\mathbf{q}} - \delta_{ij}\int_{\mathcal{H} \leq E}(\mathcal{H} - E) \, d\mathbf{p}d\mathbf{q}\right) \notag
\end{align}
...
\begin{align}
    \braket{x_i\frac{\partial \mathcal{H}}{\partial x_j}} &= \delta_{ij}\left(\frac{\partial \Omega}{\partial E}\right)^{-1}\frac{\partial}{\partial E}\int_{\mathcal{H} \leq E}(E - \mathcal{H}) \, d\mathbf{p}d\mathbf{q} \notag \\
    &= \delta_{ij}\left(\frac{\partial\Omega}{\partial E}\right)^{-1}\int_{\mathcal{H} \leq E}\frac{\partial}{\partial E}(E - \mathcal{H}) \, d\mathbf{p}d\mathbf{q} + \cancel{(E - \mathcal{H})|_{\mathcal{H} = E}} \notag \\
...
\end{gather}
dove $\omega_{3N}(R)$ è la misura del disco $3N$-dimensionale di raggio $R = \sqrt{2mE}$. Dobbiamo quindi determinare la seguente quantità per $n$ generico
\begin{gather}
    \omega_n(R) = \int_{\|\mathbf{x}\| \leq R}dx_1 \dots dx_n \notag
\end{gather}
Ragionando per induzione, sappiamo che la misura del disco bidimensionale di raggio $R$ è $\pi R^2$ e la misura del disco tridimensionale di raggio $R$ è $\frac{4}{3}\pi R^3$. Supponiamo che anche la misura del disco $n - 1$ dimensionale sia proporzionale a $n - 1$, ovvero che esista $c_{n - 1} > 0$ tale che $m(\mathbf{D}^{n - 1}) = c_{n - 1}R^{n - 1}$ e verifichiamo se vale anche per la sfera $n$ dimensionale $\mathbf{D}^n$. $\mathbf{D}^n$ è identificata dalla seguente equazione
\begin{gather}
    \mathbf{D}^n = \{\mathbf{x} \in \mathbf{R}^n : x^2_1 + \dots + x^2_n \leq R^2\} \notag
\end{gather}
Allora
\begin{align}
    \omega_n(R) &= \int_{-R}^{R}dx_n\int_{x^2_1 + \dots + x^2_{n - 1} \leq R^2 - x^2_n}dx_1 \dots dx_{n - 1}dx_n \notag \\
...
\end{align}
in questo caso con $\Gamma$ si intende la funzione gamma di Eulero. Segue che
\begin{gather}
    c_{n - 1} = \frac{\pi^{\frac{n - 1}{2}}}{\Gamma\left(\frac{n}{2} + \frac{1}{2}\right)} \tag{1.4.5}
\end{gather}
Pertanto si ottiene
\begin{align}
    \omega_n(R) &= \frac{\pi^{\frac{n - 1}{2}}}{\Gamma\left(\frac{n}{2} + \frac{1}{2}\right)}R^n\int_{-1}^1(1 - u^2)^{\frac{n - 1}{2}} \, du \notag \\
    &= \frac{\pi^{\frac{n}{2}}}{\Gamma\left(\frac{n}{2} + 1\right)}R^n \tag{1.4.6}
\end{align}
dove si è usato
\begin{align}
    \int_{-1}^1 (1 - u^2)^{\frac{n - 1}{1}} \, du = \sqrt{\pi}\frac{\Gamma\left(\frac{n}{2} + \frac{1}{2}\right)}{\Gamma\left(\frac{n}{2} + 1\right)} \notag
\end{align}
...
\end{gather}
per poter trattare l'espressione $(1.4.7)$ dell'entropia $S$ come potenziale termodinamico dobbiamo far uso della \textit{formula di Stirling} (vedere appendice), ovvero
\begin{gather}
...
Tuttavia, l'espressione $(1.4.8)$ dell'entropia non è estensiva a causa del termine $V$ all'interno del logaritmo. Questo accade perché non abbiamo tenuto conto dell'indistinguibilità classica delle particelle nell'integrazione rispetto a $d\mathbf{q}$ per il calcolo di $\Omega(E)$. Necessitiamo dunque di un fattore $N!$, detto \textit{fattore di Gibbs}, a dividere $\Gamma$ (o $\Omega$) per correggere il problema. Tale fattore non sarà necessario quando considereremo l'indistinguibilità quantistica delle particelle. \\ \\
...
\end{align}
identifica un rombo nello spazio $N$ dimensionale degli impulsi. In questo caso, dato che $|\mathbf{p}_i| = \sqrt{p^2_{ix} + p^2_{iy} + p^2_{iz}}$, è conveniente passare in coordinate polari, allora si ha
\begin{align}
...
Nel contesto dell'ensemble microcanonico, consideriamo un sistema $X$ descritto dalla terna $(E, N, V)$ e consideriamo un sottosistema $X_1$ di $X$ descritto dalla terna $(E_1, N_1, V_1)$. Poniamo $X_2 = X \setminus X_1$, il complementare di $X_1$, descritto dalla terna $(E_2, N_2, V_2)$ e supponiamo che $V_1 << V_2$ e $N_1 << N_2$. Inoltre, supponiamo che $N_1, N_2, V_1, V_2$ siano fissate, ovvero che $X_1$ e $X_2$ possano interagire solamente per scambio di energia. \\
Al limite termodinamico, ovvero $V_1 >> 1$, l'interazione tra $X_1$ e $X_2$, che è essenzialmente limitata ad una regione limitrofa al bordo di $X_1$, diventa infinitesima, e quindi trascurabile. Possiamo dunque trattare $X_1$ come un sistema isolato. \\
...
\begin{align}
    \rho_1(\mathbf{p}^{(1)}, \mathbf{q}^{(1)}) \propto \Gamma_2(E - E_1) \notag
\end{align}
ovvero
\begin{align}
    \rho_1(\mathbf{p^{(1)}}, \mathbf{q}^{(1)}) \propto \exp\left(\frac{S_2(E - E_1)}{k}\right) \tag{2.1.1}
\end{align}
Dato che $E_1 << E$, possiamo sviluppare l'esponente in $(2.1.2)$ in serie di Taylor
\begin{align}
    \rho_1(\mathbf{p^{(1)}}, \mathbf{q}^{(1)}) \propto \exp\left(\frac{S_2(E)}{k} - \frac{E_1}{k}\frac{\partial S_2(E)}{\partial E}\right) \notag
\end{align}
quindi
\begin{align}
    \rho_1(\mathbf{p}^{(1)}, \mathbf{q}^{(1)}) &\propto \exp\left(\frac{S_2(E)}{k}-\frac{E_1}{kT}\right) \tag{2.1.2}
\end{align}
dove si è usato $\frac{\partial S_2}{\partial E} = \frac{1}{T}$. In particolare, $S_2$ non dipende dalla variabili dinamiche microscopiche del sottosistema $X_1$, pertanto possiamo includere il fattore $e^{S_2(E)/k}$ nella costante di proporzionalità, ottenendo
\begin{align}
...
\end{align}
Resta da determinare la costante di normalizzazione, che poi, come nel capitolo precedente, determinerà il potenziale termodinamico associato alla descrizione del sistema. Definiamo 
\begin{align}
...
L'ensemble statistico associato a sistemi all'equilibrio termodinamico che si trovano a contatto con un reservoir e sono descritti dalle variabili termodinamiche $(N, T, V)$ è detto \textit{ensemble canonico}. \\ \\
Osserviamo che il termine $E$ nella definizione macroscopica dell'energia libera non è l'energia del sistema, infatti quest'ultima è una variabile stocastica nel contesto dell'ensemble canonico, dato che il sistema scambia energia con il reservoir per termalizzarsi. La corretta definizione di $F$ è 
\begin{align}
...
Vediamo ora come si comportano le fluttuazioni dell'energia rispetto alla media. Notiamo che la definizione di momento di ordine due (rispetto a $\rho$) dell'hamiltoniana $\mathcal{H}$ è legata al concetto di varianza dell'energia, infatti si ha 
\begin{align}
...
\begin{align}
    0 &= \frac{\partial}{\partial \beta}\left(\frac{1}{h^{3N}}\int d\mathbf{p}d\mathbf{q} \, (\braket{\mathcal{H}} - \mathcal{H})e^{\beta(F - \mathcal{H})}\right) \notag \\
    &= \frac{1}{h^{3N}}\int d\mathbf{p}d\mathbf{q} \, \left(\frac{\partial \braket{\mathcal{H}}}{\partial \beta} + (\braket{\mathcal{H}} - \mathcal{H})\left(F - \mathcal{H} + \beta\left(\frac{\partial F}{\partial \beta}\right)_{N, V}\right)\right)e^{\beta(F - \mathcal{H})} \tag{2.3.5}
\end{align}
...
\begin{align}
    -kT^2\frac{\partial U}{\partial T} + \braket{(U - \mathcal{H})^2} = 0 \notag
\end{align}
ovvero 
\begin{align}
    \braket{(U - \mathcal{H})^2} = kT^2\frac{\partial U}{\partial T} \tag{2.3.8}
\end{align}
...
Inoltre, per il \textit{teorema dell'equipartizione dell'energia}, sappiamo che $c_V \propto N$, pertanto
\begin{align}
    \frac{\sqrt{\braket{(U - \mathcal{H})^2}}}{U} = \frac{\sqrt{kT^2c_V}}{U} \propto \frac{1}{\sqrt{N}} \tag{2.3.10}
\end{align}
pertanto le fluttuazioni dell'energia sono trascurabili al limite termodinamico ($N >> 1$).
...
\end{align}
Sostituendo $(2.8.2)$ in $(2.8.1)$ si ha 
\begin{align}
...
\end{align}
Dato che sia $S_{mic}$ che $E$ sono proporzionali a $N$, ci aspettiamo che, nel limite termodinamico $N \rightarrow \infty$, l'integrale in $(2.8.3)$ riceva contributo solamente per $E$ attorno al punto di massimo $\overline{E}$ della funzione integranda. In particolare 
\begin{align}
    \frac{\partial}{\partial E}(TS_{mic}(E) - E)_{E = \overline{E}} = 0 \notag
\end{align}
fornisce le seguenti condizioni su $\overline{E}$
\begin{align}
    \begin{cases}
        T\left(\frac{\partial S_{mic}}{\partial E}\right)_{E = \overline{E}} &= 1 \\
        \left(\frac{\partial^2S_{mic}}{\partial E^2}\right)_{E = \overline{E}} &< 0
    \end{cases} \notag
\end{align}
La prima condizione implica che $\overline{E} = U$. Sviluppiamo l'esponente della funzione integranda attorno ad $\overline{E}$
\begin{align}
    TS_{mic}(E) - E &\sim TS_{mic}(\overline{E}) - \overline{E} + \frac{1}{2}(E - \overline{E})^2T\left(\frac{\partial^2S_{mic}}{\partial E^2}\right)_{E = \overline{E}} \notag \\
    &= TS_{mic}(U) - U + \frac{1}{2}(E - U)^2T\left(\frac{\partial^2S_{mic}}{\partial E^2}\right)_{E = U} \tag{2.8.4}
\end{align}
Ponendo $\eta = E - U$ si ottiene 
\begin{align}
    Q_N &\sim \exp\left(\beta(TS_{mic}(U) - U)\right)\int_{-U}^\infty d\eta \, \exp\left(\frac{1}{2k}\left(\frac{\partial^2S_{mic}}{\partial E^2}\right)_{E = U}\eta^2\right) \notag \\
    &\sim \exp\left(\beta(TS_{mic}(U) - U)\right)\int_{-\infty}^\infty d\eta \, \exp\left(\frac{1}{2k}\left(\frac{\partial^2S_{mic}}{\partial E^2}\right)_{E = U}\eta^2\right) \tag{2.8.5}
\end{align}
In particolare, per il \textit{teorema di equipartizione}, si ha
\begin{align}
    \frac{\partial^2S_{mic}}{\partial E^2} = \frac{\partial}{\partial E}\left(\frac{1}{T}\right) = -\frac{1}{T^2}\frac{\partial T}{\partial E} \propto \frac{1}{N} \notag
\end{align}
Concludiamo che la funzione integranda in $(2.8.5)$ è una gaussiana con varianza $\sigma = \sqrt{N}$ 
\begin{align}
    Q_N \sim \exp\left(\beta(TS_{mic}(U) - U)\right)\left(-2\pi k\left(\frac{\partial^2S_{mic}}{\partial E^2}\right)^{-1}_{E = U}\right)^{1/2} \tag{2.8.6}
\end{align}
Pertanto l'entropia canonica è maggiore dell'entropia microcanonica (a causa delle fluttazioni) e differisce da quest'ultima di un termine che va come $\log N$. In particolare, usando la definizione statistica dell'energia libera, si ottiene 
\begin{align}
    F = U - TS_{can} \sim U - TS_{mic} - \frac{kT}{2}\log\left(2\pi kT^2c_V\right) \tag{2.8.7}
\end{align}
quindi 
\begin{align}
...
Consideriamo un sistema $X$, composto da $N$ particelle indipendenti e indistinguibili in un volume $V$, a contatto con un reservoir termalizzato e suddividiamo $X$ in due sottosistemi $X_1, X_2$ caratterizzati dalle grandezze termodinamiche $N_1, V_1$ e $N_2, V_2$, che soddisfano 
\begin{align}
    N &= N_1 + N_2 \notag \\
    V &= V_1 + V_2 \notag
\end{align}
...
Per definizione
\begin{align}
    Q_N &= \frac{1}{h^{3N}N!}\int d\mathbf{p}d\mathbf{q} \, e^{-\beta\mathcal{H}_N} \notag
\end{align}
Cerchiamo un modo per scrivere la funzione di partizione canonica come segue
\begin{align}
    Q_N = \sum_{N_1}\frac{1}{h^{3N_1}N_1!}\int d\mathbf{p}_1d\mathbf{q}_1 \, (\dots) \tag{3.1.1}
\end{align}
...
\begin{align}
    F_{N - N_1}(T, V - V_1) - F_N(T, V) &\sim -\underbrace{\left(\frac{\partial F}{\partial N}\right)_{T, V}}_{\mu}N_1 - \underbrace{\left(\frac{\partial F}{\partial V}\right)_{N, T}}_{-P}V_1 \notag \\
...
\end{align}
ovvero $P(N, E_N)$ è una distribuzione poissoniana. \\ \\
...
\begin{align}
    \braket{(\Delta N)^2} = Z\left(\frac{\partial}{\partial Z}\braket{N}\right)_{T, V} \tag{3.2.7}
\end{align}
Si ha che 
\begin{align}
    Z\left(\frac{\partial}{\partial Z}\braket{N}\right)_{T, V} &= Z\left(\frac{\partial}{\partial Z}\frac{\sum_{N}NZ^NQ_N}{\sum_{N}Z^NQ_N}\right)_{T, V} \notag \\
...
\end{align}
Pertanto $(3.2.7)$ è dimostrata. Segue che 
\begin{align}
    \braket{(\Delta N)^2} = Z\frac{\partial}{\partial Z}\left(Z\frac{\partial}{\partial Z}\log \tilde{Q}\right)_{T, V} \tag{3.2.9}
\end{align}
Definiamo il potenziale termodinamico di Gibbs $G$
\begin{align}
    G = U - TS + PV = F + PV \tag{3.2.10}
\end{align}
Il differenziale di $G$ è dato da
\begin{align}
    dG = -SdT + VdP + \mu dN \tag{3.2.11}
\end{align}
$G$ è un potenziale termodinamico, pertanto è una funzione estensiva delle variabili termodinamiche. Confrontando con $(3.2.11)$ concludiamo che 
\begin{align}
    G = N\mu(T, P) \tag{3.2.12}
\end{align}
Infatti, $T$ e $P$ sono grandezze intensive e necessariamente la dipendenza da $N$ deve essere fattorizzata per far sì che $G$ sia estensiva nel complesso. Allora, otteniamo una nuova espressione per il differenziale $dG$
\begin{align}
    dG = Nd\mu + \mu dN \tag{3.2.13}
\end{align}
Confrontando $(3.2.11)$ e $(3.2.13)$ e ricordando che il sistema è termalizzato a temperatura $T$ ($dT = 0$), si ottiene
\begin{align}
    VdP + \mu dN = Nd\mu + \mu dN \notag
\end{align}
ovvero
\begin{align}
    d\mu = \frac{V}{N}dP \tag{3.2.14}
\end{align}
Riscriviamo $(3.2.9)$ come segue
\begin{align}
    \braket{(\Delta N)^2} = Z\frac{\partial}{\partial Z}\left(Z\frac{\partial}{\partial Z}\left(\frac{PV}{kT}\right)\right)_{T, V} \tag{3.2.15}
\end{align}
Osserviamo che 
\begin{align}
    Z\frac{\partial}{\partial Z} = \frac{\partial}{\partial \log Z} = \frac{\partial}{\partial(\beta\mu)} = \frac{1}{\beta}\frac{\partial}{\partial\mu} = \frac{1}{\beta}\frac{\partial P}{\partial \mu}\frac{\partial}{\partial P} = \frac{1}{\beta v}\frac{\partial}{\partial P} \notag
\end{align}
Sostituendo in $(3.2.15)$
\begin{align}
    \braket{(\Delta N)^2} &= \frac{1}{\beta v}\frac{\partial}{\partial P}\left(\frac{1}{\beta v}\frac{\partial}{\partial P}\left(\frac{PV}{kT}\right)\right)_{T, V} \notag \\
    &= \frac{V}{\beta v}\frac{\partial}{\partial P}\left(\frac{1}{v}\right)_{T, V} \notag \\
    &= -\frac{V}{\beta v^3}\left(\frac{\partial v}{\partial P}\right)_{T, V} \notag \\
    &= -\frac{\braket{N}}{\beta v^2}\underbrace{\left(\frac{\partial P}{\partial v}\right)^{-1}_{T, V}}_{\text{compressibilità isoterma}} \tag{3.2.16}
\end{align}
Osserviamo che le fluttuzioni del numero di particelle sono legate alla compressibilità isoterma $\kappa_T = -\frac{1}{v}\left(\frac{\partial P}{\partial v}\right)^{-1}_T$. Inoltre
\begin{align}
    \frac{\sqrt{\braket{(\Delta N)^2}}}{\braket{N}} = \left(\frac{\kappa_T}{\beta v}\right)^{1/2}\left(\frac{1}{\braket{N}}\right)^{1/2} \tag{3.2.17}
\end{align}
ovvero le fluttuazioni diventano trascurabili al limite termodinamico $\braket{N} \rightarrow \infty$ ($v$ è fissata dato che al limite termidinamico $V \rightarrow \infty$ e $\braket{N} \rightarrow \infty$ in modo tale che $V/N \propto 1$).
...
\begin{align}
    \rho(E_n) &= \begin{cases}
        \frac{1}{\Gamma(E_n)} \qquad E &\leq E_n \leq E + \Delta \\
        0 \qquad &\text{altrimenti}
    \end{cases} \qquad &\text{microcanonico} \notag \\
    \rho(E_n) &= \frac{1}{Q_N}e^{-\beta E_n} \qquad &\text{canonico} \notag \\
    \rho(E_n, N) &= \frac{1}{\tilde{Q}}e^{-\beta(E_n + \mu N)} \qquad &\text{grancanonico} \notag 
\end{align}
...
Consideriamo un sistema quantistico composto da $N$ particelle indipendenti. La funzione di partizione canonica associata a tale sistema è data da
\begin{align}
...
\end{align}
dove $\{n_\varepsilon\}$ è l'insieme delle possibili configurazioni che soddisfano il vincolo $(4.3.3)$, mentre $g\{n_\varepsilon\}$ è il peso statistico assegnato ad una data configurazione. In particolare, il valore di $g$ per una data configurazione differesce nel caso di fermioni e bosoni. Per bosoni si ha
\begin{align}
...
\end{align}
Nota la funzione di partizione grancanonica, possiamo determinare le espressioni caratteristiche dell'ensemble grancanonico. In particolare, indicando con $\mathcal{N}$ il numero medio di particelle, si ha
\begin{align}
...
Nel limite termodinamico gli stati energetici sono molto densi, pertanto è possibile sostituire la sommatoria con un integrale nello spazio delle fasi
\begin{align}
...
\begin{align}
    ...
    \mathcal{N} &= g_s\int \frac{d\mathbf{p}d\mathbf{q}}{h^3} \, \frac{1}{Z^{-1}e^{\beta\mathcal{H}(\mathbf{p}, \mathbf{q})} + (-1)^{g_s}} \tag{4.3.16} \\ \notag \\
    U &= g_s\int \frac{d\mathbf{p}d\mathbf{q}}{h^3} \, \frac{\mathcal{H}(\mathbf{p}, \mathbf{q})}{Z^{-1}e^{\beta\mathcal{H}(\mathbf{p}, \mathbf{q})} + (-1)^{g_s}} \tag{4.3.17}
\end{align}
...
Infine, osserviamo che nel caso di particelle libere possiamo giustificare direttamente il passaggio da sommatoria a integrale. In particolare, sappiamo che la soluzione dell'hamiltoniano di particelle libera è data da
\begin{align}
    u(x, y, z) = \left(\frac{2}{L}\right)^{3/2}\sin\frac{p_x x}{\hbar}\sin\frac{p_y y}{\hbar}\sin\frac{p_z z}{\hbar} \notag
\end{align}
dove 
\begin{align}
    p_i = \frac{\pi\hbar}{L}n_i \qquad i = x, y, z \notag
\end{align}
Allora
\begin{align}
    \sum_{n_x, n_y, n_z} 1 = \left(\frac{L}{\pi\hbar}\right)^{3}\sum_{n_x, n_y, n_z}\left(\frac{\pi\hbar}{L}\right)^{3}(n_x + 1 - n_x)(n_y + 1 - n_y)(n_z + 1 - n_z) \notag
\end{align}
notiamo che 
\begin{align}
    \frac{\pi\hbar}{L}(n_i + 1 - n_i) = \Delta p_i \qquad i = x, y, z \notag
\end{align}
Pertanto, al limite termodinamico $L \rightarrow \infty$, otteniamo
\begin{align}
    \sum_{n_x, n_y, n_z} 1 = \left(\frac{L}{\pi\hbar}\right)^3\int_0^\infty dp_xdp_ydp_z = \frac{V}{h^3}\int_{-\infty}^\infty dp_xdp_ydp_z \notag
\end{align}
...
Nel contensto dell'ensemble grancanonico quantistico, consideriamo una trasformazione per la quale 
\begin{align}
...
\begin{align}
    \frac{f_{3/2}(Z)}{\lambda^3} \propto 1 \tag{4.4.18}
\end{align}
dunque, dato che $\lambda^3$ va come $\beta^{3/2}$, otteniamo
\begin{align}
    f_{3/2}(Z) \propto (\log Z)^{3/2} \tag{4.4.19}
\end{align}
infatti $Z = e^{\beta\mu}$. Poniamo $\eta = \log Z$ (fissato), allora
\begin{align}
...
\end{align}
Sviluppando in serie $x^{3/2}$ attorno a $\eta$ e ponendo $t = x - \eta$, si ottiene
\begin{align}
    f_{3/2}(Z) &\sim \frac{4}{3\sqrt{\pi}}\int_{-\eta}^\infty dt \, \frac{e^t}{(e^t + 1)^2}\left(\eta^{3/2} + \frac{3}{2}\eta^{1/2}t + \frac{3}{8}\eta^{-1/2}t^2 \dots\right) \tag{4.4.21}
\end{align}
Per linearità dell'integrale si ha
\begin{align}
    \int_{-\eta}^\infty(\dots) = \int_{-\infty}^\infty(\dots) - \int_{-\infty}^{-\eta}(\dots) \notag
\end{align}
Inoltre, il secondo integrale risulta essere dell'ordine di $e^{-\eta}$, pertanto
\begin{align}
    f_{3/2}(Z) \sim \frac{4}{3\sqrt{\pi}}\int_{-\infty}^\infty dt \, \frac{e^t}{(e^t + 1)^2}\left(\eta^{3/2} + \frac{3}{2}\eta^{1/2}t + \frac{3}{8}\eta^{-1/2}t^2 \dots\right) + O(e^{-\eta}) \notag
\end{align}
Fissato $n \in \mathbf{N}$, poniamo
\begin{align}
    I_n = \int_{-\infty}^\infty dt \, t^n\frac{e^t}{(e^t + 1)^2} \tag{4.4.22}
\end{align}
Allora, sostituendo in $(4.4.21)$ si ha 
\begin{align}
    f_{3/2}(Z) \sim \frac{4}{3\sqrt{\pi}}\left(\eta^{3/2}I_0 + \frac{3}{2}\eta^{1/2}I_1 + \frac{3}{8}\eta^{-1/2}I_2 \dots\right) + O(Z^{-1}) \tag{4.4.23}
\end{align}
...
\begin{align}
    ...
    &= 2(n!)\sum_{l = 1}^\infty \frac{(-1)^{l - 1}}{l^n} \notag \\
    &= 2(n!)\left(\zeta(n) - \frac{2}{2^n} - \frac{2}{4^n} - \frac{2}{6^n} \dots\right) \notag \\
    &= 2(n!)\left(1 - \frac{1}{2^{n - 1}}\right)\zeta(n)  \tag{4.4.25}
\end{align}
In particolare, $I_0 = 1$ e $I_2 = \frac{\pi^2}{3}$, pertanto
\begin{align}
    f_{3/2}(Z) = \frac{4}{3\sqrt{\pi}}\left((\log Z)^{3/2} + \frac{\pi^2}{8}(\log Z)^{-1/2} + \dots\right) + O(Z^{-1}) \tag{4.4.26}
\end{align}
Assegnato un valore di $\frac{\lambda^3}{v}$ è possibile determinare il valore di $Z$ associato facendo riferimento al grafico di $f_{3/2}$. Inoltre, notiamo che $Z$ crescere in maniera monotona al crescere di $\frac{\lambda^3}{v}$.
\begin{center}
    \begin{figure}[h]
        \centering
        \includegraphics[width=0.50\linewidth]{immagini/Screenshot 2026-07-10 152458.png}
        \caption{$f_{3/2}(Z)$}
        \label{}
    \end{figure}
\end{center}
Per basse temperature e alte densità, ovvero in condizioni in cui ci aspettiamo di osservare effetti quantistici, vale $\frac{\lambda^3}{v} >> 1$, pertanto anche $Z >> 1$. Allora $(4.4.26)$ si riduce a 
\begin{align}
    f_{3/2}(Z) \sim \frac{4}{3\sqrt{\pi}}(\log Z)^{3/2} \tag{4.4.27}
\end{align}
Confrontando con $(4.4.14)$ si ottiene 
\begin{align}
    \frac{1}{v}\left(\frac{\beta h^2}{2\pi m}\right)^{3/2} = \frac{4}{3\sqrt{\pi}}(\log Z)^{3/2} \tag{4.4.28}
\end{align}
Per cui
\begin{align}
    Z \sim e^{\beta\varepsilon_F} \qquad \varepsilon_F = \frac{\hbar^2}{2m}\left(\frac{6\pi^2}{v}\right)^{2/3} \tag{4.4.29}
\end{align}
dove $\varepsilon_F$ è il potenziale chimico al limite di temperatura nulla, detto \textit{energia di Fermi}. \\
Nel limite $T \rightarrow 0$, la funzione integranda che definisce il numero medio di particelle del contesto dell'ensemble grancanonico quantistico diventa una funzione a gradino. Questo è dovuto al principio di esclusione di Pauli, infatti uno stesso livello energetico non può essere occupato simultaneamente da due fermioni. \\ \\
Concludiamo che $\varepsilon_F$ è il livello energetico di singola particella più alto ad essere occupato nel limite di temperatura nulla. Inoltre, per il principio di esclusione di Pauli, osserviamo che sotto all'energia di Fermi ci sono esattamente $\mathcal{N}$ livelli energetici. \\ \\
Per temperature prossime allo zero assoluto si ha
\begin{align}
    \mu = kT\log Z = \varepsilon_F\left(1 - \frac{\pi^2}{12}\left(\frac{kT}{\varepsilon_F}\right)^2 + \dots\right) \notag
\end{align}
sviluppando il potenziale chimico $\mu$.

\subsubsection*{Condensazione di Bose-Einstein}
Consideriamo ora il caso di bosoni liberi, poniamo $\varepsilon_p = \frac{p^2}{2m}$. La funzione di partizione grancanonica è data da
\begin{align}
    \tilde{Q} = \prod_p \frac{1}{1 - Ze^{-\beta\varepsilon_p}} \tag{4.4.30}
\end{align}
Notiamo che nel limite $Z \rightarrow 1$ il passaggio da sommatoria a integrale non è scontato, questo perché il termine della sommatoria in $(4.4.30)$ che corrisponde a $\varepsilon_p = 0$ diverge per $Z \rightarrow 1$. \\ \\
A tal proposito, isoliamo il termine dovuto all'energia dello stato fondamentale $\varepsilon_p = 0$ (ovvero $p = 0$) e riscriviamo $(4.4.1)$ come segue
\begin{align}
    \frac{P}{kT} = -\frac{4\pi g_s}{h^3}\int_0^\infty dp \, p^2\log(1 - Ze^{-\beta\frac{p^2}{2m}}) - \overbrace{\log(1 - Z)}^{\text{contr. stato fond.}}\tag{4.4.31}
\end{align}
Allora 
\begin{align}
    \mathcal{N} = \frac{4\pi g_sV}{h^3}\int_0^\infty dp \, p^2\frac{1}{Z^{-1}e^{\beta\frac{p^2}{2m}} - 1} + \overbrace{\frac{Z}{1 - Z}}^{\mathcal{N}_0} \tag{4.4.32}
\end{align}
Analogamente a quanto fatto nel caso di fermioni, definiamo le \textit{funzioni di Bose}
\begin{align}
    g_\nu(Z) = \frac{1}{\Gamma(\nu)}\int_0^\infty dx \, \frac{x^{\nu - 1}}{Z^{-1}e^x - 1} \tag{4.4.33}
\end{align}
Notiamo immediatamente che $g_\nu$ è crescente monotona per $0 < Z < 1$. Inoltre, come fatto in $(4.4.16)$, possiamo riscrivere $g_\nu$ in termini di serie note
\begin{align}
    g_\nu(Z) = \sum_{l = 1}^\infty \frac{Z^l}{l^\nu} \notag
\end{align}
In particolare, $g_\nu(0) = 0$ e $g_\nu(1) = \zeta(\nu)$, pertanto $g_\nu(Z) \leq \zeta(\nu)$ per ogni $Z \in [0, 1]$. Da $(4.4.32)$ otteniamo
\begin{align}
    \overbrace{\frac{\mathcal{N}}{V}}^{\text{intensivo}} = \overbrace{\frac{4\pi g_s}{h^3}\int_0^\infty dp \, p^2\frac{1}{Z^{-1}e^{\beta\frac{p^2}{2m}} - 1}}^{\text{intensivo}} + \overbrace{\frac{1}{V}\left(\frac{Z}{1 - Z}\right)}^{\text{estensivo}} \tag{4.4.34}
\end{align}
Per far sì che l'espressione $(4.4.34)$ abbia senso, deve valere
\begin{align}
    \frac{Z}{1 - Z} \sim V \qquad Z \rightarrow 1 \notag
\end{align}
in modo tale da rendere anche il terzo termine intensivo. \\
Riscrivendo $(4.4.34)$ in termini delle funzioni di Bose e del parametro $\lambda$ introdotto precedentemente, si ottiene
\begin{align}
    \frac{\mathcal{N}}{V} = \frac{1}{\lambda^3}g_{3/2}(Z) + \frac{1}{V}\left(\frac{Z}{1 - Z}\right) \tag{4.4.35}
\end{align}
che porta a 
\begin{align}
    \lambda^3\frac{\mathcal{N}_0}{V} = \frac{\lambda^3}{v} - g_{3/2}(Z) \tag{4.4.36}
\end{align}
Notiamo che se 
\begin{align}
    \frac{\lambda^3}{v} < \zeta(3/2) \tag{4.4.37}
\end{align}
esiste unica $Z \in [0, 1]$ tale che
\begin{align}
    \frac{\lambda^3}{v} = g_{3/2}(Z) \tag{4.4.38}
\end{align}
Pertanto si ottiene $\mathcal{N}_0 = 0$ al limite termodinamico. Mentre, se 
\begin{align}
    \frac{\lambda^3}{v} > \zeta(3/2) \tag{4.4.39}
\end{align}
non esiste $Z \in [0, 1]$ che soddisfa $(4.4.38)$, infatti, come già visto, $\zeta(3/2)$ è il valore massimo assunto da $g_{3/2}$ per $Z \in [0, 1]$. Pertanto, se vale $(4.4.39)$, non resta che porre $Z = 1$ e concludere che i livelli eccitati sono saturi, quindi le particelle iniziano a condensarsi nello stato fondamentale. Tale fenomeno è detto \textit{condensazione di Bose-Einstein}. \\ \\
Il plot di $Z$ soluzione di $(4.4.36)$ è il seguente
\begin{center}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.40\linewidth]{immagini/Screenshot 2026-07-10 142023.png}
        \caption{$Z$ soluzione di $(4.4.36)$}
        \label{}
    \end{figure}
\end{center}
Notiamo che $0 < Z < 1$. Inoltre, al limite $V \rightarrow \infty$ si ottiene
\begin{align}
    Z = \begin{cases}
        1 \qquad &\frac{\lambda^3}{v} \leq \zeta(3/2) \\
        \text{soluzione di } (4.4.38) \qquad &\frac{\lambda^3}{v} > \zeta(3/2)
    \end{cases} \tag{4.4.40}
\end{align}
Fissato $v$, l'equazione
\begin{align}
    \frac{\lambda^3_c}{v} = g_{3/2}(1) \tag{4.4.41}
\end{align}
identifica una determinata temperatura $T_c$, detta \textit{temperatura critica}, data da
\begin{align}
    T_c = \frac{h^2}{2\pi km}\left(\frac{1}{v\zeta(3/2)}\right)^{2/3} \tag{4.4.42}
\end{align}
Per $T > T_c$ si ha $\frac{\lambda^3}{v} < \zeta(3/2)$, pertanto
\begin{align}
    \frac{\mathcal{N}_0}{\mathcal{N}} = 1 - \frac{v}{\lambda^3}g_{3/2}(Z) = 0 \tag{4.4.43}
\end{align}
ovvero la frazione di particelle che occupa lo stato fondamentale diventa infinitesima al limite termodinamico. Mentre, per $T < T_c$ si ottiene
\begin{align}
    \frac{\mathcal{N}_0}{\mathcal{N}} = 1 - \frac{v}{\lambda^3}\overbrace{g_{3/2}(1)}^{\lambda^3_c/v} = 1 - \left(\frac{T}{T_c}\right)^{2/3} \tag{4.4.44}
\end{align}
ovvero la frazione di particelle che occupano lo stato fondamentale rimane finita anche al limite termodinamico, ciò significa che $\mathcal{N}_0 \sim V$.
\begin{center}
    \begin{figure}[h]
        \centering
        \includegraphics[width=0.40\linewidth]{immagini/Screenshot 2026-07-10 143224.png}
        \caption{$\mathcal{N}_0/\mathcal{N}$ vs $T$ per $T < T_c$}
        \label{}
    \end{figure}
\end{center}
Analogamente, se si fissa la temperatura del sistema e si lascia variare $v$, si ottiene un'equazione che identifica una \textit{densità critica} $v_c$ che soddisfa
\begin{align}
    Z = \begin{cases}
        1 \qquad &v \leq v_c \\
        \text{soluzione di } (4.4.38) \qquad &v > v_c
    \end{cases} \qquad
    P = \begin{cases}
        \frac{kT}{\lambda^3}\zeta(3/2) \qquad &v \leq v_c \\
        \frac{kT}{\lambda^3}g_{3/2}(Z) \qquad &v > v_c
    \end{cases} \tag{4.4.45}
\end{align}
Per poter osservare il fenomeno di condensazione di Bose-Einstein deve esistere $T_c > 0$. Tuttavia, non tutti i sistemi ammettono temperatura critica non nulla, un primo esempio è il caso di bosoni liberi in un dominio bidimensionale.

\newpage
\subsection{Equivalente quantistico del teorema di equipartizione dell'energia}
Consideriamo un sistema governato da un hamiltoniano dipendente solamente dell'impulso, $\mathcal{H} = \mathcal{H}(p)$. Allora, integrando per parti l'espressione $(4.3.15)$ si ottiene
\begin{align}
    \frac{PV}{kT} &= (-1)^{g_s}\frac{4\pi g_sV}{h^3}\int_0^\infty dp \, p^2\log(1 + (-1)^{g_s}Ze^{-\beta\mathcal{H}}) \notag \\
    &= \frac{g_sV}{3h^3}\int_0^\infty 4\pi p^2dp \, \frac{Z\beta p\frac{d\mathcal{H}}{dp}}{1 + (-1)^{g_s}Ze^{-\beta\mathcal{H}}} \notag \\
    &= \frac{g_s\beta V}{3h^3}\int d\mathbf{p} \, \frac{p\frac{d\mathcal{H}}{dp}}{Z^{-1}e^{\beta\mathcal{H}} + (-1)^{g_s}} \notag \\
    &= \frac{g_s\beta V}{3\mathcal{N}h^3}\mathcal{N}\int d\mathbf{p} \, \frac{p\frac{d\mathcal{H}}{dp}}{Z^{-1}e^{\beta\mathcal{H}} + (-1)^{g_s}} \tag{4.5.1}
\end{align}
A questo punto, confrontando $(4.5.1)$ e $(4.3.16)$, notiamo che
\begin{align}
    PV = \frac{1}{3}\mathcal{N}\braket{p\frac{d\mathcal{H}}{dp}} \tag{4.5.2}
\end{align}
dove il braket indica la statistica di singola particella. Concludiamo che vale un'analogo quantistico del teorema di equipartizione dell'energia. \\ \\
Lo possiamo verificare costruendo "a mano" un esempio. Consideriamo un volume $V$ che contiene $N$ particelle e supponiamo di avere una distribuzione di probabilità (di singola particella) $f$ legata alla velocità delle particelle. Allora
\begin{align}
    \frac{dN_{\mathbf{u}}}{dV} = \frac{N}{V}f(\mathbf{u})d\mathbf{u} \tag{4.5.3}
\end{align}
ovvero prendiamo $f$ in modo tale che la densità di particelle con velocità compresa tra $\mathbf{u}$ e $\mathbf{u} + d\mathbf{u}$ sia proporzionale alla densità di particelle $N/V$ (del tutto ragionevole). Consideriamo ora una porzione di superficie $dA$ sul bordo di $V$, sia $\hat{\mathbf{z}}$ il versore ortogonale (uscente) a $dA$. Allora, data una velocità $\mathbf{u}$, tutte le particelle con velocità $\mathbf{u}$ che si trovano nel \textit{cilindro di collisione} $l \times dA$, dove $l = \|\mathbf{u}\|dt$, incideranno su $dA$ al più in un tempo $dt$. \\
Segue che il numero di particelle che incide su $dA$ in un tempo $dt$ è dato da 
\begin{align}
    dN_{\mathbf{u}} &= \overbrace{n}^{\mathcal{N}/V}(\mathbf{u} \cdot \hat{\mathbf{z}}dA)dtf(\mathbf{u})d\mathbf{u} \notag \\
    &= nu_zdAdtf(\mathbf{u})d\mathbf{u} \tag{4.5.4}
\end{align}
La variazione dell'impulso totale a seguito dell'impatto è
\begin{align}
    dp_{\mathbf{u}} = 2np_zu_zdAdtf(\mathbf{u})d\mathbf{u} \tag{4.5.5}
\end{align}
Quindi, la pressione esercitata in $dA$ dovuta all'urto delle particelle con velocità $\mathbf{u}$ è data da
\begin{align}
    P_\mathbf{u} = \frac{dp_\mathbf{u}}{dAdt} = 2np_zu_zf(\mathbf{u})d\mathbf{u} \tag{4.5.6}
\end{align}
A questo punto osserviamo che, in un sistema all'equilibrio, ha senso supporre che $f$ dipenda solamente dal modulo della velocità $u$, altrimenti esisterebbero direzioni preferite, il che comporta moto ordinato delle particelle. \\
Integrando su tutte le velocità otteniamo la pressione totale
\begin{align}
    P = 2n\int_0^\infty du_x\int_0^\infty du_y\int_0^\infty du_z p_zu_zf(u) \tag{4.5.7}
\end{align}
Passando in coordinate sferiche
\begin{align}
    P &= 2n\int_0^\pi d\theta \, sin\theta \cos^2\theta\int_0^{2\pi}d\varphi \int_0^\infty du \, pu^3f(u) \notag \\
    &= \frac{n}{3}\int_0^\infty \underbrace{4\pi u^2du}_{d\mathbf{u}} \, puf(u) \notag \\
    &= \frac{n}{3}\braket{pu} \tag{4.5.8}
\end{align}

\newpage
\subsection{Limite classico delle equazioni caratteristiche}
...
\begin{align}
    \mathcal{H}(\mathbf{p}) = c\|\mathbf{p}\| \tag{4.6.1}
\end{align}
Nel contesto dell'ensemble grancanonico quantistico si ha
\begin{align}
    U = \frac{4\pi g_sV}{h^3}\int_0^\infty dp p^2 \, \frac{pc}{Z^{-1}e^{\beta pc} + 1} \tag{4.6.2}
\end{align}
Poniamo $x = \beta pc$, allora $p = \frac{x}{\beta c}$ e $dp = \frac{1}{\beta c}dx$. Sostituendo in $(4.6.3)$ si ottiene 
\begin{align}
    U = \frac{4\pi g_sV}{\beta^4c^3h^3}\int_0^\infty dx \, \frac{x^3}{Z^{-1}e^x + 1} \tag{4.6.3} 
\end{align}
Facendo lo stesso per l'espressione del numero medio di particelle $\mathcal{N}$ si ottiene
\begin{align}
    \mathcal{N} = \frac{4\pi g_sV}{\beta^3c^3h^3}\int_0^\infty dx \, \frac{x^2}{Z^{-1}e^x + 1} \tag{4.6.4}
\end{align}
Sviluppiamo in serie $(4.6.4)$ per $Z << 1$
\begin{align}
    ...
    &= \frac{4\pi g_s V}{\beta^3c^3h^3}(2Z - \frac{1}{4}Z^2) \tag{4.6.5}
\end{align}
dove si è usato 
\begin{align}
...
\begin{align}
    \frac{1}{4}Z^2 - 2Z + n' = 0 \tag{4.6.6}
\end{align}
In particolare, $(4.6.6)$ ammette le seguenti soluzioni
\begin{align}
    Z_{\pm} = 4 \pm 2\sqrt{4 - n'} \sim 4 \pm \left(4 - \frac{1}{2}n' - \frac{1}{8}(n')^2\right) \tag{4.6.7}
\end{align}
...
\begin{align}
    Z = \frac{1}{2}n' + \frac{1}{8}(n')^2 \tag{4.6.8}
\end{align}
...
\begin{align}
    ...
    &= \frac{4\pi g_sV}{\beta^4(ch)^3}\left(6Z - \frac{3}{8}Z^2\right) \tag{4.6.9}
\end{align}
e sostituendo la soluzione ottenuta in $(4.6.8)$, tenendo conto solamente dei contributi fino all'ordine $2$, si ha
\begin{align}
    ...
    &= 3\mathcal{N}kT\left(1 + \frac{5}{16}n'\right) \tag{4.6.10}
\end{align}
Pertanto, dato che $PV = \frac{1}{3}U$, abbiamo ottenuto
\begin{align}
    PV = \mathcal{N}kT\left(1 + \frac{5}{16}n'\right) \tag{4.6.11}
\end{align}
...
Consideriamo un corpo nero di volume $V$ e a temperatura $T$ contenente radiazione elettromagnetica. \\
Dalle equazioni di Maxwell nel vuoto notiamo che il vettore campo elettrico $\mathbf{E}$ soddisfa l'equazione dell'oscillatore armonico, pertanto possiamo trattare tale sistema come un gas di oscillatori armonici quantistici. Questo segue dal fatto che possiamo esprimere il campo elettrico come sovrapposizione di onde piane a diverse frequenze, ovvero
\begin{align}
    E(\mathbf{r}, t) = \hat{\boldsymbol{\epsilon}}\int d\omega \, A(\omega)e^{i(\mathbf{k} \cdot \mathbf{r} - \omega t - \delta_\omega)} \tag{4.7.1}
\end{align}
La condizione $\nabla \cdot \mathbf{E} = 0$ comporta $\hat{\boldsymbol{\epsilon}} \cdot \mathbf{k} = 0$, pertanto, assegnata una direzione di propagazione $\mathbf{k}$, esistono solamente due direzioni di polarizzazione indipendenti ammesse per il campo elettrico. \\ \\
Imponendo condizioni al contorno periodiche per il campo elettrico si ottengono i vettori d'onda $\mathbf{k}$ ammessi. In particolare
\begin{align}
    k_i = \frac{2\pi n_i}{L} \qquad i = x, y, z \quad n_i \in \mathbf{N} \tag{4.7.2}
\end{align}
Come nel caso di particelle libere, al limite termodinamico è possibile passare da sommatoria a integrale
\begin{align}
    \sum_{k_x, k_y, k_z} 1 \sim \left(\frac{L}{2\pi}\right)^3\int_{-\infty}^\infty dk_x \int_{-\infty}^\infty dk_y \int_{-\infty}^\infty dk_z \notag
\end{align}
L'energia di un oscillatore armonico quantistico unidimensionale è
\begin{align}
    E_n = \hbar\omega\left(n + \frac{1}{2}\right) \qquad n \in \mathbf{N} \notag
\end{align}
Viene naturale pensare che i fotoni sono i quanti di energia dell'oscillatore armonico, pertanto l'energia $E_n$ di oscillatore con frequenza $\omega$ dipende da quanti fotoni si trovano alla frequenza $\omega$. \\ \\
Sappiamo che i fotoni hanno massa nulla, pertanto viaggiano alla velocità della luce, hanno spin $\hbar$ e impulso $p = \hbar k$, dove $k = \omega/c$ (dalle equazione di Maxwell). Segue che l'energia di ogni fotone libero è $E = pc$. \\ \\
Nel contesto dell'ensemble canonico quantistico, sappiamo da $(2.7.3)$ che 
\begin{align}
    Q_1 = \frac{1}{1 - e^{-\beta\hbar\omega}} \tag{4.7.3}
\end{align}
dove è stato abbandonato il termine $\frac{1}{2}\hbar\omega$ dato che non porta grandi conseguenze nel limite termodinamico. \\
A questo punto, il valore atteso di energia per un oscillatore armonico di frequenza $\omega$ è dato da
\begin{align}
    \braket{E}_\omega = \frac{\hbar\omega}{e^{\beta\hbar\omega} - 1} \tag{4.7.4}
\end{align}
ovvero l'energia dell'oscillatore è il prodotto tra il quanto di energia $\hbar\omega$ e il numero medio di occupazione dello stato identificato da $\omega$
\begin{align}
    \braket{n}_\omega = \frac{1}{Q_1}\sum_{n = 0}^\infty ne^{-\beta n\hbar\omega} \tag{4.7.5}
\end{align}
In particolare, ponendo $x = e^{-\beta\hbar\omega}$ si ottiene
\begin{align}
    \braket{n}_\omega = \frac{1}{Q_1}\sum_{n = 0}^\infty nx^n = \frac{1}{Q_1}x\frac{d}{dx}\left(\sum_{n = 0}^\infty x^n\right) = x\frac{d}{dx}\left(\log Q_1\right) \tag{4.7.6}
\end{align}
Inoltre, dobbiamo moltiplicare per un fattore $2$ dovuto alla doppia possibilità di polarizzazione dell'onda. \\ \\
Possiamo ora determinare l'energia interna del sistema. Passando da sommatoria a integrale si ottiene
\begin{align}
    U &= \frac{2L^3}{8\pi^3}\int d\mathbf{k} \, \frac{\hbar\omega}{e^{\beta\hbar\omega} - 1} \notag \\
    &= \frac{2L^3}{8\pi^3}\int d\mathbf{k} \, \frac{pc}{e^{\beta pc} - 1} \notag \\
    &= \frac{L^3}{\pi^2\hbar^3}\int_0^\infty dp \, \frac{p^3c}{e^{\beta pc} - 1} \tag{4.7.7}
\end{align}
Poniamo $x = \beta pc$, allora $p = \frac{1}{\beta c}x$ e $dp = \frac{1}{\beta c}dx$. Sostituendo in $(4.7.7)$
\begin{align}
    U = \frac{L^3}{\pi^2\hbar^3}\frac{1}{\beta^4c^3}\int_0^\infty dx \, \frac{x^3}{e^x - 1} = \frac{L^3}{\pi^2\hbar^3c^3\beta^4}g_4(1) \tag{4.7.8}
\end{align}
In particolare $g_4(1) = \zeta(4) = \frac{\pi^4}{15}$, quindi
\begin{align}
    U = \frac{\pi^2V}{15\hbar^3c^3}(kT)^4 \tag{4.7.9}
\end{align}
Notiamo subito che $u(T) = aT^4$, dove
\begin{align}
    a = \frac{\pi^2k^4}{15\hbar^3c^3} \tag{4.7.10}
\end{align}
Inoltre, segue che il calore specifico (per unità di volume) $c_V$ è
\begin{align}
    c_V = \frac{\partial u}{\partial T} = \frac{4\pi^2k^4T^3}{15\hbar^3c^3} \tag{4.7.11}
\end{align}
Mentre l'espressione $u(T)$ che abbiamo ricavato può essere ottenuta solamente tramite considerazioni sulla natura quantistica del sistema, le relazioni
\begin{align}
    P = \frac{1}{3}U \qquad U \propto T^4 \notag
\end{align}
possono essere ricavate anche da considerazioni classiche. \\ \\
Infine, confrontando con le equazioni caratteristiche dell'ensemble grancanonico quantistico, osserviamo che sono identiche a meno di porre $Z = 1$, ovvero $\mu = 0$. Questo perché i fotoni sono liberi di essere assorbiti ed emessi dalle pareti del corpo nero.
