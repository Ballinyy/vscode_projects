...
\end{align}
Notiamo che $Q_N = Q_N(T, V)$, ovvero dipende dalla terna $(N, T, V)$. Il potenziale termodinamico le cui variabili naturali sono $(N, T, V)$ è l'\textit{energia libera di Helmotz} $F$, infatti 
\begin{align}
    F = E - TS \notag
\end{align}
pertanto 
...
\begin{align}
    F \sim -NkT\log\frac{V}{N} - NkT\left(1 - \frac{3}{2}\log\frac{2\pi mkT}{h^2}\right) \tag{2.1.24}
\end{align}
...
Segue che 
\begin{align}
    F = -3NkT\log \frac{kT}{\hbar\omega} \tag{2.1.28}
\end{align}
inoltre 
\begin{align}
...
\begin{align}
    F &= N\frac{\hbar\omega}{2} + NkT\log\left(1 - e^{-\beta\hbar\omega}\right) \tag{2.1.34}
\end{align}
...
\end{align}
Studiando lo stesso sistema come ensemble canonico possiamo determinare l'energia libera $F$ (e quindi l'entropia canonica) passando per la funzione di partizione canonica
\begin{align}
...
\begin{align}
    ...
    &= e^{-\beta(F_{N_2}(V_2, T) - F_N(V, T))}\frac{e^{-\beta\mathcal{H}_{N_1}}}{N_1!h^{3N_1}} \tag{3.1.4}
\end{align}
...
\begin{align}
    F_{N_2}(V_2, T) - F_N(V, T) &\sim -\underbrace{\left(\frac{\partial F}{\partial N}\right)_{V, T}}_{\mu}N_1 - \underbrace{\left(\frac{\partial F}{\partial V}\right)_{N, T}}_{-P}V_1 \notag \\
...
Allora si ottiene 
\begin{align}
    \braket{E} &= -\left(\frac{\partial}{\partial \beta}ZQ_1(T, V)\right)_{Z, V} \notag \\
    &= -Z\frac{\partial T}{\partial \beta} \left(\frac{\partial}{\partial T}Q_1(T, V)\right)_{Z, V} \notag \\
