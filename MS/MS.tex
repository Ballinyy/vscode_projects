...
\end{align}
A questo punto possiamo abbandonare il pedice che caratterizza il sottosistema $1$ e lo trattiamo come il sistema generale. Pertanto, per un insieme di particelle indipendenti e indistinguibili nel contesto dell'ensemble grancanonico, si ha
\begin{align}
...
\newpage
\subsection{Fluttuazioni del numero di particelle (spettro discreto)}
...
\begin{align}
    P(E_N) = \frac{Z^Nd(E_N)e^{-\beta E_N}}{\sum_{N, E_N}Z^Nd(E_N)e^{-\beta E_N}} \tag{3.2.2}
\end{align}
ben normalizzata. A questo punto, per costruzione, si ha 
\begin{align}
    \braket{E} = \sum_{N, E_N} E_NP(E_N) = -\left(\frac{\partial}{\partial\beta}\log \tilde{Q}\right) _{V, Z}\notag
\end{align}
...
\begin{align}
    ...
    &= \frac{\sum_{N}N^2Z^NQ_N}{\sum_{N}Z^NQ_N} - \left(\frac{\sum_{N}NZ^NQ_N}{\sum_{N}Z^NQ_N}\right)^2 \tag{3.2.8}
\end{align}
...
\begin{align}
    \braket{E} = -\left(\frac{\partial}{\partial \beta}\log\tilde{Q}\right)_{V, Z} \tag{3.3.2}
\end{align}
Allora si ottiene 
\begin{align}
    \braket{E} &= -\left(\frac{\partial}{\partial \beta}ZQ_1(T, V)\right)_{V, Z} \notag \\
...
\end{align}
Mentre ricordiamo che per l'ensemble canonico si ha $\braket{E}_{can} = \frac{3}{2}NkT$ con $N$ costante del sistema.
