...
Per cui
\begin{align}
    Z \sim e^{\beta\varepsilon_F} \qquad \varepsilon_F = \frac{\hbar^2}{2m}\left(\frac{6\pi^2}{v}\right)^{2/3} \tag{2.2.29}
\end{align}
...
Consideriamo un gas di onde elettromagnetiche confinate in un corpo nero. Decomponendo le onde elettromegnetiche nella base delle onde piane possiamo descrivere le onde elettromagnetiche come sovrapposizione (somma) di onde piane. \\
Confrontando con le equazioni di Maxwell nel vuoto osserviamo che le onde piane soddisfano l'equazione dell'oscillatore armonico. In particolare, dalle equazioni di Maxwell sappiamo che
\begin{align}
    \nabla^2\mathbf{E} - \frac{1}{c^2}\frac{\partial^2 \mathbf{E}}{\partial t^2} = 0 \tag{4.6.1}
\end{align}
Dato che $\mathbf{E}$ è sovrapposizione di onde piane, ovvero
\begin{align}
    \mathbf{E} = \hat{\boldsymbol{\epsilon}}\int d\omega \, A(\omega)e^{i(\mathbf{k} \cdot \mathbf{r} - \omega t)} \tag{4.6.2}
\end{align}
Data che la regolarità della funzione integranda garantisce la linearità rispetto alle operazioni di derivazione, possiamo considerare ogni termine singolarmente. Dalle equazioni di Maxwell otteniamo
\begin{align}
    \nabla \cdot \mathbf{E} = 0 \implies \hat{\boldsymbol{\epsilon}} \cdot \mathbf{k} = 0 \notag 
\end{align}
Da $(4.6.1)$ si ottiene
\begin{align}
    c\|\mathbf{k}\| = \omega \notag
...
\end{align}
ovvero $\mathbf{E}$ soddisfa l'equazione differenziale dell'oscillatore armonico, pertanto possiamo trattare il gas di onde elettromagnetiche nel corpo nero come un insieme di oscillatori armonici quantistici. \\ \\
Analogamente a quanto visto per le particelle libere, si ha
\begin{align}
    \sum_{k_x, k_y, k_z} 1 \sim \left(\frac{L}{2\pi}\right)^3\int_{-\infty}^\infty dk_x\int_{-\infty}^\infty dk_y\int_{-\infty}^\infty dk_z \tag{4.6.3}
\end{align}
al limite termodinamico. \\
Classicamente penseremmo di associare ad ogni onda nel corpo nero un energia $kT$, tuttavia questo modello porta alla \textit{catastrofe dell'ultravioletto}, ovvero prevede un'energia interna infinita, infatti
\begin{align}
    U = 2\left(\frac{L}{2\pi}\right)^2\int_{-\infty}^\infty dk_x\int_{-\infty}^\infty dk_y\int_{-\infty}^\infty dk_z (kT) = +\infty \notag
\end{align}
dove il fattore $2$ a moltiplicare tiene conto della degenerazione dovuta alle possibili polarizzazioni dell'onda. \\ \\
La soluzione è trattare, come prima giustificato, il gas di onde elettromagnetiche come un gas di oscillatori armonici, ognuno dei quali ha energia $E = \frac{1}{2}\hbar\omega$. In particolare, da $(2.7.3)$ sappiamo che
\begin{align}
    Q_1 = \frac{1}{e^{\beta\frac{\hbar\omega}{2}} - 1} \notag
\end{align}
pertanto, trascurando il fattore $1/2$ nel limite di molti fotoni, si ottiene
\begin{align}
    U = \frac{2L^3}{8\pi^3}\int d\mathbf{k} \, \frac{\hbar\omega}{e^{\hbar\omega} - 1} < \infty \tag{4.6.4}
\end{align}
