...
\subsection{Densità di probabilità per ensemble microcanonico}
...
\end{gather}
Il sistema è isolato, quindi non ci sono sorgenti o pozzi. Posto $\mathbf{v} = \mathbf{v}$, segue che $P$ soddisfa la relazione di continuità
\begin{gather}
...
\begin{gather}
    S = k_B\log \Gamma(E) \tag{1.2.1}
\end{gather}
Vogliamo dimostrare che questa definizione di entropia è estensiva e quindi può essere usata come potenziale termodinamico. Inoltre, vogliamo dimostrare che è legata alla definizione macroscopica di entropia data da
\begin{gather}
    TdS = dE + PdV - \mu dN \tag{1.2.2}
\end{gather}
Consideriamo un sistema isolato confinato in un volume $V = V_1 + V_2$ e con un numero $N = N_1 + N_2$ di particelle libere e indipendenti. L'hamiltoniana che descrive tale sistema è
\begin{gather}
...
\end{align*}
ponendo $r = \|\mathbf{x}\|$. Inoltre vale la seguente identità
\begin{align}
...
\begin{gather}
    S(N, V, E) = k\left(\log c_{3N} + N\log \frac{V}{h^3} + \frac{3}{2}\log (2mE)\right) \tag{1.2.22}
\end{gather}
per poter trattare l'espressione $(1.2.22)$ dell'entropia $S$ come potenziale termodinamico dobbiamo far uso della \textit{formula di Stirling}, ovvero
\begin{gather}
...
e
\begin{gather}
    S(N, V, E) \sim kN\log\left(V\left(\frac{4\pi m}{3h^2}\frac{E}{N}\right)^{3/2}\right) + \frac{3}{2}Nk \qquad N \rightarrow \infty \tag{1.2.23} 
\end{gather}
Esprimendo $E$ in funzione di $(N, S, V)$ e usando la relazione macroscopica $TdS = dE + PdV - \mu dN$ possiamo ricavare le relazione che legano i potenziali termodinamici con le grandezze termodinamiche globali, infatti
\begin{gather}
    E(N, S, V) = \left(\frac{3h^2}{4\pi m}\right)\frac{N}{V^{2/3}}\exp\left(\frac{2}{3}\frac{S}{Nk} - 1\right) \tag{1.2.24}
\end{gather}
e in particolare
\begin{align}
...
\begin{align}
    S(N, V, E) = kN\log(Vu^{2/3}) + \frac{3}{2}kNS_0 \tag{1.2.25}
\end{align}
dove
\begin{align}
...
\begin{align}
    k\log\Sigma \longrightarrow k\log\Sigma - k\log N! \tag{1.2.26}
\end{align}
...
\begin{align}
    S(N, V, E) &= kN\log\left(\frac{V}{N}\left(\frac{E}{N}\right)^{3/2}\right) + \frac{5}{2}kNS_0 \notag \\
    &= kN\log(uv^{3/2}) - \frac{5}{2}kNS_0 \tag{1.2.27}
\end{align}
...
\begin{align}
    ...
    &= 3kN\left(1 + \log\left(\frac{1}{3\hbar\omega}\frac{E}{N}\right)\right) \tag{1.2.29}
\end{align}
...
\begin{align}
    S &\sim k\log \Sigma \notag \\
    &\sim kN\left(4 + \log\left(\frac{8\pi VE^3}{27h^3c^3N^4}\right)\right) \tag{1.2.43}
\end{align}
...
\begin{align}
    ...
    &= \delta_{ij}\left(\frac{\partial \log\Sigma}{\partial E}\right)^{-1} \notag \\
...
\begin{align}
    ...
    &= k\left(N\log N - (N - m)\log(N - m) - m\log m\right) \notag
\end{align}
Notiamo che 
\begin{align}
...
\begin{align}
    S(N, E) &\sim kN\left(\left(u + \frac{3}{2}\right)\log N\left(u + \frac{3}{2}\right) - \left(u - \frac{3}{2}\right)\log N\left(u - \frac{3}{2}\right) - 3\log 3N\right) \notag \\
    &= kN\left(u\log \frac{u + 3/2}{u - 3/2} + \frac{3}{2}\log\left(u^2 - \frac{9}{4}\right) -3\log3\right) \notag
\end{align}
Notiamo che 
\begin{align}
    \frac{1}{T} &= \left(\frac{\partial S}{\partial E}\right)_N \notag \\
...
\begin{align}
    G(N, V, T) = -kT\log Q_N(V, T) \tag{2.1.8}
\end{align}
...
\begin{align}
    G \sim -NkT\log\frac{V}{N} - NkT\left(1 - \frac{3}{2}\log\frac{2\pi mkT}{h^2}\right) \tag{2.1.24}
\end{align}
...
\begin{align}
    ...
    &= -\frac{\partial}{\partial\beta}\left(-\frac{3}{2}N\log\frac{1}{\beta}\right) = \frac{3}{2}NkT \tag{2.1.25}
\end{align}
...
Segue che 
\begin{align}
    G = -3NkT\log \frac{kT}{\hbar\omega} \tag{2.1.28}
\end{align}
inoltre 
\begin{align}
...
\begin{align}
    G &= N\frac{\hbar\omega}{2} + NkT\log\left(1 - e^{-\beta\hbar\omega}\right) \tag{2.1.34}
\end{align}
...
\begin{align}
    ...
    &-\left.\frac{1}{2}\left(N - \frac{E}{\varepsilon}\right)\log\frac{1}{2}\left(N - \frac{E}{\varepsilon}\right) - N\log\frac{1}{2}\right) \tag{2.1.41}
\end{align}
dove 
\begin{align}
    ...
    &= \frac{k}{2}\log\frac{4\varepsilon^2}{kT^2c_V} \tag{2.1.42}
\end{align}
...
\begin{align}
    G &= -NkT\log(e^{\beta E} + e^{-\beta E}) \notag \\
    &= -NkT\log(2\cosh \beta E) \tag{2.1.44}
\end{align}
segue che 
\begin{align}
...
\begin{align}
    ...
    &= Nk\log\left(2\cosh \beta E\right) - N\beta kE\tanh \beta E \tag{2.1.46}
\end{align}
...
\end{align}
Concludiamo che l'entropia canonica è maggiore dell'entropia microcanonica (a causa delle fluttazioni) e differisce da quest'ultima per un termine che va come $\log N$. In particolare, usando la definizione statistica dell'energia libera $(2.1.8)$, si ottiene 
\begin{align}
    U - TS_{can} \sim \overline{E} - TS_{mic} - \frac{kT}{2}\log\left(2\pi kT^2\frac{\partial \overline{E}}{\partial T}\right) \tag{2.2.7}
\end{align}
e, dato che $U \sim \overline{E}$, si ottiene 
\begin{align}
    S_{can} \sim S_{mic} + \frac{kT}{2}\log\left(2\pi kT^2c_V\right) \tag{2.2.8}
\end{align}
...
\begin{align}
    ...
    &= Z\left(\frac{\partial}{\partial Z}\log \tilde{Q}\right)_{V, T} \tag{3.1.14}
\end{align}
...
\begin{align}
    \braket{E} = -\left(\frac{\partial}{\partial \beta}\log\tilde{Q}\right)_{Z, V} \tag{3.1.19}
\end{align}
Allora si ottiene 
\begin{align}
...
\begin{align}
    \braket{E} = \sum_{N, E_N} E_NP(E_N) = -\left(\frac{\partial}{\partial\beta}\log \tilde{Q}\right) _{Z, V}\notag
\end{align}
per $(3.1.12)$ si ha che 
\begin{align}
    -\left(\frac{\partial}{\partial\beta}\log \tilde{Q}\right)_{Z, V} = -ZV\frac{\partial}{\partial\beta}f(T) = ZVf'(T)kT^2 \tag{3.1.23}
\end{align}
