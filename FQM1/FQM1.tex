...
\begin{align}
    ...
    \end{cases} \notag
\end{align}
Dove $|M_{fi}|^2 = |\overline{M}_{if}|^2$, ovvero la rate di emmissione e assorbimento sono uguali. \\ \\
...
\end{align}
se e solo se $m - m' + q = 0$ (per $t \rightarrow \infty$), allora $\Delta m = 0$ o $\Delta m = \pm 1$. \\
...
\end{align}
Notiamo che il segno del termine $f_{ij}$ dipende solamente da $\omega_{ij}$. In particolare se $f_{ij} > 0$ è relativo ad un processo di emissione, mentre se $f_{ij} < 0$ è relativo ad un processo di assorbimento. \\
...
\begin{align}
    (\tau_b)^{-1} = \sum_{k : E_k < E_b}W^{spont}_{BK} \tag{1.4.22}
\end{align}
Il numero di particelle che popola tale stato eccitato ha la seguente forma 
\begin{align}
    N(t) = N(0)e^{-t/\tau_b} \tag{1.4.23}
\end{align}
ovvero il numero decrescente esponenzialmente con rapidità dettata dal tempo di vita.
