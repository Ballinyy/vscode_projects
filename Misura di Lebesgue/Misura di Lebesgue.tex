...
\title{Misura di Lebesgue in $\mathbf{R}^n$}
...
\textbf{Definizione 1.1}: un insieme $Q$ è detto \textbf{intervallo} di $\mathbf{R}^n$ se è della forma
\begin{equation}
    Q = (a_1,b_1) \times (a_2,b_2) \times ... \times (a_n, b_n) \notag
\end{equation}
ovvero prodotto cartesiano di intervalli di $\mathbf{R}$. Può essere scritto anche $Q = \{ \vec{x} \in \mathbf{R}^n: a_1 \leq x_1 \leq b_1,..., a_n \leq x_n \leq b_n \}$ con $\vec{x} = (x_1,x_2,x_3,...,x_n)$.\\ \\
\textbf{Definizione 1.2}: la misura di un intervallo $Q \in \mathbf{R}^n$ è definita da $m(Q) := \prod_{i \leq n}(b_i - a_i)$.\\ \\
A questo punto possiamo dare una definizione di misura per insiemi più generici detti \textbf{plurintervalli}, le proprietà della misura definita sui plurintervalli saranno poi "ereditate" dagli insiemi detti misurabili di $\mathbf{R}^n$.\\
...
\end{equation}
L'unione di questi $k_1 + k_2 + \dots + k_n$ iperpiani è detto \textbf{reticolato} su $\mathbf{R}^n$. Un reticolato $\mathcal{P}$ suddivide $\mathbf{R}^n$ in un numero finito di intervalli chiusi e limitati, detti \textbf{intervalli del reticolato}, più un numero finito di insiemi illimitati. \\ \\
\textbf{Definizione 1.3:} un insieme $X$ è detto \textbf{plurintervallo} di $\mathbf{R}^n$ se esiste un reticolato $\mathcal{P}$ tale che $X$ è unione finita di \textit{intervalli} di $\mathcal{P}$ (anche non tutti). Allora se $I_1, I_2, \dots, I_m$ sono intervalli di $\mathcal{P}$ si ha che:
\begin{equation}
...
Diamo ora una definizione di misura per insiemi \textit{aperti} e insiemi \textit{chiusi} \textbf{limitati }di $\mathbf{R}^n$ (topologia euclidea sottintesa). \\ \\
...
\textbf{Definizione 2.3:} sia $E \subset \mathbf{R}^n$
\begin{equation}
...
\textbf{Definizione 2.4:} sia $E \subset \mathbf{R}^n$
\begin{equation}
...
\textbf{Proposizione 2.1:} per ogni $E \subset \mathbf{R}^n$ si ha che $\overline{m}(E) \geq \underline{m}(E)$. \\ \\
...
\textbf{Definizione 2.5:} sia $E \subset \mathbf{R}^n$, $E$ è detto \textbf{misurabile} (secondo Lebesgue) se $\overline{m}(E) = \underline{m}(E)$. \\ \\
...
\textbf{Proposizione 2.2:} un insieme $E \subset \mathbf{R}^n$ è misurabile se e solo se per ogni $\varepsilon > o$ esistono un aperto $A$ e un compatto $K$ tali che $K \subset E \subset A$ e $m(A) - m(K) < \varepsilon$. \\ \\
...
\textbf{Proposizione 2.3:} siano $E, F \subset \mathbf{R}^n$, allora $\overline{m}(E \cup F) \leq \overline{m}(E) + \overline{m}(F)$, inoltre se $E \cap F = \emptyset$ si ha $\underline{m}(E \cup F) \geq \underline{m}(E) + \underline{m}(F)$. \\ \\
...
Siano ora $E, F$ disgiunti, $K, L \subset \mathbf{R}^n$ chiusi tali che $K \subset E$ e $L \subset F$, allora $K \cup L \subset E \cup F$, pertanto $\underline{m}(E \cup F) \geq m(K \cup L)$. Inoltre anche $K$ e $L$ sono disgiunti, quindi per il \textbf{lemma 2.3} si ha $m(K \cup L) \geq m(K) + m(L)$, ciò comporta $\underline{m}(E \cup F) \geq m(K) + m(L)$, possiamo concludere $\underline{m}(E \cup F) \geq \underline{m}(E) + \underline{m}(F)$ dalla definizione di misura interna. \\ \\
...
\textbf{2.} Sia $S \subset \mathbf{R}^2$ il segmento $S := \{ (x, y) \in \mathbf{R}^2 : y = 0, \ 0 \leq x \leq 1 \}$, dimostrare che $S$ è misurabile e che $m_2(S) = 0$. \\ \\
...
\textbf{Proposizione 3.1:} sia $\{ E_i \}_{i \in I}$ una famiglia numerabile di insiemi di $\mathbf{R}^n$ e sia $E := \bigcup_{i \in I} E_i$, allora $\overline{m}(E) \leq \sum_{i \in I} \overline{m}(E_i)$; inoltre se $E_j \cap E_k = \emptyset \ \forall j \neq k$ si ha $\underline{m}(E) \geq \sum_{i \in I} \underline{m}(E_i)$. \\ \\
...
\textbf{Proposizione 3.2;} sia $\{ E_i \}_{i \in I}$ una famiglia numerabile di insiemi di $\mathbf{R}^n$ tali che $E_1 \subset E_2 \subset E_3 \subset \dots$, sia $E := \bigcup_{i \in I} E_i$. Allora $\overline{m}(E) = \lim_{i \rightarrow \infty} \overline{m}(E_i)$. \\ \\
...
\textbf{Definizione 4.1:} un insieme $E \subset \mathbf{R}^n$ si dice misurabile se per ogni $r > 0$ l'insieme $E \cap B_r(0)$ è misurabile, cioè se per ogni $r > 0$ si ha $\underline{m}(E \cap B_r(0)) = \overline{m}(E \cap B_r(0))$. \\
Notiamo che questa definizione è ancora valida per gli insiemi di misura finita. \\ \\ \\
\textbf{Proposizione 4.1:} sia $E \subset \mathbf{R}^n$ un insieme misurabile, allora $\underline{m}(E) = \overline{m}(E) = \\ = \lim_{r \rightarrow \infty} m(E \cap B_r(0))$. \\ \\
...
\textbf{Teorema 5.1} siano $E \subset \mathbf{R}^n$ e $F \subset \mathbf{R}^m$ insiemi misurabili, allora $E \times F \subset \mathbf{R}^{n + m}$ è misurabile e $m(E \times F) = m(E) \cdot m(F)$.\\ \\
...
\textbf{Proposizione 5.1:} siano $E \subset \mathbf{R}^n$ e $F \subset \mathbf{R}^m$, si ha che $\overline{m}(E)\underline{m}(F) \leq \overline{m}(E \times F)$. \\ \\
...
\textbf{Definizione 6.1:} una \textbf{funzione semplice} è una combinazione lineare di \textit{funzioni caratteristiche} di insiemi limitati e disgiunti di $\mathbf{R}^n$:
\begin{gather}
...
\textbf{Definizione 6.2:} sia $f : \mathbf{R}^n \longrightarrow \mathbf{R}$ una funzione limitata e nulla al di fuori di un compatto. Definiamo
\begin{gather}
...
\end{gather}
\\ \\ \\
\textbf{Proposizione 6.1:} sia $f : \mathbf{R}^n \longrightarrow \mathbf{R}$ limitata e nulla al di fuori di un compatto. Condizione necessaria e sufficiente affinché $f$ sia sommabile è che esistono due successioni di funzioni semplici:
\begin{gather}
...
\end{gather}
\textbf{Proposizione 6.2:} Condizione sufficiente e necessaria affinché una funzione \\ $f : \mathbf{R}^n \longrightarrow \mathbf{R}$ sia sommabile è che per ogni $\varepsilon > 0$ esistono due funzioni semplici $\alpha \geq f$ e $\beta \leq f$ tali che:
\begin{gather}
...
\textbf{Definizione 7.1:} una funzione $f : \mathbf{R}^n \longrightarrow \mathbf{R}$ è detta \textbf{misurabile} se l'insieme:
\begin{gather}
...
\textbf{2.} $F_t^{'} := \{ \mathbf{x} \in \mathbf{R}^n : f(\mathbf{x}) \leq t \}$ è misurabile. \\
\textbf{3.} $F_t^{''} := \{ \mathbf{x} \in \mathbf{R}^n : f(\mathbf{x}) < t \}$ è misurabile. \\
\textbf{4.} $F_t^{'''} := \{ \mathbf{x} \in \mathbf{R}^n : f(\mathbf{x}) \geq t \}$ è misurabile. \\
\textbf{5.} $F_t^{(4)} := \{ \mathbf{x} \in \mathbf{R}^n : f(\mathbf{x}) = t \}$ è misurabile. \\ \\
\textbf{Dimostrazione:} $\mathbf{(1 \rightarrow 2)}$ sia $f$ misurabile, allora per ogni $t \in \mathbf{R}$ l'insieme $F_t$ è misurabile, pertanto anche $F_t^{'} = \mathbf{R}^n \setminus F_t$ è misurabile poiché differenza di insiemi misurabili. \\ \\
...
$\mathbf{(3 \rightarrow 4)}$ sia $F_t^{''}$ misurabile, allora anche $F_t^{'''} = \mathbf{R}^n \setminus F_t^{''}$ è misurabile poiché differenza di insiemi misurabili. \\ \\
...
\textbf{Lemma 7.1:} siano $f, g$ funzioni misurabili, allora $E := \{ \mathbf{x} \in \mathbf{R}^n : f(\mathbf{x}) > g(\mathbf{x}) \}$ è misurabile. \\ \\
...
\textbf{(2)} siano $f, g$ misurabili, allora per il \textbf{lemma 7.1} l'insieme \\ $E_t := \{ \mathbf{x} \in \mathbf{R}^n : f(\mathbf{x}) + g(\mathbf{x}) > t \}$ è misurabile (a meno di ridefinire una funzione $h := t - g$ che è misurabile per il punto \textbf{(1)}). \\ \\
\textbf{(3)} sia $\{ f_i \}_{i \in I} \subset \mathcal{M}$, si ha che $\{ \mathbf{x} \in \mathbf{R}^n : M(\mathbf{x}) > t \} = \bigcup_{i \in I} F_{i;t}$, pertanto misurabile poiché unione numerabile di insiemi misurabili. Analogamente si ha che $\{ \mathbf{x} \in \mathbf{R}^n : m(\mathbf{x}) < t \} = \bigcup_{i \in I} F_{i;t}^{''}$, pertanto anch'esso misurabile. \\ \\
\textbf{(4)} sia $\{ f_i \}_{i \in I} \subset \mathcal{M}$ tale che $f_1 \leq f_2 \leq \dots$, allora $\{ \mathbf{x} \in \mathbf{R}^n : f(\mathbf{x}) > t \} = \bigcup_{i \in I} F_t$ ed è dunque misurabile. \\ \\
...
\textbf{Definizione 7.2:} sia $f : \mathbf{R}^n \longrightarrow \mathbf{R}$, è detto \textbf{sottografico} di $f$ l'insieme
\begin{gather}
