...
\title{Introduzione alla topologia e alla geometria differenziale}
...
\textbf{W.O.P (assioma):} sia $S \subseteq \mathbb{N}$, con $\mathbb{N} := \{0, 1, 2, \dots\}$, allora esiste $m \in S$ tale che $m \leq x$ per ogni $x \in S$.
\subsection{Spazi topologici e applicazioni continue}
\textbf{Definizione:} sia $X$ un insieme. Una \textit{topologia} su $X$ è un sottoinsieme $\tau \subseteq P(X)$ tale che: \\
...
- sia $X = \mathbb{R}$, sia $A \in \tau$ se e solo se $A$ è della forma $A = \bigcup_{k \in \mathbb{N}} (a_k, b_k)$. Allora $\tau$ è una topologia, detta \textit{topologia euclidea}. Sia $B = \bigcup_{j \in \mathbb{N}} (c_j, d_j)$, si ha che $A \cap B = \bigcup_{k \in \mathbb{N}} (a_k, b_k) \cap \bigcup_{j \in \mathbb{N}} (c_j, d_j) = \bigcup_k \bigcup_j ((a_k, b_k) \cap (c_j, d_j))$, pertanto anche $A \cap B \in \tau$. \\ \\
\textbf{Definizione:} sia $(X, \tau)$ uno spazio topologico, sia $C \subseteq X$, $C$ è detto \textit{chiuso} se $X \setminus C \in \tau$. \\ \\
...
\textbf{Esempi:} - sia $X = \mathbb{R}$, allora $\bigcap_{n \in \mathbb{N}} \left(-\frac{1}{n}, \frac{1}{n}\right) = \emptyset$, ovvero in generale un'intersezione numerabile di aperti non è un aperto. \\
- sia $X = \mathbb{R}$, allora $\bigcup_{-1 < a < b < 1} [a, b] = (-1, 1)$, pertanto in generale l'unione (in questo caso non numerabile) di chiusi non è un chiuso. \\ \\
\textbf{Definizione:} sia $(X, \tau)$ uno spazio topologico, sia $Y \subseteq X$. Allora $\tau_Y := \{ A \cap Y : A \in \tau \}$ è una topologia su $Y$, detta \textit{topologia indotta} su $Y$ da $X$, in tal caso si dice che $Y$ è un \textit{sottospazio} di $X$. Notiamo che gli aperti di $(Y, \tau_Y)$ NON sono uguali agli aperti di $(X, \tau)$. \\ \\
\textbf{Definizione:} siano $(X, \tau)$ uno spazio topologico, $x_0 \in X$ e $A \subseteq X$. $A$ è detto \textit{intorno} di $x_0$ se esiste $W \in \tau$ tale che $W \subseteq A$ e $x_0 \in W$. Poniamo $\mathcal{N}(x_0) := \{ V \in \tau : V \text{ intorno di } x_0 \}$. \\ \\
...
\textbf{Definizione:} siano $(X, \tau)$ uno spazio topologico e $\mathcal{B} \subseteq \tau$ una famiglia di aperti. $\mathcal{B}$ è una \textit{base} per $\tau$ se ogni aperto $\tau$ di è unione di aperti di $\mathcal{B}$. \\ \\
...
\begin{gather}
    \mathcal{B}(x) := \{ D_{1/n}(x) : n \in \mathbb{N} \} \notag
\end{gather}
è un sistema fondamentale di intorni numerabile. \\
In particolare $\mathbb{R}^n$ soddisfa il secondo assioma di numerabilità perché la famiglia
\begin{gather}
    \mathcal{B} := \{ D_{1/n}(\mathbf{x}) := n \in \mathbb{N}, \mathbf{x} \in \mathbb{Q}^n \} \notag
\end{gather}
è una base numerabile della topologia euclidea. \\ \\
\textbf{Definizione:} siano $X, Y$ spazi topologici, $x_0 \in X$, $f : X \longrightarrow Y$, allora $f$ è detta \textit{continua} in $x_0$ se per ogni $V \in \mathcal{N}(f(x_0))$ esiste $W \in \mathcal{N}(x_0)$ tale che $f(W) \subset V$. \\
...
$f$ continua NON implica in generale che $f(A)$ è un aperto per ogni aperto $A \subseteq X$; infatti sia $X := \mathbb{R}$, topologia euclidea, e sia $f(x) = x^2$ per ogni $x \in \mathbb{R}$, allora $f$ è continua, consideriamo l'intervallo aperto $(-1, 1)$, si ha che $f((-1, 1)) = [0, 1)$ che non è aperto né chiuso. \\ \\
\textbf{Definizione:} sia $f : X \longrightarrow Y$ un'applicazione tra spazi topologici, $f$ è detta \textit{omeomorfismo} se: \\
...
\textbf{Definizione:} sia $\mathcal{P}$ una proprietà, $\mathcal{P}$ è detta \textit{proprietà topologica} se per $X \cong Y$ si ha che $X$ ha $\mathcal{P}$ se e solo se $Y$ ha $\mathcal{P}$, ovvero se la proprietà è invariante per omeomorfismi. \\ \\
\textbf{Definizione:} si dice che $f : X \longrightarrow X$ è un \textit{automorfismo} se $f$ è un omeomorfismo. \\ \\
\textbf{Definizione:} sia $X$ uno spazio topologico, sia $A \subseteq X$, è detta \textit{parte interna di $A$} l'insieme
\begin{gather}
...
\textbf{Definizione:} siano $X$ uno spazio topologico, $A \subseteq X$ e $x_0 \in A$. $x_0$ è detto \textit{punto di accumulazione} per $A$ se per ogni $V \in \mathcal{N}(x_0)$ si ha che
\begin{gather}
...
\textbf{Dimostrazione:} sia $x \in D(A)$, supponiamo per assurdo che $x \in X \setminus \overline{A}$, poiché $\overline{A}$ è chiuso si ha che $X \setminus \overline{A}$ è aperto, pertanto esiste $V \in \mathcal{N}(x)$ tale che $V \subset X \setminus \overline{A}$, ciò comporta che $\overline{A} \cap (V \setminus \{x\}) = \emptyset$ e poiché $\overline{A} \supseteq A$ si ottiene $A \cap (V \setminus \{x\}) = \emptyset$, ma è un assurdo. \qed \\ \\
\textbf{Definizione:} siano $X$ uno spazio topologico, $A \subseteq X$, la \textit{frontiera} (o \textit{bordo}) di $A$ è $\partial A := \overline{X \setminus A} \cap \overline{A}$. \\ \\ 
...
\textbf{Definizione:} siano $X_1, X_2$ spazi topologici, un rettangolo aperto in $X := X_1 \times X_2$ è un insieme della forma $A_1 \times A_2$ dove $A_1$ è aperto in $X_1$ e $A_2$ è aperto in $A_2$. \\
...
\textbf{Definizione:} siano $X$ uno spazio topologico, $\{ x_n \}_{n \in \mathbb{N}} \subseteq X$ e $y \in X$. Si dice che $y$ è un limite per $\{ x_n \}_{n \in \mathbb{N}}$ per $n \rightarrow \infty$ se per ogni $V \in \mathcal{N}(y)$ esiste $M \in \mathbb{N}$ tale che $x_n \in V$ per ogni $n \geq M$, in questo caso scriviamo $x_n \rightarrow y$, per $n \rightarrow \infty$. \\ \\
\textbf{Esempio:} in topologia banale si ha che ogni successione converge ad ogni punto, poiché l'unico intorno è $X$ stesso. \\ \\
\textbf{Definizione:} siano $X$ uno spazio topologico e $Y \subset X$, $Y$ è detto \textit{denso} se $\overline{Y} = X$. \\ \\
\textbf{Definizione:} sia $X$ uno spazio topologico, $X$ è detto \textit{separabile} se esiste $Y \subset X$ denso e numerabile (numerabile se esiste biezione con $\mathbb{N}$).
\subsubsection{Esercizi}
\textbf{1.} siano $X := \mathbb{R}$, $\tau := \{ \emptyset, X \} \cup \{ [q, +\infty) : q \in \mathbb{R} \}$, verificare se $\tau$ è una topologia su $\mathbb{R}$. \\

\textbf{Soluzione:} siano $q, p \in \mathbb{R}$, allora
\begin{gather}
...
\end{gather}
pertanto $\tau$ non è una topologia su $\mathbb{R}$. \\ \\
...
\textbf{3.} Siano $X := \mathbb{N}$ e $\tau := \{ \emptyset, X \} \cup \{q, q+1, \dots : q \in \mathbb{N} \}$. Determinare l'interno di $Y := \{ 5, 11, 12, 25 \}$. \\

\textbf{Soluzione:} per definizione si ha che $\mathring{Y}$ è dato dal più "grande" aperto contenuto in $Y$. Ovviamente vale $\emptyset \subset Y$, notiamo inoltre che per ogni $p \in \mathbb{N}$ si ha che $\{ p, p+1, \dots \} \not\subset Y$, infatti tutti gli aperti di $\tau$ (eccetto $\emptyset$) sono illimitati. Pertanto concludiamo che $\mathring{Y} = \emptyset$. \\ \\
\textbf{4.} Siano $X := \mathbb{R}$ e $\tau := \{ \emptyset, X \} \cup \{ (q, +\infty) : q \in \mathbb{R} \}$. Determinare $\overline{\mathbb{Z}}$ come sottoinsieme di $\mathbb{R}$, successivamente determinare $D(\mathbb{\overline{Z}})$. \\

\textbf{Soluzione:} i chiusi per la topologia $\tau$ definita sono della forma $(-\infty, q]$ con $q \in \mathbb{R}$. Per ogni $q < +\infty$ si ha che $\mathbb{Z} \not\subset (-\infty, q]$, pertanto il più "piccolo" chiuso che contiene $\mathbb{Z}$ è $\mathbb{R}$, dunque $\overline{\mathbb{Z}} = \mathbb{R}$. \\ \\
Sia $x_0 \in \mathbb{R}$, consideriamo un intorno $V \in U(x_0)$, poiché $\overline{\mathbb{Z}} = \mathbb{R}$ si ha che $\overline{\mathbb{Z}} \cap V \setminus \{ x_0 \} \neq \emptyset$, ciò vale per ogni $x_0 \in \mathbb{R}$, dunque $D(\mathbb{Z}) = \mathbb{R}$. \\ \\
\textbf{5.} Determinare $D([4, 9])$ come sottoinsieme di $\mathbb{R}$ per la topologia definita nell'esercizio precedente. \\
...
Ovviamente questo risultato può essere generalizzato a qualsiasi intervallo chiuso della retta reale: siano $a, b \in \mathbb{R}$, allora $D([a, b]) = (-\infty, b]$. \\ \\
...
\textbf{i.} $\partial (A \cap B) \subset \partial (A) \cap \partial (B)$. \\
...
\textbf{Definizione:} sia $X$ un insieme, sia $d_X : X \times X \longrightarrow [0, + \infty)$, $d_X$ è detta \textit{metrica} (o \textit{distanza}) su $X$ se: \\
...
\textbf{Definizione:} sia $(X, d_X)$ uno spazio metrico, è detta \textit{sfera aperta di raggio $r$ e centro $x_0$} l'insieme $D_r(x_0) := \{ x \in X : d_X(x, x_0) < r \}$. \\ \\
...
\textbf{Esempi:} - sia $X := \mathbb{R}^n$, la metrica $d(x_1, x_2) := \sqrt{\sum_{i = 1}^n (x_1^i - x_2^i)^2}$ per ogni $x_1, x_2 \in \mathbb{R}^n$ è detta \textit{metrica euclidea} e induce la topologia euclidea. \\
- sia $X := \mathbb{R}^n$, la metrica $d(x_1, x_2) := \sum_{i = 1}^n |x_1^i - x_2^i|$ per ogni $x_1, x_2 \in \mathbb{R}^n$ induce ancora una volta la metrica euclidea (in questo caso gli aperti sono rombi). \\
- siano $a, b \in \mathbb{R}$, $X := C^0([a, b])$, poniamo $d(f, g) := \max_{t \in [a, b]} |f(t) - g(t)|$ per ogni $f, g \in C^0([a, b])$, si verifica facilmente che $d$ è una metrica e dunque induce una topologia su $C^0([a, b])$. \\ \\
\textbf{Definizione:} sia $V$ uno spazio vettoriale reale, una funzione $\| \cdot \| : V \times V \longrightarrow [0, +\infty)$ è detta \textit{norma} se: \\
\textbf{1.} $\| v \| \geq 0$ per ogni $v \in V$ e $\| v \| = 0$ se e solo se $v = \vec{0}$. \\
\textbf{2.} $\| v + w \| \leq \| v \| + \| w \|$ per ogni $v, w \in V$. \\
\textbf{3.} $\| \lambda v \| = |\lambda| \| v \|$ per ogni $v \in V$ e per ogni $\lambda \in \mathbb{R}$. \\ \\
...
\textbf{Definizione:} sia $X$ uno spazio topologico, $X$ è detto $T_2$ (o \textit{separato} o \textit{di Hausdorff}) se per ogni $x, y \in X$ con $x \neq y$ esistono $V \in \mathcal{N}(x)$ e $W \in \mathcal{N}(y)$ tali che $V \cap W = \emptyset$. \\ \\
...
\textbf{Dimostrazione:} sia $\{ x_n \}_{n \in \mathbb{N}} \subseteq X$ una successione, supponiamo per assurdo che essa converga in due punti $y_1$ e $y_2$ di $X$. Per ipotesi si ha che esistono $V \in \mathcal{N}(y_1)$ e $W \in \mathcal{N}(y_2)$ tali che $V \cap W = \emptyset$. Per definizione di limite esiste $M \in \mathbb{N}$ tale che $x_n \in V$ e $x_n \in W$ per ogni $n \geq M$, ma ciò è un assurdo. \qed \\ \\
...
\textbf{Definizione:} uno spazio topologico $X$ è detto $T_1$ se i punti sono chiusi. \\ \\
...
\textbf{Proposizione 1.2.7:} sia $\{ x_n : n \in \mathbb{N} \}$ una successione di punti in uno spazio $X$ $T_1$ che soddisfa il primo assioma di numerabilità e sia $x \in X$ un punto di accumulazione per $\{ x_n \}$. Allora $\{ x_n \}$ ammette una sottosuccessione convergente in $x$. \\

\textbf{Dimostrazione:} ogni sottoinsieme finito di $\{ x_n \}$ è discreto (i punti sono chiusi), dunque non ammette punti di accumulazione, pertanto $x$ è punto di accumulazione per $\{ x_n : n \geq N \}$ per ogni $N \geq 1$. Sia $U := \{ U_k : k \in \mathbb{N} \}$ un sistema fondamentale di intorni numerabile di $x$ tale che $U_{k + 1} \subset U_k$ per ogni $k \geq 1$. Per ipotesi esiste $n_1 \in \mathbb{N}$ tale che $x_{n_1} \in U_1$. Analogamente esiste $n_2 \in \mathbb{N}$, $n_2 > n_1$, tale che $x_{n_2} \in U_2$. Ragionando per induzione possiamo costruire una sottosuccessione $\{ x_{n_k} : k \in \mathbb{N} \}$ tale che $x_{n_k} \in U_k$ per ogni $k \geq 1$, inoltre è chiaro che $x = \lim_{k \rightarrow \infty} x_{n_k}$. \qed \\ \\
\textbf{Definizione:} uno spazio \textit{regolare} è uno spazio topologico $X$ che soddisfa le seguenti proprietà: \\
\textbf{1.} $X$ è $T_1$. \\
...
\textbf{Definizione:} uno spazio \textit{normale} è uno spazio topologico $X$ che soddisfa le seguenti proprietà: \\
\textbf{1.} $X$ è $T_1$. \\
...
Ad esempio, se uno spazio $X$ ha la proprietà che per ogni coppia di punti distinti $p, q \in X$ esiste un'applicazione continua $f : X \longrightarrow \mathbb{R}$ tale che $f(p) \neq f(q)$, si ha che $X$ è uno spazio di Hausdorff, infatti ponendo $r := d(f(p), f(q))$ e
\begin{gather}
...
\textbf{6.} Siano $(X, d)$ uno spazio metrico, $x \in X$ e $B \subset X$. Dimostrare che $x$ è un punto di accumulazione di $B$ se e solo se esiste una successione $\{x_j : j \in \mathbb{N}\} \subset B\setminus X$ tale che $x_j \rightarrow x$. \\
...
\begin{align}
    \mathcal{V} = \{D_{1/n}(x) : n \in \mathbb{N}\} \notag
\end{align}
Per ogni $n \in \mathbb{N}$ scegliamo un elemento $x_n \in D_{1/n}(x) \cap B$, abbiamo quindi costruito una successione $\{x_n : n \in \mathbb{N}\} \subset B$ tale che
\begin{align}
    d(x_n, x) < \frac{1}{n} \notag
\end{align}
per ogni $n \in \mathbb{N}$, ovvero $x_n \rightarrow x$. \\
$(\leftarrow)$ Sia $\{x_n : n \in \mathbb{N}\} \subset B$ una successione convergente a $x$. Per definizione, per ogni $\varepsilon > 0$ esiste $N_\varepsilon \in \mathbb{N}$ tale che
\begin{align}
    d(x_n, x) < \varepsilon \notag
\end{align}
...
\textbf{Definizione:} sia $X$ uno spazio topologico, $X$ è detto \textit{sconnesso} se esistono $A, B \subset X$ aperti non vuoti tali che $A \cup B = X$ e $A \cap B = \emptyset$. \\ Altrimenti $X$ è detto \textit{connesso}. \\ \\
...
\textbf{Definizione:} $Y \subset X$ è \textit{sconnesso} se e solo se è sconnesso per la topologia indotta da $X$, ovvero $\tau_Y := \{ Y \cap A : A \text{ aperto in } X \}$. \\ \\
...
\textbf{Definizione:} due punti di uno spazio topologico $X$ sono detti \textit{connessi} se esiste un sottoinsieme connesso di $X$ che li contiene entrambi. \\ \\
...
\textbf{Teorema 1.3.1:} gli intervalli di $\mathbb{R}$, in particolare $\mathbb{R}$ stesso, sono connessi. \\
...
\textbf{Definizione:} se per ogni $p, q \in X$ esiste un arco $\gamma$ tale che $\gamma(0) = p$ e $\gamma(1) = q$, $X$ si dice \textit{connesso per archi}. \\ \\
...
\textbf{Definizione:} uno spazio topologico $X$ è detto compatto se per ogni \textit{ricoprimento aperto} $\{ \mathcal{U}_i \}_{i \in I}$ esiste un \textit{sottoricoprimento finito} $U_1, \dots, U_n$ tale che $X = U_1 \cup \dots \cup U_n$. \\
...
$\mathbb{R}$ non è compatto, infatti la famiglia $\{ (-n, n) \}_{n \geq 1}$ non ammette sottoricoprimenti finiti. \\
Ogni intervallo aperto $(a, b)$ di $\mathbb{R}$ non è compatto, infatti la famiglia di intervalli
\begin{gather}
...
\textbf{Teorema (di Heine-Borel):} ogni intervallo chiuso e limitato $[a, b]$ di $\mathbb{R}$ è compatto. \\

\textbf{Dimostrazione:} sia $\mathcal{U}$ un ricoprimento di $[a, b]$ con aperti di $\mathbb{R}$ e sia
\begin{gather}
...
\textbf{Corollario 1.5.1:} un sottoinsieme $K$ di $\mathbb{R}$ è compatto se e solo se è chiuso e limitato. \\

\textbf{Dimostrazione:} $(\rightarrow)$ Sia $K \subset \mathbb{R}$ compatto e sia $X$ di Hausdorff. Per la \textit{proposizione 1.5.1} si ha che $K$ è chiuso. Inoltre è sicuramente limitato, altrimenti $\{ (-n, n) \}_{n \geq 1}$ sarebbe un ricoprimento aperto che non ammette sottoricoprimento finito. \\ \\
$(\leftarrow)$ Sia $K$ chiuso e limitato. Poniamo
\begin{gather}
    a := \inf K, \quad b := \sup K \notag
\end{gather}
poiché $K$ è chiuso si ha $a, b \in K$, allora $K \subseteq [a, b]$ che è compatto perché è un intervallo chiuso di $\mathbb{R}$, inoltre $K$ è chiuso in $[a, b]$, pertanto $K$ è un sottoinsieme chiuso di uno spazio compatto, per la \textit{proposizione 1.5.1} $K$ è compatto. \qed \\ \\
...
\textbf{Teorema (di Bolzano - Weierstrass):} ogni successione limitata $\{ x_n : n \in \mathbb{N} \}$ di numeri reali ammette una sottosuccessione convergente. \\

\textbf{Dimostrazione:} poiché $\{ x_n \}$ è limitata esiste $M > 0$ tale che $\{ x_n \} \subset [-M, M]$, ma $[-M, M]$ è un sottoinsieme compatto di $\mathbb{R}$, pertanto, per la \textit{proposizione 1.5.2}, $\{ x_n \}$ ammette una sottosuccessione convergente. \qed \\ \\
...
\textbf{Corollario 1.5.2:} sia $X$ uno spazio topologico e $f : X \longrightarrow \mathbb{R}$ continua. Se $X$ è compatto $f$ assume un valore massimo ed un valore minimo su $\mathbb{R}$. \\
...
\textbf{Corollario 1.5.3:} un sottoinsieme $K$ di $\mathbb{R}^n$ è compatto se e solo se è chiuso e limitato. \\

\textbf{Dimostrazione:} sia $K$ compatto, poiché $\mathbb{R}^n$ è di Hausdorff, si ha che $K$ è chiuso, inoltre è limitato poiché altrimenti la famiglia dei dischi aperti centrati nell'origine di raggio crescente sarebbe un ricoprimento aperto che non ammette un sottoricoprimento finito. \\ \\
Sia $K$ chiuso e limitato, allora $K \subset \mathbf{I}_1 \times \dots \times \mathbf{I}_1$ ($n$ copie di $\mathbf{I}_1$) che è compatto per il \textit{teorema di Tichonov}, inoltre $K$ è chiuso in $\mathbf{I}_1 \times \dots \times \mathbf{I}_1$ poiché è chiuso in $\mathbb{R}^n$, pertanto $K$ è un sottoinsieme chiuso di uno spazio compatto, segue che $K$ è compatto. \qed
\subsubsection{Osservazioni e complementi}
\textbf{1.} Dal \textit{corollario 1.5.1} segue che gli unici intervalli compatti di $\mathbb{R}$ sono gli intervalli chiusi e limitati. Inoltre due intervalli chiusi e limitati qualsiasi sono omeomorfi, pertanto, poiché la compattezza è una proprietà topologica, si ha che se $\mathbf{J}_1, \mathbf{J}_2 \subset \mathbb{R}$ sono intervalli omeomorfi e $\mathbf{J}_1$ è compatto, anche $\mathbf{J}_2$ è compatto. \\ \\
\textbf{2.} Dal \textit{corollario 1.5.3} segue che $D_r(\mathbf{x})$, $\mathbf{x} \in \mathbb{R}^n$, non è compatto poiché è aperto. Un ricoprimento aperto che non ammette sottoricoprimento finito è
\begin{gather}
...
\textbf{3.} La sfera $\mathbf{S}^n$, $n \geq 1$, è compatta poiché chiusa e limitata in $\mathbb{R}^{n + 1}$. \\
Il \textit{toro} $n$-dimensionale
\begin{gather}
    \mathbf{T}^n = \mathbf{S}^1 \times \dots \times \mathbf{S}^1 \notag
\end{gather}
è compatto per il \textit{teorema di Tichonov}. \\ \\
\textbf{4.} Il gruppo ortogonale $\mathbf{O}_n(\mathbb{R})$ è compatto. Infatti la condizione che definisce le matrici $A = (a_{ij}) \in \mathbf{O}_n(\mathbb{R}^n)$ è $AA^t = \mathbf{I}_n$, cioè:
\begin{gather}
...
\begin{gather}
    \sum_{h, i} a^2_{hi} = n \notag
\end{gather}
Pertanto $\mathbf{O}_n(\mathbb{R})$ è un sottoinsieme chiuso di $\mathbb{R}^{n^2}$ contenuto nel disco chiuso $\overline{D}_{\sqrt{n}}(0)$. Pertanto $\mathbf{O}_n(\mathbb{R})$ è chiuso e limitato, cioè compatto. In modo analogo si dimostra per $\mathbf{SO}_n(\mathbb{R})$. \\ \\
...
\textbf{3.} I seguenti sottoinsiemi di $\mathbb{R}^n$, $n \geq 2$, non sono compatti:
\begin{gather}
    \mathbf{D}^n \setminus \{ 0 \}, \quad \mathbf{S}^{n - 1} \setminus \{ (1, 0, \dots, 0) \}, \quad \{ x \in \mathbb{R}^n : x_n = 0 \}, \quad \mathring{\mathbf{I}}^n \notag
\end{gather}
Esibire per ognuno un ricoprimento aperto che non ammette sottoricoprimento finito. \\

\textbf{Soluzione:} $\mathbb{R}^n$ è uno spazio di Hausdorff. Per ogni $\mathbf{x} \in \mathbf{D}^n \setminus \{ 0 \}$ scegliamo $V_\mathbf{x}, W_\mathbf{x}$ aperti disgiunti tali che
\begin{gather}
...
\begin{gather}
    \det G := \{ \det A : A \in G \} \subseteq \mathbb{R} \notag
\end{gather}
non è compatto. Infatti per ogni $M \in \mathbb{R}$ esiste $n \in \mathbb{N}$ tale che
\begin{gather}
    |\det A^n| = a^n > M \notag
\end{gather}
basta scegliere $n > \log_a M$, perciò $\det G$ non è limitato e per il teorema di Heine-Borel non è compatto. \\
Tuttavia $\det : \mathbf{M}_n \longrightarrow \mathbb{R}$ è continua, dunque per la \textit{proposizione 1.5.3} si ha che $\det(G)$ è compatto, ma ciò è un assurdo. \\ \\
...
\textbf{Definizione:} uno spazio topologico $X$ è detto \textit{localmente compatto} se ogni elemento $p \in X$ ammette un intorno compatto. \\ \\
Esempi di insiemi localmente compatti sono $\mathbb{R}^n$ per ogni $n \in \mathbb{N}$, gli aperti di $\mathbb{R}^n$ e in generale le varietà topologiche, mentre $\mathbb{Q}^n$ non è localmente compatto per ogni $n \in \mathbb{N}$. \\ \\
...
\textbf{Proposizione 1.6.2:} sia $X$ uno spazio topologico localmente compatto che soddisfa il secondo assioma di numerabilità, allora $X$ è unione di una famiglia numerabile di sottoinsiemi compatti $\{K_i : i \in \mathbb{N}\}$ tale che $K_i \subset \mathring{K}_{i + 1}$ per ogni $i \in \mathbb{N}$. \\

\textbf{Dimostrazione:} sia $x \in X$, sia $U$ un aperto contenente $x$ e sia $K$ un intorno compatto di $x$. Per ipotesi $X$ possiede una base numerabile di aperti $\mathcal{B}$, allora esiste $U_i \in \mathcal{B}$ tale che $U_i \subset \mathring{K} \cap U$. Inoltre per il punto $(b)$ della \textit{proposizione 1.6.1} si ha che $U_i$ è relativamente compatto in $\mathring{K} \cap U$. Dunque per ogni $x \in X$ e ogni aperto $U$ contenente $x$ esiste $U_i \in \mathcal{B}$ relativamente compatto tale che $x \in U_i \subset U$. Allora $\{U_i : i \in \mathbb{N}\}$ è una base numerabile di aperti relativamente compatti di $X$. Per ogni $i \in \mathbb{N}$ si ha che $\overline{U}_i$ è ricoperto da un numero finito di $U_j$, la cui unione ha chiusura compatta $H_i$ dato che si tratta di insiemi relativamente compatti. Pertanto
\begin{align}
    \overline{U}_i \subset \mathring{H}_i \notag
\end{align}
per ogni $i \in \mathbb{N}$ e quindi
\begin{align}
    X = \bigcup_{i \in \mathbb{N}}\mathring{H}_i \notag
\end{align}
A questo punto definiamo induttivamente
\begin{align}
    K_1 := H_1 \qquad K_{i + 1} := \bigcup_{j \in J(i)}H_j \notag
\end{align}
dove $J(i)$ è un sottoinsieme di $\mathbb{N}$ tale che
\begin{align}
    K_i \subset \bigcup_{j \in J(i)}\mathring{H}_j \notag
\end{align}
La famiglia di sottoinsiemi compatti $\{K_i\}$ soddisfa le proprietà ricercate. \\ \\
L'esistenza di una famiglia di compatti con le proprietà enunciate nella \textit{proposizione 1.6.2} si può dimostrare direttamente nel caso $X = \mathbb{R}^n$, o più generale nel caso $X = S \subset \mathbb{R}^n$, con $S$ aperto o chiuso. Infatti se $X$ è chiuso basta prendere
\begin{align}
    K_i = X \cap \overline{D}_i(p) \notag
\end{align}
fissato un punto $p$ di $X$. Mentre se $X$ è aperto basta prendere
\begin{align}
    K_i = \{x \in X : \|x\| \leq i, \, d(x, \mathbb{R}^n \setminus X) \geq 1/i\} \notag
\end{align}
notiamo che in questo caso $K_i$ è chiuso nella topologia relativa ed è compatto poiché chiuso e limitato in $\mathbb{R}^n$. \qed \\ \\
\textbf{Definizione:} sia $X$ uno spazio topologico e siano $\mathcal{U}, \mathcal{V}$ ricoprimenti aperti di $X$. $\mathcal{V}$ è detto \textit{raffinamento} di $\mathcal{U}$, o \textit{più fine} di $\mathcal{U}$, se ogni $V \in \mathcal{V}$ è contenuto in qualche aperto di $\mathcal{U}$. \\ \\
\textbf{Definizione:} una famiglia $\mathcal{F}$ di sottoinsiemi di $X$ si dice \textit{localmente finita} se ogni $x \in X$ possiede un intorno $U_x$ che interseca solo con un numero finito di elementi di $\mathcal{F}$. \\ \\
\textbf{Definizione:} uno spazio topologico $X$ si dice \textit{paracompatto} se ogni ricoprimento $\mathcal{U}$ di $X$ possiede un raffinamento $\mathcal{V}$ localmente finito. \\ \\
...
\textbf{Corollario 1.6.1:} ogni aperto o chiuso di $\mathbb{R}^n$ è paracompatto, ogni varietà topologica è paracompatta. \\ \\
\textbf{Definizione:} uno spazio topologico $X$ è detto \textit{numerabilmente compatto} se ogni sottoinsieme infinito $Z \subset X$ possiede un punto di accumulazione. Uno spazio topologico $X$ è detto \textit{compatto per successioni} se ogni successione di punti in $X$ ammette una sottosuccessione convergente in $X$. \\ \\
...
\textbf{Definizione:} sia $X$ uno spazio metrizzabile e sia $\mathcal{U}$ un ricoprimento aperto di $X$. Se esiste $\varepsilon > 0$ tale che il ricoprimento aperto costituito dai dischi aperti di raggio $\varepsilon$ è un raffinamento di $\mathcal{U}$, $\varepsilon$ è detto numero di Lebesgue del ricoprimento $\mathcal{U}$. \\ \\
...
\end{align}
mostriamo che $\varepsilon_0 > 0$. Supponiamo per assurdo $\varepsilon_0 = 0$, allora esiste una successione $\{x_n : n  \in \mathbb{N}\}$ tale che 
\begin{align}
...
\end{align}
Per definizione esiste $N \in \mathbb{N}$ tale che $d(x_0, x_n) < \frac{1}{4}\varepsilon(x_0)$ per ogni $n > N$. Allora si ha 
\begin{align}
...
\textbf{Definizione:} sia $X$ uno spazio metrico, una successione $\{x_n\}_{n \in \mathbb{N}}$ è detta di Cauchy se per ogni $\varepsilon > 0$ esiste $N_\varepsilon \in \mathbb{N}$ tale che $d(x_n, x_m) < \varepsilon$ per ogni $n, m > N_\varepsilon$. \\ \\
\textbf{Osservazione:} ogni successione convergente è anche di Cauchy. Infatti data $\{x_n\}_{n \in \mathbb{N}}$ convergente a $x \in X$, per ogni $\varepsilon > 0$ esiste $N_\varepsilon \in \mathbb{N}$ tale che $d(x_n, x_m) \leq d(x_n, x) + d(x, x_m) < 2\varepsilon$ per ogni $n, m > N_\varepsilon$, per la disuguaglianza triangolare. \\ \\
\textbf{Definizione:} uno spazio metrico è detto \textit{completo} se ogni successione di Cauchy è anche convergente. \\ \\
Dai corsi di Analisi Matematica sappiamo che $\mathbb{R}^n, \mathbb{C}^n$ sono spazi metrici completi rispetto alla metrica indotta dalla norma per ogni $n \geq 1$ (partendo dal caso $n = 1$ e usando successioni a variazione finita, poi generalizzando al caso $n \geq 1$). \\ \\
La completezza non è una proprietà topologica, ovvero non si conserva rispetto a omeomorfismi. Ad esempio $\mathbb{R} \cong (0, 1)$, tuttavia $\mathbb{R}$ è completo, mentre $(0, 1)$ no, infatti $\{1/n\}_{n \in \mathbb{N}}$ è una successione di Cauchy in $(0, 1)$ ma non è ivi convergente. \\ \\
\textbf{Definizione:} uno spazio metrico è detto \textit{totalmente limitato} se per ogni $\varepsilon > 0$ esiste un ricoprimento finito costituito da dischi chiusi di raggio $\varepsilon$. \\ \\
...
\textbf{Dimostrazione:} $(\rightarrow)$ sia $(X, d)$ uno spazio metrico compatto. Per il \textit{teorema 1.6.1} $(X, d)$ è compatto per successioni, pertanto ogni successione in $X$ ammette una sottosuccessione convergente ad un opportuno punto di $X$. Sia $\{x_n\}_{n \in \mathbb{N}}$ una successione di Cauchy in $X$. $\{x_n\}_{n \in \mathbb{N}}$ ammette una sottosuccessione $\{x_{\nu(k)}\}_{k \in \mathbb{N}}$ convergente ad un opportuno $x \in X$. Fissato $k$ abbastanza grande segue che 
\begin{align}
    d(x_n, x) \leq d(x_n, x_{\nu(k)}) + d(x_{\nu(k)}, x) < 2\varepsilon \notag
\end{align}
per ogni $n > N_\varepsilon$ (per ipotesi di Cauchy), ovvero $\{x_n\}_{n \in \mathbb{N}}$ converge a $x$, pertanto $(X, d)$ è completo. Inoltre $X$ è totalmente limitato, infatti ogni ricoprimento aperto ammette sottoricoprimento finito, in particolare anche il ricoprimento aperto costituito dai dischi di raggio $\varepsilon$ per ogni $\varepsilon > 0$. \\ \\
$(\leftarrow)$ Sia $(X, d)$ completo e totalmente limitato. Dimostriamo che $X$ è compatto per successioni e dal \textit{teorema 1.6.1} seguirà la tesi. Sia $\{x_n\}_{n \in \mathbb{N}}$ una successione di punti in $X$. Se $\{x_n\}_{n \in \mathbb{N}}$ ha un numero finito di punti distinti esiste certamente una sottosuccessione convergente. Supponiamo allora che $\{x_n\}_{n \in \mathbb{N}}$ abbia un numero infinito di punti distinti e costruiamo per induzione una sottosuccessione convergente. Per ogni $k \in \mathbb{N}$ consideriamo un ricoprimento chiuso di $X$ composto da un numero finito di dischi di raggio $\frac{1}{2^k}$. Sia $k = 1$, per ipotesi uno dei dischi contiene un numero infinito di termini della successione, chiamiamo $D_1$ tale disco ed indichiamo con $I_1$ l'insieme (infinito) degli indici che identificano i termini della successione contenuti in $D_1$. Sia $k = 2$, poiché $\{x_n\}_{n \in I_1}$ ha sempre infiniti termini distinti esiste un disco, che chiamiamo $D_2$, che contiene un numero infinito di termini della successione ed indichiamo con $I_2$ l'insieme degli indici che li identificano, ovviamente si ha $I_2 \subset I_1$. Per ogni $k \geq 2$ possiamo costruire induttivamente $I_k$ in modo analogo. Costruiamo una sottosuccessione $\{x_{n_k}\}_{k \in \mathbb{N}}$ con $n_1 < n_2 < \dots$ tale che $n_k \in I_k$ per ogni $k \in \mathbb{N}$. Allora per ogni $i, j > k$ si ha 
\begin{align}
    d(x_{n_i}, x_{n_j}) < \frac{1}{2^{k - 1}} \notag
\end{align}
ovvero $\{x_{n_k}\}_{k \in \mathbb{N}}$ è di Cauchy. Per la completezza di $X$ la sottosuccessione $\{x_{n_k}\}$ convergente ad un opportuno punto in $X$ e quindi $X$ è compatto per successioni. \qed \\ \\
...
\textbf{Dimostrazione:} poiché per ipotesi $D_1 \supset D_2 \supset \dots$ si ha che $d(x_n, x_m) < |r_n - r_m|$ per ogni $n, m \in \mathbb{N}$. Allora $\{x_n\}_{n \in \mathbb{N}}$ è di Cauchy e quindi convergente, pertanto esiste $x \in X$ tale che $x_n \rightarrow x$ per $n \rightarrow \infty$. \\
...
\textbf{Definizione A.5:} sia $n \in \mathbb{N}$. Il gruppo simmetrico (di rango $n$) è
\begin{gather}
...
\textbf{Teorema (di Cayley):} sia $G$ un gruppo finito, allora esiste $n \in \mathbb{N}$ e $S \leq S_n$ tali che $G$ è isomorfo a $S$. \\
...
\begin{gather}
    ...
    \end{pmatrix} \notag
\end{gather}
dove $\langle \cdot, \cdot \rangle : \mathbb{R}^n \times \mathbb{R}^n \longrightarrow \mathbb{R}$ è il prodotto scalare euclideo. \\ \\
\textbf{Definizione:} sia $n \in \mathbb{N}$, poniamo
\begin{gather}
    \mathbf{M}_n(\mathbb{R}) := \{ \text{ matrici $n \times n$ a coefficienti reali} \} \notag
\end{gather}
Se $A \in \mathbf{M}_n(\mathbb{R})$ si ha che $a_{ij}$ è l'elemento in riga $i$ e colonna $j$; $A_i$ è l'$i$-esima riga. \\ \\
\textbf{Definizione:} il \textit{gruppo generale lineare} (di rango $n$) è
\begin{gather}
    \mathbf{GL}_n(\mathbb{R}) := \{ A \in \mathbf{M}_n(\mathbb{R}) : A \text{ invertibile} \} \notag
\end{gather}
che equivale a
\begin{gather}
    \mathbf{GL}_n(\mathbb{R}) := \{ A \in \mathbf{M}_n(\mathbb{R}) : \det A \neq 0 \} \notag
\end{gather} \\ \\
\textbf{Definizione:} il \textit{gruppo ortogonale} (di rango $n$) è
\begin{gather}
    \mathbf{O}_n(\mathbb{R}) := \{ A \in \mathbf{GL}_n(\mathbb{R}) : \langle x, y \rangle = \langle xA, yA \rangle \text{ per ogni } x, y \in \mathbb{R}^n \} \notag
\end{gather} \\ \\
\textbf{Teorema 2.2.1:} sia $A \in \mathbf{GL}_n(\mathbb{R})$, i seguenti punti sono equivalenti: \\
\textbf{1.} $A \in \mathbf{O}_n(\mathbb{R})$. \\
...
\end{gather} \qed \\
\textbf{Osservazioni:} $\mathbf{GL}_n(\mathbb{R})$ è un gruppo rispetto al prodotto tra matrici, inoltre $\mathbf{O}_n(\mathbb{R}) \leq \mathbf{GL}_n(\mathbb{R})$. \\ \\
\textbf{Proposizione 2.2.1:} $\mathbf{O}_n(\mathbb{R}) = \{ A \in \mathbf{GL}_n(\mathbb{R}) : \| x \| = \| xA \| \text{ per ogni } x \in \mathbb{R}^n \}$ \\

\textbf{Dimostrazione:} sia $A \in \mathbf{O}_n(\mathbb{R})$, si ha che
\begin{gather}
    \| x \| = \sqrt{\langle x, x \rangle} = \sqrt{\langle xA, xA \rangle} = \| xA \| \notag
\end{gather}
per definizione. \qed \\ \\
\textbf{Definizione:} il \textit{gruppo speciale lineare} è
\begin{gather}
    \mathbf{SL}_n(\mathbb{R}) := \{ A \in \mathbf{M}_n(\mathbb{R}) : \det A = 1 \} \notag
\end{gather}
Notiamo che si ha $\mathbf{SL}_n(\mathbb{R}) \leq \mathbf{GL}_n(\mathbb{R})$. \\ \\
\textbf{Proposizione 2.2.2:} sia $A \in \mathbf{O}_n(\mathbb{R})$, allora $|\det A| = 1$. \\

\textbf{Dimostrazione:} vale $\det \mathbf{I}_n = \det (AA^t) = (\det A)^2$. \qed \\ \\
\textbf{Definizione:} il \textit{gruppo speciale ortogonale} è
\begin{gather}
    \mathbf{SO}_n(\mathbb{R}) := \{ A \in \mathbf{O}_n(\mathbb{R}) : \det A = 1 \} \notag
\end{gather}
Notiamo subito che $\mathbf{SO}_n(\mathbb{R}) \leq \mathbf{O}_n(\mathbb{R})$. \\ \\
\textbf{Definizione:} $f : \mathbb{R}^n \longrightarrow \mathbb{R}^n$ si dice \textit{isometria} se $\| x - y \| = \| f(x) - f(y) \|$ per ogni $x, y \in \mathbb{R}^n$. \\ \\
\textbf{Teorema 2.2.2:} sia $B \in \mathbf{O}_n(\mathbb{R})$ e sia $f$ un'isometria tale che $f(0) = 0$ (fissa l'origine). Allora: \\
\textbf{1.} $x \hookrightarrow xB$ è un'isometria. \\
\textbf{2.} esiste $A \in \mathbf{O}_n(\mathbb{R})$ tale che $f(x) = xA$ per ogni $x \in \mathbb{R}^n$. \\
...
\begin{gather}
    ...
    \end{pmatrix} \notag
\end{gather}
$A \in \mathbf{O}_n(\mathbb{R})$ perché le righe di $A$ sono una base ortonormale, infatti
\begin{gather}
    \langle f(\mathbf{e}_i), f(\mathbf{e}_j) \rangle = \langle \mathbf{e}_i, \mathbf{e}_j \rangle = \delta_{ij} \notag
\end{gather}
Inoltre per ogni $x \in \mathbb{R}^n$ si ha $x = \sum \lambda_i \mathbf{e}_i$, dunque
\begin{gather}
    xA = (\sum_i \lambda_i \mathbf{e}_i)A = \sum_i \lambda_i A_i = \sum_i \lambda_i f(\mathbf{e}_i) \notag
\end{gather}
Sia $g$ un'isometria definita ponendo $g(x) = f(x)A^{-1}$ per ogni $x \in \mathbb{R}^n$, mostriamo che $g = \mathbf{I}_n$. Poiché $g$ conserva il prodotto scalare cartesiano si ha che
\begin{gather}
...
\end{gather}
ovvero $g(x) = x$ per ogni $x \in \mathbb{R}^n$, dunque $g = \mathbf{I}_n$. \qed \\ \\
\textbf{Definizione:} una base ordinata ortonormale $\{ v_1, v_2, v_3 \}$ di $\mathbb{R}^3$ è detta \textit{destra} se $v_3 = v_1 \wedge v_2$. \\ \\
\textbf{Proposizione 2.2.3:} sia $A \in \mathbf{O}_3(\mathbb{R})$, se $\{ A_1, A_2, A_3 \}$ è una base destra di $\mathbb{R}^3$ si ha che $A \in \mathbf{SO}_n(\mathbb{R})$. \\

\textbf{Dimostrazione:} immediata. \qed \\ \\
Notiamo che gli elementi di $\mathbf{SO}_3(\mathbb{R})$ sono le isometrie che fissano l'origine e mandano la base canonica di $\mathbb{R}^3$ in una base destra. \\ \\
Poniamo $Isom(\mathbb{R}^n) := \{ f : \mathbb{R}^n \longrightarrow \mathbb{R}^n : f \text{ isometria} \}$. \\ \\
Osserviamo che $Isom(\mathbb{R}^n)$ è un gruppo rispetto alla composizione di funzioni. Inoltre $\mathbf{O}_(\mathbb{R}) \leq Isom(\mathbb{R}^n)$, infatti il gruppo ortogonale è composto dalle isometrie che fissano l'origine.  \\
Tuttavia ponendo $v := f(0)$ si ha che $g(x) := f(x) - v$ è un isometria che fissa l'origine per ogni isometria $f$, pertanto, per il \textit{teorema 2.2.2} esiste $A \in \mathbf{O}_n(\mathbb{R})$ tale che $g(x) = xA$ per ogni $x \in \mathbb{R}^n$, cioè $f(x) = xA + v$. \\
...
\begin{gather}
    ...
    = (xA + v, 1) \notag
\end{gather} \\ \\
\textbf{Teorema 2.2.3:} sia $A \in \mathbf{SO}_3(\mathbb{R})$. Allora esiste une retta $r$ passante per l'origine tale che $x \hookrightarrow xA$ è una rotazione attorno a $r$. \\

\textbf{Dimostrazione:} siano $\lambda_1, \lambda_2, \lambda_3 \in \mathbb{C}$ gli autovalori di $A$, mostriamo che almeno uno di loro è uguale a $1$. Poiché $A$ è a coefficienti reali almeno uno degli autovalori è reale, supponiamo, per fissare le idee, che $\lambda_1 \in \mathbb{R}$. \\
Notiamo che se $\lambda_1 \in \mathbb{R}$ si ha $|\lambda_1| = 1$ perché si conservano le distanze e i moduli. Infatti, sia $v_1 \in \mathbb{R}^3 \setminus \{ 0 \}$ autovettore corrispondente all'autovalore $\lambda_1$, si ha che
\begin{gather}
    \| v_1 \| = \| v_1A \| = \| \lambda_1 v_1 \| = |\lambda_1|\| v_1 \| \notag
\end{gather}
perciò $|\lambda_1| = 1$. Inoltre se $\lambda_2 \in \mathbb{C} \setminus \mathbb{R}$ si ha che $\lambda_3 = \overline{\lambda_2}$, quindi $\lambda_2 \cdot \lambda_3 \in \mathbb{R}$. Pertanto si ottiene
\begin{gather}
...
\textbf{Definizione:} sia $X \subset \mathbb{R}^n$, il \textit{gruppo delle simmetrie di $X$} è
\begin{gather}
    Symm(X) := \{ f \in Isom(\mathbb{R}^n) : f(X) = X \} \notag
\end{gather}
Ad esempio
\begin{gather}
    Symm(\mathbf{S}^{n - 1}) = \{ f \in Isom(\mathbb{R}^n) : f(0) = 0 \} = \mathbf{O}_n(\mathbb{R}) \notag
\end{gather}
ovvero il gruppo delle simmetrie della sfera centrata nell'origine in $\mathbb{R}^n$ è composto da tutti i movimenti rigidi che fissano l'origine. \\ \\
Poniamo
\begin{gather}
...
\end{gather} \\ \\
\textbf{Definizione:} sia $m \in \mathbb{N}$, $m \geq 3$, il \textit{gruppo diedrale} di ordine $2m$ è
\begin{gather}
    D_{2m} := Symm(m\text{-gono regolare}) \notag
\end{gather}
anche scritto $I_2(m)$. \\ \\
\textbf{Definizione:} il \textit{gruppo ciclico} di ordine $m$ è
\begin{gather}
    \mathbb{Z}_m := Symm^+(m\text{-gono regolare}) \notag
\end{gather} \\
Con regolare si intende lati e angoli tutti uguali. \\ \\
\textbf{Teorema (di Leonardo da Vinci):} sia $X \subset \mathbb{R}^2$ tale che $|Symm(X)| < +\infty$. Allora $Symm(X) \cong \mathbb{Z}_m$ o $Symm(X) \cong I_2(m)$ per qualche $m \geq 2$. \\

\textbf{Dimostrazione:} omessa. \qed \\ \\
\textbf{Esempio:} $I_2(2)$ sono le simmetrie di un segmento di $\mathbb{R}^2$. \\ \\
\textbf{Proposizione 2.2.4:} sia $X \subseteq \mathbb{R}^n$, si ha che
\begin{gather}
    Symm^+(X) = Symm(X) \notag
\end{gather}
oppure
...
\textbf{Dimostrazione:} omessa. \qed \\ \\
\textbf{Teorema 2.2.3:} sia $X \subseteq \mathbb{R}^3$ tale che $|Symm^+(X)| < +\infty$, si ha che $Symm^+(X) \cong S_4$ oppure $A_4$ o $A_5$ o $\mathbb{Z}_m$ o $I_2(m)$ per qualche $m \geq 2$, con $A_n := \{ \sigma \in S_n : \sigma \text{ è pari} \}$. \\

\textbf{Dimostrazione:} omessa. \qed \\ \\
\textbf{Definizione:} sia $X$ un poliedro in $\mathbb{R}^3$. $X$ è detto \textit{regolare} se valgono: \\
...
\textbf{Dimostrazione:} omessa. \qed \\ \\
Dato $X \subset \mathbb{R}^3$ scriviamo un algoritmo per calcolare $Symm^+(X)$. \\ \\
\textbf{1.} Posizioniamo $X$ in modo tale che il baricentro corrisponda con l'origine degli assi. Perciò se $f \in Symm^+(X)$ si ha $f(0) = 0$, ovvero $Symm^+(X) \leq \mathbf{SO}_3(\mathbb{R})$. \\ \\
\textbf{2.} Per il \textit{teorema 2.2.3} si ha che gli elementi di $\mathbf{SO}_3(\mathbb{R})$ sono rotazioni attorno ad una retta $r$ passante per l'origine. Pertanto gli elementi di $Symm^+(X)$ sono rotazioni attorno a qualche retta $r$ passante per l'origine $\underline{0}$ che mandano $X$ in $X$. \\ \\
...
\textbf{6.} Si ha che $n/2$ può essere $60, 24, 12$ o un numero diverso dai precedenti. Se $n/2 = 60$ si ha $Symm^+(X) \cong \mathbb{Z}_{60}$ (oppure $\cong I_2(30)$, oppure $\cong A_5$). Se $n/2 = 24$ si ha $Symm^+(X) \cong \mathbb{Z}_{24}$ (oppure $\cong I_2(12)$, oppure $\cong S_4$). Se $n/2 = 12$ si ha $Symm^+(X) \cong \mathbb{Z}_{12}$ (oppure $\cong I_2(6)$, oppure $\cong A_4$). Se $n/2 = m$ diverso dai numeri precedenti si ha $Symm^+(X) \cong \mathbb{Z}_m$ (oppure $\cong I_2(m/2)$). \\ \\
\textbf{7.} Determinare quante coppie $(\hat{u}, \theta)$ sono tali che $\theta = \pi$ (rotazioni che eseguite due volte danno l'identità). Se $n/2 = 60$ possono essere $1 \implies Symm^+(X) \cong \mathbb{Z}_{60}$, $31 \implies Symm^+(X) \cong I_2(30)$ oppure $15 \implies Symm^+(X) \cong A_5$. \\
Se $n/2 = 24$ possono essere $1 \implies Symm^+(X) \cong \mathbb{Z}_{24}$, $13 \implies Symm^+(X) \cong I_2(12)$ oppure $9 \implies Symm^+(X) \cong S_4$. \\
Se $n/2 = 12$ possono essere $1 \implies Symm^+(X) \cong \mathbb{Z}_{12}$, $7 \implies Symm^+(X) \cong I_2(6)$ oppure $3 \implies Symm^+(X) \cong A_4$. \\
Se $n/2 = m$ possono essere $\leq 1 \implies Symm^+(X) \cong \mathbb{Z}_m$ oppure $\geq 2 \implies Symm^+(X) \cong I_2(m/2)$.

\newpage
\subsection{Topologia dei gruppi di matrici}
Notiamo che $\mathbf{M}_n(\mathbb{R}) \cong \mathbb{R}^{n^2}$ pertanto possiamo trattare lo spazio delle matrici $n \times n$ come uno spazio topologico euclideo. \\ \\
\textbf{Definizione:} un \textit{gruppo di matrici} è un sottogruppo $G$ di $\mathbf{GL}_n(\mathbb{R})$ tale che $G$ è chiuso in $\mathbf{GL}_n(\mathbb{R})$ (topologia relativa). \\ \\
Ovviamente $\mathbf{GL}_n(\mathbb{R})$ è chiuso in se stesso e quindi è un gruppo di matrici. \\ \\
\textbf{Proposizione 2.3.1:} $\mathbf{O}_n(\mathbb{R}), \mathbf{SO}_n(\mathbb{R})$ e $\mathbf{SL}_n(\mathbb{R})$ sono gruppi di matrici. \\

\textbf{Dimostrazione:} sappiamo che $\mathbf{O}_n(\mathbb{R})$ è un sottogruppo di $\mathbf{GL}_n(\mathbb{R})$, mostriamo che è chiuso nella topologia relativa. Consideriamo la funzione
\begin{gather}
    f : \mathbf{M}_n(\mathbb{R}) \longrightarrow \mathbf{M}_n(\mathbb{R}), \quad f(A) = AA^t \notag
\end{gather}
ovviamente $f$ è continua (è un polinomio), inoltre $\{ \mathbf{I}_n \}$ è chiuso (per le proprietà di separazione) e
\begin{gather}
    f^{-1}(\{ \mathbf{I}_n \}) = \mathbf{O}_n(\mathbb{R}) \notag
\end{gather}
dunque $\mathbf{O}_n(\mathbb{R})$ è chiuso in $\mathbb{R}^{n^2}$, pertanto è chiuso anche in $\mathbf{GL}_n(\mathbb{R})$, nella topologia relativa. \\ \\
Analogamente $\det : \mathbf{M}_n(\mathbb{R}) \longrightarrow \mathbb{R}$ è continua (anch'essa è un polinomio) e
\begin{gather}
    \mathrm{det}^{-1}(\{ 1 \}) = \mathbf{SL}_n(\mathbb{R}) \notag
\end{gather}
pertanto anche $\mathbf{SL}_n(\mathbb{R})$ è chiuso in $\mathbb{R}^{n^2}$ e quindi anche nella topologia relativa su $\mathbf{GL}_n(\mathbb{R})$. \\ \\
Infine si ha che $\mathbf{SO}_n(\mathbb{R}) = \mathbf{SL}_n(\mathbb{R}) \cap \mathbf{O}_n(\mathbb{R})$, dunque intersezione di due chiusi e quindi chiuso. \qed \\ \\
\textbf{Proposizione:} $\mathbf{O}_n(\mathbb{R})$ e $\mathbf{SO}_n(\mathbb{R})$ sono compatti. \\

\textbf{Dimostrazione:} sia $A \in \mathbf{O}_n(\mathbb{R})$, si ha che $AA^t = \mathbf{I}_n$, pertanto
\begin{gather}
...
\begin{gather}
    \sum_{h, i} a^2_{ih} = n \notag
\end{gather}
cioè $\mathbf{O}_n(\mathbb{R}) \subset \overline{D_{\sqrt{n}}(0)}$, allora $\mathbf{O}_n(\mathbb{R})$ è un sottoinsieme chiuso e limitato di $\mathbb{R}^{n^2}$ e per il \textit{corollario 1.5.3} è compatto. Inoltre $\mathbf{SO}_n(\mathbb{R})$ è un sottoinsieme chiuso di un insieme compatto e pertanto compatto. \qed

\subsubsection{Esercizi}
\textbf{1.} Dimostrare se $\mathbf{O}_n(\mathbb{R}) \leq \mathbf{O}_{n + 1}(\mathbb{R})$, cioè se esiste $S \leq \mathbf{O}_{n + 1}(\mathbb{R})$ tale che $\mathbf{O}_n(\mathbb{R}) \cong S$. \\

\textbf{Dimostrazione:} consideriamo $f : \mathbf{O}_n(\mathbb{R}) \longrightarrow S$ definita ponendo
\begin{gather}
    f(A) = \begin{pmatrix}
        A & 0 \\
        0 & 1
    \end{pmatrix} \text{ per ogni } A \in \mathbf{O}_n(\mathbb{R}) \notag
\end{gather}
con $S := f(\mathbf{O}_n(\mathbb{R}))$. Notiamo immediatamente che $f$ è biettiva e continua e anche $f^{-1}$ lo è. Inoltre $S \leq \mathbf{O}_{n + 1}(\mathbb{R})$, infatti
\begin{gather}
    \begin{pmatrix}
        A & 0 \\
        0 & 1
...
\end{gather}
dunque se $M \in S$ si ha $M \in \mathbf{O}_{n + 1}(\mathbb{R})$, inoltre $\mathbf{I}_{n + 1} \in S$, $S$ è chiuso rispetto al prodotto di matrici e per ogni $M \in S$ esiste $M^{-1} \in S$ tale che $MM^{-1} = \mathbf{I}_{n + 1}$, infatti $A^{-1} \in \mathbf{O}_n(\mathbb{R})$, e
\begin{gather}
    \begin{pmatrix}
        A & 0 \\
        0 & 1
...
\textbf{2.} Sia
\begin{gather}
    \textit{Aff}_n(\mathbb{R}) := \left\{ \begin{pmatrix}
        A & 0 \\
        v & 1
    \end{pmatrix} : A \in \mathbf{O}_n(\mathbb{R}), v \in \mathbb{R} \right\} \notag
\end{gather}
detto \textit{gruppo affine} di rango $n$. Dimostrare se $\textit{Aff}_n(\mathbb{R}) \leq \mathbf{GL}_{n + 1}(\mathbb{R})$. \\

\textbf{Dimostrazione:} come nell'\textit{esercizio 1}, dimostriamo che esiste un sottogruppo $S$ di $\mathbf{GL}_{n + 1}(\mathbb{R})$ tale che $\textit{Aff}_n(\mathbb{R}) \cong S$. Sia $\mathbf{1}_{n + 1} : \textit{Aff}_n(\mathbb{R}) \longrightarrow S$ la funzione identità. Ovviamente è un omeomorfismo (ponendo $S = \mathbf{1}_{n + 1}(\textit{Aff}_n(\mathbb{R})$), mostriamo ora che $S$ è un sottogruppo di $\mathbf{GL}_{n + 1}(\mathbb{R})$. Si ha che
\begin{gather}
    \det \begin{pmatrix}
        A & 0 \\
        v & 1
    \end{pmatrix} = \det A \neq 0 \text{ per ogni } A \in \mathbf{O}_n(\mathbb{R}) \notag
\end{gather}
dunque $S \subseteq \mathbf{GL}_{n + 1}(\mathbb{R})$. Inoltre $\mathbf{I}_{n + 1} \in S$, perché $\mathbf{I}_n \in \mathbf{O}_n(\mathbb{R})$, e per ogni elemento di $S$ esiste l'inverso in $S$ stesso, infatti
\begin{gather}
    \begin{pmatrix}
        A & 0 \\
        v & 1
...
se e solo se
\begin{gather}
    B = A^t = A^{-1} \in \mathbf{O}_n(\mathbb{R}), \quad w = -vB \in \mathbb{R}^n \notag
\end{gather}
pertanto $S \leq \mathbf{GL}_{n + 1}(\mathbb{R})$. \\ \\
...
\textbf{4.} Dimostrare se $\textit{Aff}_n(\mathbb{R})$ è un gruppo di matrici abeliano. \\
...
\textbf{5.} Dimostrare se $\mathbf{GL}_n(\mathbb{R})$ è compatto. \\

\textbf{Soluzione:} ricordiamo che
\begin{gather}
    \mathbf{GL}_n(\mathbb{R}) = \{ A \in \mathbf{M}_n(\mathbb{R}) : \det A \neq 0 \} \notag
\end{gather}
Sia $k \in \mathbb{N}$, consideriamo la seguente matrice
\begin{gather}
    M(k) = \begin{pmatrix}
        k & 0 & 0 & \dots & 0 \\
        0 & k & 0 & \dots & 0 \\
    ...
    \end{pmatrix} \notag
\end{gather}
per ogni $\rho > 0$ esiste $\widetilde{k} \in \mathbb{N}$ tale che
\begin{gather}
    \widetilde{k} > \rho \implies M(\widetilde{k}) \not\subset D_\rho(0) \notag
\end{gather}
ovvero $\mathbf{GL}_n(\mathbb{R})$ non è limitato, pertanto per il \textit{teorema di Heine-Borel} non può essere compatto. \\ \\
\textbf{6.} Dimostrare se $\textbf{SL}_n(\mathbb{R})$ è compatto in $\mathbf{M}_n(\mathbb{R}$). \\

\textbf{Dimostrazione:} sappiamo che $\mathbf{SL}_n(\mathbb{R})$ è un gruppo di matrici e quindi chiuso in $\mathbf{GL}_n(\mathbb{R})$ e pertanto chiuso anche in $\mathbf{M}_n(\mathbb{R})$. Tuttavia non è compatto, infatti sia $k \in \mathbb{N}$, consideriamo la matrice
\begin{gather} A(k) =
    \begin{pmatrix}
        1 & k & 0 & \dots & 0 \\
        0 & 1 & 0 & \dots & 0 \\
        ...
        . & & & & . \\
        0 & 0 & 0 & \dots & 1
    \end{pmatrix} \in \mathbf{SL}_n(\mathbb{R}) \notag
\end{gather}
per ogni $k \in \mathbb{N}$. Inoltre, analogamente all'esercizio precedente, si ha che per ogni $\rho > 0$ esiste $\widetilde{k} \in \mathbb{N}$ tale che
\begin{gather}
    \sqrt{\sum_{i, j = 1}^n a^2_{ij}(\widetilde{k})} = \sqrt{n + \widetilde{k}^2} > \rho \notag
\end{gather}
infatti basta scegliere
\begin{gather}
    \widetilde{k} > \sqrt{\rho^2 - n} \notag
\end{gather}
e si ha
\begin{gather}
    A(\widetilde{k}) \not\subset D_\rho(0) \notag
\end{gather}
pertanto $\mathbf{SL}_n(\mathbb{R})$ non è limitato e dunque è compatto per il \textit{teorema di Heine-Borel}. \\ \\ \\
\textbf{7.} Sia $G := \left\{ A \in \mathbf{M}_n(\mathbb{R}) : \det A \in \{ 2^k : k \in \mathbb{Z} \} \right\}$, dimostrare che $G$ è un gruppo di matrici. \\

\textbf{Dimostrazione:} poniamo $2^{\mathbb{Z}} := \{ 2^k : k \in \mathbb{Z} \}$, ovviamente $\mathbf{I}_n \in G$ poiché $\det \mathbf{I}_n = 1 = 2^0 \in 2^{\mathbb{Z}}$. Inoltre se $A \in G$ si ha che $A^{-1} \in G$, infatti
\begin{gather}
    \det A^{-1} = \frac{1}{\det A} = \frac{1}{2^{\overline{k}}} \text{ per qualche } \overline{k} \in \mathbb{Z} \notag
\end{gather}
pertanto
\begin{gather}
    \det A^{-1} = 2^{-\overline{k}} \text{ e } -\overline{k} \in \mathbb{Z} \notag
\end{gather}
...
\end{gather}
con $p, q \in \mathbb{Z}$, pertanto anche $p + q \in \mathbb{Z}$ perché $\mathbb{Z}$ è un gruppo rispetto all'addizione. \\ \\
Dunque $G$ è un sottogruppo di $\mathbf{GL}_n(\mathbb{R})$, verifichiamo ora che sia chiuso in $\mathbf{GL}_n(\mathbb{R})$. \\
Si ha che $2^{\mathbb{Z}}$ è non è chiuso in $\mathbb{R}$, infatti $0$ è un punto di accumulazione per $2^{\mathbb{Z}}$ ma $0 \not\in 2^{\mathbb{Z}}$, tuttavia è chiuso in $\mathbb{R} \setminus \{ 0 \}$ che è sufficiente poiché siamo interessati alle matrici invertibili. Si ha che la funzione
\begin{gather}
    \det|_{\mathbf{GL}_n(\mathbb{R})} : \mathbf{GL}_n(\mathbb{R}) \longrightarrow \mathbb{R} \setminus \{ 0 \} \notag
\end{gather}
è continua, pertanto
\begin{gather}
    \mathrm{det}^{-1}(2^{\mathbb{Z}}) = G \notag
\end{gather}
è chiuso in $\mathbf{GL}_n(\mathbb{R})$, infatti $\mathrm{det}^{-1}(\mathbb{R} \setminus \{ 0 \}) = \mathbf{GL}_n(\mathbb{R})$. \\ \\
\textbf{8.} Dimostrare se $Isom(\mathbb{R}^n)$ è un gruppo di matrici. \\

\textbf{Dimostrazione:} sappiamo che
\begin{gather}
    Isom(\mathbb{R}^n) = \left\{ \begin{pmatrix}
        A & 0 \\
        v & 1
    \end{pmatrix} : A \in \mathbf{O}_n(\mathbb{R}), v \in \mathbb{R}^n \right\} \notag
\end{gather}
$\mathbf{I}_{n + 1} \in Isom(\mathbb{R}^n)$ perché $\mathbf{I}_n \in \mathbf{O}_n(\mathbb{R})$. Inoltre
\begin{gather}
    \begin{pmatrix}
        A & 0 \\
        v & 1
    ...
    \mathbf{I}_{n + 1} \notag
\end{gather}
se e solo se $B = A^{-1} \text{ e } u = -vB$ e poiché $A^{-1} \in \mathbf{O}_n(\mathbb{R})$ e $-vB \in \mathbb{R}^n$ si ha che $Isom(\mathbb{R}^n)$ contiene l'inverso di ogni suo elemento. Notiamo anche che $Isom(\mathbb{R}^n)$ è chiuso rispetto al prodotto di matrici. Dunque è un gruppo. \\ \\
Dimostriamo ora che è chiuso in $\mathbf{GL}_{n + 1}(\mathbb{R})$. Consideriamo la funzione \\
$f : Isom(\mathbb{R}^n) \longrightarrow \mathbf{M}_{n + 1}(\mathbb{R})$ definita ponendo
\begin{gather}
    f \begin{pmatrix}
        A & 0 \\
        v & 1
...
\begin{gather}
    f^{-1}(\{ \mathbf{I}_{n + 1} \} = Isom(\mathbb{R}^n) \notag
\end{gather}
ed essendo $\{ \mathbf{I}_{n + 1} \}$ chiuso in $\mathbf{M}_{n + 1}(\mathbb{R})$, poiché è uno spazio $T_1$, si ha che anche $Isom(\mathbb{R}^n)$ è chiuso in $\mathbf{M}_{n + 1}(\mathbb{R})$, inoltre, poiché $\mathbf{GL}_{n + 1} \cap Isom(\mathbb{R}^n) = Isom(\mathbb{R}^n)$ è chiuso nella topologia relativa e quindi è un gruppo di matrici.



\newpage
\subsection{Algebre di Lie}
\textbf{Definizione:} sia $X \subseteq \mathbb{R}^n$ e sia $p \in X$. Lo \textit{spazio tangente} ad $X$ in $p$ è
\begin{gather}
...
\textbf{Definizione:} sia $G$ un gruppo di matrici, l'\textit{algebra di Lie} di $G$ è
\begin{gather}
...
\end{gather} \\ \\
\textbf{Proposizione 2.4.1:} siano $\beta, \gamma : (-\varepsilon, \varepsilon) \longrightarrow \mathbf{M}_n(\mathbb{R})$ differenziabili, definiamo
\begin{gather}
    \beta\gamma : (-\varepsilon, \varepsilon) \longrightarrow \mathbf{M}_n(\mathbb{R}) \notag
\end{gather}
ponendo
\begin{gather}
...
\textbf{Dimostrazione:} sia $A \in \mathfrak{g}(G)$ e sia $\lambda \in \mathbb{R}$, per ipotesi esiste $\gamma$ differenziabile tale che $\gamma'(0) = A$, consideriamo il cammino
\begin{gather}
    \widetilde{\gamma} : \left( -\frac{\varepsilon}{|\lambda|}, \frac{\varepsilon}{|\lambda|} \right) \longrightarrow G, \quad \widetilde{\gamma}(t) = \gamma(\lambda t) \notag
\end{gather}
per ogni $t \in (\varepsilon, \varepsilon)$. $\widetilde{\gamma}$ è differenziabile, $\widetilde{\gamma}(0) = \mathbf{I}_n$ e inoltre
\begin{gather}
    \widetilde{\gamma}'(0) = \lambda \cdot \gamma'(0) = \lambda \cdot A \notag
\end{gather}
...
\textbf{Definizione:} sia $G$ un gruppo di matrici. La dimensione di $G$ è $\dim G = \dim \mathfrak{g}(G)$ come spazio vettoriale reale. \\ \\
Poniamo $\mathfrak{gl}_n(\mathbb{R}) = \mathfrak{g}(\mathbf{GL}_n(\mathbb{R}))$, $\mathfrak{sl}_n(\mathbb{R}) = \mathfrak{g}(\mathbf{SL}_n(\mathbb{R}))$, $\mathfrak{so}_n(\mathbb{R}) = \mathfrak{g}(\mathbf{SO}_n(\mathbb{R}))$. \\ \\
\textbf{Proposizione 2.4.3:} sia $n \in \mathbb{N}$, si ha che $\mathfrak{gl}_n(\mathbb{R}) = \mathbf{M}_n(\mathbb{R})$. \\

\textbf{Dimostrazione:} ovviamente $\mathfrak{gl}_n(\mathbb{R}) \subset \mathbf{M}_n(\mathbb{R})$. Sia $A \in \mathbf{M}_n(\mathbb{R})$, consideriamo il cammino
\begin{gather}
    \gamma : \mathbb{R} \longrightarrow \mathbf{M}_n(\mathbb{R}), \quad \gamma(t) = \mathbf{I}_n + tA \notag
\end{gather}
per ogni $A \in \mathbf{M}_n(\mathbb{R})$. Chiaramente $\gamma$ è differenziabile poiché è un polinomio,  $\gamma(0) = \mathbf{I}_n$ e $\gamma'(0) = A$. \\
Inoltre la funzione $\det$ è continua dunque, per il teorema della permanenza del segno, esiste $\varepsilon > 0$ tale che $\det \gamma(t) > 0$ per ogni $t \in (-\varepsilon, \varepsilon)$, pertanto la tesi segue e $\dim \mathbf{GL}_n(\\mathbb{R}) = n^2$. \qed \\ \\
\textit{Lemma 2.3.1:} sia $\gamma : (-\varepsilon, \varepsilon) \longrightarrow \mathbf{M}_n(\mathbb{R})$ differenziabile tale che $\gamma(0) = \mathbf{I}_n$, si ha che
\begin{gather}
...
\textbf{Teorema 2.4.2:} sia $n \in \mathbb{N}$, si ha che
\begin{gather}
    \mathfrak{sl}_n(\mathbb{R}) = \{ A \in \mathbf{M}_n(\mathbb{R}) : \mathrm{tr} A = 0 \} \notag
\end{gather}
e in particolare $\dim \mathbf{SL}_n(\mathbb{R}) = n^2 - 1$. \\

\textbf{Dimostrazione:} sia $A \in \mathfrak{sl}_n(\mathbb{R})$, per ipotesi esiste $\gamma$ differenziabile tale che $\gamma(0) = \mathbf{I}_n$ e $\gamma'(0) = A$. Inoltre deve soddisfare $\gamma(t) \in \mathbf{SL}_n(\mathbb{R})$ per ogni $t \in (-\varepsilon, \varepsilon)$, pertanto
\begin{gather}
    \frac{\mathrm{d}}{\mathrm{d}t} \det \gamma(t) = \mathrm{tr} (\gamma'(0)) = 0 \notag
\end{gather}
imponendo $\det \gamma(t) = 1$ per ogni $t \in (-\varepsilon, \varepsilon)$. \\ \\
Viceversa, sia $\alpha : (-\varepsilon, \varepsilon) \longrightarrow \mathbf{SL}_n(\mathbb{R})$ definita ponendo
\begin{gather}
    ...
    \end{pmatrix} \notag
\end{gather}
si ha che $\alpha$ è differenziabile (per teoremi generali sulle funzioni scalari), \\$\alpha(t) \in \mathbf{SL}_n(\mathbb{R})$ per ogni $t \in (-\varepsilon, \varepsilon)$, inoltre $\alpha(0) = \mathbf{I}_n$ e
\begin{gather}
...
\textbf{Proposizione 2.4.4:} sia $A \in \mathbf{M}_n(\mathbb{R})$, si ha che le seguenti sono equivalenti: \\
\textbf{1.} $A + A^t = 0$. \\
\textbf{2.} $\langle x, yA \rangle = -\langle xA, y \rangle$ per ogni $x, y \in \mathbb{R}^n$. \\
\textbf{3.} $\langle x, xA \rangle = 0$ per ogni $x \in \mathbb{R}^n$. \\
...
\begin{gather}
    \mathfrak{so}_n(\mathbb{R}) = \{ A \in \mathbf{M}_n(\mathbb{R}) : A + A^t = 0 \} \notag
\end{gather}
ovvero l'algebra di Lie del gruppo speciale ortogonale è l'insieme della matrici antisimmetriche a coefficienti reali. \\

\textbf{Dimostrazione:} $(\rightarrow)$ sia $X \in \mathfrak{so}_n(\mathbb{R})$, per ipotesi esiste $\gamma$ differenziabile tale che $\gamma(0) = \mathbf{I}_n$, $\gamma'(0) = X$ e $\gamma(t) \in \mathbf{SO}_n(\mathbb{R})$ per ogni $t \in (-\varepsilon, \varepsilon)$. \\
Prima di tutto, poiché $\mathbf{SO}_n(\mathbb{R}) \leq \mathbf{O}_n(\mathbb{R})$, deve valere che $\gamma(t) \cdot \gamma(t)^t = \mathbf{I}_n$ per ogni $t \in (-\varepsilon, \varepsilon)$, dunque
\begin{gather}
...
\end{gather}
quindi $\mathfrak{so}_n(\mathbb{R}) \subset \{ A \in \mathbf{M}_n(\mathbb{R}) : A + A^t = 0 \}$. \\ \\
$(\leftarrow)$ Viceversa sia $X \in \mathbf{M}_n(\mathbb{R})$ tale che $X + X^t = 0$, dimostriamo che $X$ appartiene all'algebra di Lie del gruppo speciale ortogonale, ovvero dimostriamo che esiste un cammino differenziabile $\gamma$ tale che $\gamma(0) = \mathbf{I}_n$, $\gamma'(0) = X$ e $\gamma(t) \in \mathbf{SO}_n(\mathbb{R})$ per ogni $t \in (-\varepsilon, \varepsilon)$. \\
Consideriamo il cammino $\gamma : (-\varepsilon, \varepsilon) \longrightarrow \mathbf{SO}_n(\mathbb{R})$, definito ponendo
\begin{gather}
...
\begin{gather}
    \dim \mathbf{SO}_n(\mathbb{R}) = \dim \mathbf{O}_n(\mathbb{R}) = \begin{pmatrix}
        n \\
        2
    \end{pmatrix} \notag
\end{gather}
...
\textbf{4.} si scrive $\mathbf{U}_n$ e $\mathbf{SU}_n$ al posto di $\mathbf{O}_n(\mathbb{C})$ e $\mathbf{SO}_n(\mathbb{C})$, e sono detti \textit{gruppo unitario} e \textit{gruppo speciale unitario}.

\subsubsection{Esercizi}
\textbf{1.} Determinare: 
\begin{equation}
    \mathfrak{g}(Isom\, \mathbb{R}^n) := \{ \gamma ' (0) : \gamma : (- \varepsilon, \varepsilon) \longrightarrow Isom\, \mathbb{R}^n, \, \gamma \text{ diff.}, \, \gamma(0) = \mathbf{I}_n \} \notag
\end{equation} 
dove $\mathbf{I}_n$ è la matrice identità di $\mathbf{M}_n(\mathbb{R})$. \\

\textbf{Soluzione:} sia $\gamma : (- \varepsilon, \varepsilon) \longrightarrow Isom\, \mathbb{R}^n$ definita ponendo:
\begin{equation} \gamma (t) =
    ...
    \end{pmatrix} \notag
\end{equation}
dove $X \in \mathbf{M}_n$ e $v(t) \in C^1((-\varepsilon, \varepsilon);\mathbb{R}^n)$ tale che $v(0) = \vec{0}$. Per come è stato definito $Isom\, \mathbb{R}^n$ deve valere che $e^{tX} \in \mathbf{O}_n(\mathbb{R})$ per ogni $t \in (-\varepsilon, \varepsilon)$, cioè è sufficiente che:
\begin{gather}
...
\begin{equation}
    \mathfrak{g}(Isom\, \mathbb{R}^n) = \left\{
    \begin{pmatrix}
        X & 0 \\
        v'(0) & 0
    \end{pmatrix} : X + X^t = 0, \ v \in C^1((-\varepsilon, \varepsilon); \mathbb{R}^n), \ v(0) = 0\right\} \notag
\end{equation}
\\ \\
\textbf{2.} sia $G := \{ A \in \mathbf{M}_n : \det A \in 2^{\mathbb{Z}}\}$, determinare $\mathfrak{g}(G)$. \\

\textbf{Soluzione:} si ha che intorno a $\mathbf{I}_n$ il gruppo di matrici $G$ è localmente identico a $\mathbf{SL}_n(\mathbb{R})$ e poiché lo spazio tangente è univocamente determinato da caratteristiche locali segue che $\mathfrak{g}(G) = \mathfrak{sl}_n(\mathbb{R})$.

\newpage
\section{Tensori}
\subsection{Quoziente di spazi vettoriali}
\textbf{Definizione:} sia $V$ un $\mathbb{K}$-spazio vettoriale e sia $W \leq V$ (sottospazio vettoriale) lo \textit{spazio vettoriale quoziente} è
\begin{gather}
    V/W := \{ W + v : v \in V \} \notag
\end{gather}
...
\textbf{Definizione:} sia $\{ V_i : i \in I \}$ una famiglia di spazi vettoriali reali di dimensione finita. La \textit{somma diretta} di $\{ V_i : i \in I \}$ è lo spazio vettoriale
\begin{gather}
...
\end{gather}
per ogni $(x_i)_{i \in I}, (y_i)_{i \in I} \in \bigoplus_{i \in I} V_i$ e ogni $\lambda \in \mathbb{R}$. \\ \\
...
\textbf{Definizione:} sia $\{ V_i : i \in I \}$ una famiglia di spazi vettoriali reali di dimensione finita. Il \textit{prodotto diretto} della famiglia $\{ V_i : i \in I \}$ è lo spazio vettoriale
\begin{gather}
    \prod_{i \in I} V_i \notag
\end{gather}
...
\textbf{Dimostrazione:} immediata. \qed \\ \\
\textbf{Definizione:} sia $S$ un insieme, lo \textit{spazio vettoriale di base} $S$ è
\begin{gather}
    \mathbb{R}^S := \bigoplus_{s \in S} \mathbb{R} \notag
\end{gather}
per identificare un elemento di $\mathbb{R}^S$ si usa la notazione di somma formale, ovvero
\begin{gather}
...
\textbf{Definizione:} siano $V$ e $W$ spazi vettoriali reali, il prodotto tensoriale di $V$ e $W$ è lo spazio vettoriale quoziente
\begin{gather}
    V \otimes W = \frac{\mathbb{R}^{V \times W}}{D} \notag
\end{gather}
...
\begin{gather}
    ...
    a \cdot (x, y) - (x, ay) \notag
\end{gather}
per ogni $x, x_1, x_2 \in V$, ogni $y, y_1, y_2 \in W$ e ogni $a \in \mathbb{R}$. \\ \\
...
Siano $x \in V$ e $y \in W$ si scrive $x \otimes y = \pi ((x, y))$, dunque $\pi : \mathbb{R}^{V \times W} \longrightarrow V \otimes W$. \\
Per quanto visto in precedenza $(x, y)$ è un elemento di
\begin{gather}
    \mathbb{R}^{V \times W} = \bigoplus_{(v, w) \in V \times W} \mathbb{R} \notag
\end{gather}
...
\end{gather}
cioè $\otimes : \mathbb{R}^{V \times W} \longrightarrow V \otimes W$ è una forma bilineare. \\ \\
...
\begin{gather}
    \widetilde{f} : \mathbb{R}^{V \times W} \longrightarrow U \notag
\end{gather}
definita ponendo
\begin{gather}
    \widetilde{f}(\sum_{(x, y) \in V \times W} a_{x, y} \cdot (x, y)) = \sum_{(x, y) \in V \times W} a_{x, y} \cdot f(x, y) \notag
\end{gather}
si ha che, per come è stata costruita, $\widetilde{f}$ è lineare. Inoltre $\widetilde{f}(D) = 0$ proprio per la linearità di $\widetilde{f}$. \\ \\
Poniamo $h : V \otimes W \longrightarrow U$ definita ponendo
\begin{gather}
    h(D + \sum_{(x, y) \in V \times W} a_{x, y} \cdot (x, y)) = \widetilde{f}(\sum_{(x, y) \in V \times W} a_{x, y} \cdot (x, y)) \notag
\end{gather}
per ogni $\sum_{(x, y) \in V \times W} a_{x, y} \cdot (x, y) \in \mathbb{R}^{V \times W}$. Notiamo che $D + \alpha = D + \beta$ se e solo se $\alpha - \beta \in D$, pertanto si ottiene
\begin{gather}
    h(D + (\alpha - \beta)) = \widetilde{f}(\alpha - \beta) = \widetilde{f}(\alpha) - \widetilde{f}(\beta) = 0 \notag
\end{gather}
quindi $\widetilde{f}(\alpha) = \widetilde{f}(\beta)$ e $h$ è lineare. Inoltre, se $(x, y) \in V \times W$ si ha
\begin{gather}
    (h \cdot g)(x, y) = h(g(x, y)) = h(x \otimes y) = \widetilde{f}((x, y)) = f(x, y) \notag
\end{gather}
pertanto $h \cdot g = f$. \\ \\
...
\textbf{4.} $V \otimes \mathbb{R} \cong V$. \\ \\
...
\begin{gather}
    V = \bigoplus_{i = 1}^n \mathbb{R} \notag \\
    W = \bigoplus_{j = 1}^m \mathbb{R} \notag
\end{gather}
pertanto, per i punti $(3)$ e $(4)$ del \textit{corollario 3.3.1}
\begin{gather}
    V \otimes W \cong \bigoplus_i \bigoplus_j (\mathbb{R} \otimes \mathbb{R}) \cong \bigoplus_i \bigoplus_j \mathbb{R} \notag
\end{gather}
...
\textbf{3.} Sia $V$ uno spazio vettoriale, dimostrare che $V \otimes \mathbb{R} \cong V$. \\

\textbf{Dimostrazione:} sfruttiamo la \textit{proprietà universale del prodotto tensoriale}, consideriamo il seguente diagramma \\
\[
\begin{tikzcd}
&V \times \mathbb{R} \arrow[r,"f"] \arrow[d,"g", swap] & V \\
&V \otimes \mathbb{R} \arrow[ur, "\Phi", swap] 
\end{tikzcd}
\]
dunque esibiamo una funzione $f : V \times \mathbb{R} \longrightarrow V$ bilineare e dimostriamo che $\Phi$ è un isomorfismo. Come scelta naturale per $f$ si ha
\begin{gather}
    f(v, \lambda) := \lambda \cdot v \notag
\end{gather}
Per ogni $(v, \lambda) \in V \times \mathbb{R}$. Per ipotesi sappiamo che esiste unico $\Phi : V \otimes \mathbb{R} \longrightarrow V$ lineare tale che
\begin{gather}
    \Phi(v \otimes \lambda) = \lambda \cdot v = f(v, \lambda) \notag
\end{gather}
per ogni $(v, \lambda) \in V \times \mathbb{R}$. Esibiamo l'inversa di $\Phi$, consideriamo la funzione $\Psi : V \longrightarrow V \otimes \mathbb{R}$ definita ponendo
\begin{gather}
    \Psi (v) = v \otimes 1 \notag
\end{gather}
allora si ottiene
...
\end{gather}
dunque $\Phi$ è un isomorfismo e $V \times \mathbb{R} \cong V$. \\ \\
...
\newpage
\subsection{Tensori}
\textbf{Definizione:} sia $V$ uno spazio vettoriale, il \textit{duale} di $V$ è lo spazio vettoriale
\begin{gather}
    V^* := \{ f : V \longrightarrow \mathbb{R} : f \text{ lineare} \} \notag
\end{gather}
...
\textbf{Definizione:} siano $V$ uno spazio vettoriale e $p, q$ interi positivi, un \textit{tensore} di tipo $(p, q)$ è un elemento
\begin{gather}
...
\textbf{Definizione:} sia $X$ uno spazio topologico, $X$ è detto \textit{varietà topologica} se: \\
\textbf{1.} $X$ è $T_2$. \\
\textbf{2.} $X$ soddisfa il secondo assioma di numerabilità. \\
\textbf{3.} per ogni $p \in X$ esiste un intorno aperto $U$ di $p$ tale che $U$ è omeomorfo a $\{ \mathbf{x} \in \mathbb{R}^n : \| \mathbf{x} \| < 1 \}$ per qualche $n$ intero positivo, in tal caso si pone $\dim (p) = n$. \\ \\
...
\textbf{Dimostrazione:} la funzione $\dim : X \longrightarrow \mathbb{R}$ è continua (per come è definita), pertanto $\dim (X) \subset \mathbb{R}$ è connesso, segue che $\dim(X) = \{ n \}$. \\ \\
...
\textbf{Definizione:} sia $X$ uno spazio topologico, $X$ è detto \textit{varietà differenziabile} se esiste $\{ U_\mu, f_\mu \}_{\mu \in M}$ tale che: \\
\textbf{1.} $\{ U_\mu : \mu \in M \}$ è un ricoprimento aperto di $X$ per ogni $\mu \in M$. \\
\textbf{2.} per ogni $\mu \in M$ esiste $V_\mu$ aperto di $\mathbb{R}^n$ tale che $f_\mu : U_\mu \longrightarrow V_\mu$ è un omeomorfismo. \\
...
Sia $f : \mathbb{R}^n \longrightarrow \mathbb{R}$ differenziabile, $\Gamma_f$ è una varietà differenziabile con atlante $(X, p)$ dove $X = \Gamma_f$ e $p$ è definita ponendo
\begin{gather}
    p(x_1, \dots, x_{n + 1}) = (x_1, \dots, x_n) \notag
\end{gather} 
per ogni $(x_1, \dots, x_{n + 1}) \in X$. \\ \\
Sia $\mathbf{S}^2 = \{ \mathbf{x} \in \mathbb{R}^3 : \| \mathbf{x} \| < 1 \}$, si ha che $\mathbf{S}^2$ è una varietà differenziabile con atlante $\{ U_{ijk}, f_{ijk} \}$, dove
\begin{gather}
...
\end{gather}
è differenziabile in $\{ \mathbf{x} \in \mathbb{R}^3 : x^2_1 + x^2_2 < 1 \}$. Ragionando analogamente si dimostra la differenziabilità per tutte le composizioni di funzioni. \\ \\
\textbf{Definizione:} sia $X$ una varietà differenziabile con atlante $\{ U_\mu, f_\mu \}_{\mu \in M}$, si ha che $f : X \longrightarrow \mathbb{R}$ è differenziabile se $f \cdot f^{-1}_\mu$ è differenziabile per ogni $\mu \in M$. \\ \\
...
Per $i = 1, \dots, n$ sia $x_i : \mathbb{R}^n \longrightarrow \mathbb{R}$ definita ponendo
\begin{gather}
    x_i(y_1, \dots, y_n) = y_i \notag
\end{gather}
per ogni $(y_1, \dots, y_n) \in \mathbb{R}^n$. Sia $p \in X$ e sia $\mu \in M$ tale che $p \in U_\mu$, si ha che
\begin{gather}
...
\textbf{Definizione:} siano $X, Y$ varietà differenziabili, sia $f : X \longrightarrow Y$, $f$ è detto \textit{diffeomorfismo} se $f$ e $f^{-1}$ sono omeomorfismi differenziabili.
...
\end{gather}
si ha che $\{ U_i : i = 1, \dots, n \}$ è un ricoprimento aperto di $\mathbf{P}(V)$. Inoltre per ogni $i = 1, \dots, n$ consideriamo la funzione $f_i : U_i \longrightarrow \mathbb{R}^{n - 1}$ definita ponendo
\begin{gather}
...
Sia $X = \mathbb{R}^n$, per qualche $n$ intero positivo, e sia $p \in X$, ogni vettore $\mathbf{v}$ di $\mathbb{R}^n$ induce un'operazione sulle funzioni differenziabili $f$ definite in un intorno aperto di $p$ detta derivata direzionale rispetto a $\mathbf{v}$, data da
\begin{gather}
...
Nel caso in cui sia possibile determinare una parametrizzazione $\Phi : \mathbb{R}^n \longrightarrow M$ per una determinata varietà differenziabile $M$ si ha che lo spazio tangente a $M$ nel punto $x \in M$ è lo spazio generato dall'insieme
\begin{gather}
...
\textbf{Definizione:} sia $X$ una varietà differenziabili, sia $p \in X$, lo \textit{spazio tangente} a $p$ in $X$ è
\begin{gather}
    T_pX = \{ L : C^{\infty}(X) \longrightarrow \mathbb{R} : L \text{ lineare }, L(fg) = L(f) \cdot g + L(g) \cdot f \} \notag
\end{gather}
...
\end{gather}
per ogni $f \in C^{\infty}(X)$, ogni $L, L_1, L_2 \in T_pX$ e ogni $\lambda \in \mathbb{R}$, si ottiene che $T_pX$ è uno spazio vettoriale. \\ \\
\textbf{Teorema 4.2.1:} siano $X$ una varietà differenziabile e $p \in X$, si ha che $\dim T_pX = \dim X$. \\

\textbf{Dimostrazione:} sia $\{ (U_\mu, \varphi_\mu) \}_{\mu \in M}$ un atlante di $X$, sia $\mu \in M$ tale che $p \in U_\mu$ e sia $n$ intero positivo tale che $U_\mu \cong V_\mu$ dove $V_\mu$ aperto di $\mathbb{R}^n$. Consideriamo una base $\{ e_i : i = 1, \dots, n \}$ di $\mathbb{R}^n$ e per ogni $i = 1, \dots, n$ e ogni $f \in C^{\infty}(X; \mathbb{R})$ poniamo
\begin{gather}
    L_i(f) = \frac{\partial}{ \partial x_i}(f \cdot \varphi^{-1}_\mu)|_{\varphi_\mu(p)} \notag
\end{gather}
ovviamente $L_i$ è lineare e soddisfa la proprietà di Leibniz per ogni $i = 1, \dots, n$, pertanto $L_i \in T_p X$ per ogni $i = 1, \dots, n$. Notiamo che $f \cdot \varphi^{-1}_\mu : V_\mu \longrightarrow \mathbb{R}$, dobbiamo dimostrare che $\{ L_i : i = 1, \dots, n \}$ è un insieme linearmente indipendente di generatori di $T_p X$, ovvero che ogni $L \in T_p X$ può essere scritto come combinazione lineare degli $L_i$ e che
\begin{gather}
...
\end{gather}
per ogni $f \in C^{\infty}(X; \mathbb{R})$ se e solo se $\alpha_i = 0$ per ogni $i = 1, \dots, n$. Sia $L \in T_p X$, poiché $L$ ha come dominio lo spazio delle funzioni di classe $C^{\infty}$ si ha che, per intorni abbastanza piccoli di $\varphi_\mu(p)$, posso scrivere una funzione $f \cdot \varphi^{-1}_\mu$, dove $f \in C^{\infty}(X; \mathbb{R})$, come la sua espansione al primo ordine, ovvero posso applicare $L$ all'espansione al primo ordine di $f$, pertanto
\begin{gather}
...
\textbf{Definizione:} sia $f \in C^{\infty}(X; Y)$, il \textit{differenziale} di $f$ in $p$ è la funzione lineare
\begin{gather}
...
Introdurremo ora i concetti di \textit{forma esterna}, di \textit{prodotto esterno}, di \textit{forma differenziale}, di \textit{derivata esterna} e definiremo l'integrazione delle forme differenziali.
...
\begin{flalign}
    \omega(x_1 + x_2) = \omega(x_1) + \omega(x_2) \qquad \omega(\lambda x) = \lambda\omega(x) \tag{5.1.1}
\end{flalign}
...
\begin{flalign}
    (\omega_1 + \omega_2)(x) = \omega_1(x) + \omega_2(x) \qquad (\lambda\omega)(x) = \lambda\omega(x) \tag{5.1.2}
\end{flalign}
...
\begin{flalign}
    \omega = b_1dx_1 + \dots b_ndx_n \tag{5.1.4}
\end{flalign}
...
\begin{flalign}
    \omega^{(2)}(\lambda_1x_1 + \lambda_2x_2, x) = \lambda_1\omega^{(2)}(x_1, x) + \lambda_2\omega^{(2)}(x_2, x) \tag{5.1.6}
\end{flalign}
e
\begin{flalign}
    \omega^{(2)}(x_1, x_2) = -\omega^{(2)}(x_2, x_1) \tag{5.1.7} 
\end{flalign}
...
\begin{flalign}
    \omega^{(2)}(x, x) = -\omega^{(2)}(x, x) \tag{5.1.10}
\end{flalign}
...
\textbf{Proposizione 5.1.1:} lo spazio duale $(\mathbb{R}^n \times \mathbb{R}^n)^*$ ha dimensione $\frac{n(n - 1)}{2}$. \\
...
\begin{flalign}
    \omega^{(2)}(x, y) = \sum_{1 \leq i < j \leq n}\omega_{ij}dx_{ij}(x, y) \tag{5.1.13}
\end{flalign}
Dall'arbitrarietà della coppia $(x, y)$ segue che 
\begin{flalign}
    \omega^{(2)} = \sum_{1 \leq i < j \leq n} \omega_{ij}dx_{ij} \tag{5.1.14}
\end{flalign}
Inoltre 
\begin{flalign}
    \sum_{1 \leq i < j \leq n} 1 = \frac{n(n - 1)}{2} \tag{5.1.15}
\end{flalign}
...
\begin{flalign}
    \omega(\lambda x + \mu y, z_2, \dots, z_k) = \lambda\omega(x, z_2, \dots, z_k) + \mu\omega(y, z_2, \dots, z_k) \tag{5.1.16}
\end{flalign}
e
\begin{flalign}
    \omega(z_{i_1}, \dots, z_{i_k}) = (-1)^\nu\omega(z_1, \dots, z_k) \tag{5.1.17}
\end{flalign}
dove 
\begin{flalign}
    \nu = 
...
\begin{flalign}
    ...
    \end{pmatrix}
    \notag \\
    &= -(\omega_1 \wedge \omega_2)(x_1, x_2) \tag{5.1.2}
\end{flalign}
e
\begin{flalign}
    ...
    \end{pmatrix}
    \notag \\
    &= \lambda_1(\omega_1 \wedge \omega_2)(x_1, x) + \lambda_2(\omega_1 \wedge \omega_2)(x_2, x) \tag{5.2.3}
\end{flalign}
...
\begin{flalign}
    dx_i \wedge dx_i = 0 \qquad dx_i \wedge dx_j = -dx_j \wedge dx_j \tag{5.2.4}
\end{flalign}
...
\textbf{Proposizione 5.2.1:} ogni $2$-forma $\omega^{(2)}$ può essere rappresentata univocamente come combinazione lineare di $\{dx_i \wedge dx_j\}_{i < j}$. \\

\textbf{Dimostrazione:} notiamo che 
\begin{flalign}
    (dx_i \wedge dx_j)(e_k, e_k) = dx_{ij}(e_k, e_l) \tag{5.2.5}
\end{flalign}
per ogni $1 \leq i < j \leq n$ e ogni $k, l = 1, \dots, n$, dove $dx_{ij}$ è la $2$-forma definita nella \textit{proposizione 5.1.1}. La tesi segue. \qed
...
\begin{flalign}
    dx_{i_1} \wedge \dots \wedge dx_{i_k} \tag{5.3.2}
\end{flalign}
...
\begin{flalign}
    ...
    \end{pmatrix} = 0 \tag{5.3.3}
\end{flalign}
dato che si tratta del determinante di una matrice con due colonne identiche. \\ \\
\textbf{Proposizione 5.3.1:} ogni $k$-forma in $\mathbb{R}^n$ può essere rappresentata univocamente come combinazione lineare di $\{dx_{i_1} \wedge \dots \wedge dx_{i_k}\}_{i_1 < \dots < i_k}$. \\
...
\begin{flalign}
    ...
    &= \sum_{i_1 < \dots < i_k} \omega_{i_1, \dots, i_k}(dx_{i_1} \wedge \dots \wedge dx_{i_k})(x_1, \dots, x_k) \tag{5.3.4}
\end{flalign}
Dall'arbitrarietà di $x_1, \dots, x_k$ si ottiene 
\begin{flalign}
    \omega^{(k)} = \sum_{i_1 < \dots < i_k} \omega_{i_1, \dots, i_k}dx_{i_1} \wedge \dots \wedge dx_{i_k} \tag{5.3.5}
\end{flalign}
...
\begin{flalign}
    \omega^{(n)} = adx_1 \wedge \dots \wedge dx_n \tag{5.3.6}
\end{flalign}
con $a \in \mathbb{R}$. \\ \\
...
\begin{flalign}
    (\omega_1 \wedge \dots \wedge \omega_k) \wedge (\omega_{k + 1} \wedge \dots \wedge \omega_{k + l}) = \omega_1 \wedge \dots \wedge \omega_{k + l} \tag{5.3.7}
\end{flalign} \\
\textbf{Proposizione 5.3.2:} il prodotto esterno di monomi è associativo e anticommutativo graduato. \\
...
\textbf{Proposizione 5.4.1:} il prodotto $\omega^{(k)} \wedge \omega^{(l)}$ è una $k + l$-forma. \\
...
\textbf{Dimostrazione:} per la \textit{proposizione 5.3.1} sappiamo che $\omega^{(k)}$ e $\omega^{(l)}$ possono essere rappresentate come combinazione lineare dei monomi di base. Inoltre, per la \textit{proposizione 5.3.2}, sappiamo che il prodotto di monomi è anticommutativo graduato. Pertanto, concludiamo che $\omega^{(k)} \wedge \omega^{(l)}$ può essere rappresentato come somma di prodotti di monomi, ognuno anticommutativo graduato. \\ \\
La distributività si vede immediatamente notando che la mappa
\begin{flalign}
    (\omega^{(k)}, \omega^{(l)}) \mapsto \omega^{(k)} \wedge \omega^{(l)} \notag
\end{flalign}
è bilineare, infatti ogni termine della $(5.4.1)$ è lineare rispetto a $\omega^{(k)}$ e $\omega^{(l)}$. \\ \\
...
implica
\begin{flalign}
    ((\omega'_1 + \omega''_1) \wedge \omega_2) \wedge \omega_3 = (\omega'_1 + \omega''_1) \wedge (\omega_2 \wedge \omega_3) \tag{5.4.3}
\end{flalign}
...
\begin{flalign}
    ...
    \end{cases} \tag{5.4.4}
\end{flalign}
Per la \textit{proposizione 5.3.1} sappiamo che ogni forma è somma di monomi, pertanto è sufficiente dimostrare l'associatività per i monomi. \\
Per ora indicheremo il prodotto esterno tra monomi con il simbolo $*$, dato che ancora non è stata verificata l'uguaglianza con al definizione $(5.4.1)$. \\
Mostriamo che il prodotto esterno tra monomi è il monomio 
\begin{flalign}
    (\omega_1 * \dots * \omega_k) \wedge (\omega_{k + 1} * \dots * \omega_{k + l}) = \omega_1 * \dots * \omega_{k + l} \tag{5.4.5}
\end{flalign}
Valutiamo le espressioni dei due membri della $(5.4.5)$ in $(x_1, \dots, x_{k + l})$. Il membro di destra è uguale al determinante $\det\{\omega_i(x_j)\}$. Il membro di sinistra è uguale a 
\begin{flalign}
    \sum_{\substack{i_1 < \dots < i_k \\ j_1 < \dots < j_l}} (\pm) \det\{\omega_\nu(x_{i_m})\} \cdot \det\{\omega_\mu(x_{j_m})\} \tag{5.4.6}
\end{flalign}
dove $1 \leq \nu \leq k$ e $k + 1 \leq \mu \leq k + l$. Per il \textit{teorema di Laplace}, il termine $\det\{\omega_i(x_j)\}$ è uguale alla $(5.4.6)$, scegliendo opportunamente i segni di ogni termine. \\ \\
Concludiamo che le operazioni $*$ e $\wedge$ coincidono. Inoltre, per quanto sopra osservato, dall'associatività del prodotto di $1$-forme segue l'associatività per il prodotto definito in $(5.4.1)$ e quindi si ottiene la tesi. \qed \\ \\
\textbf{Osservazione:} l'anticommutatività graduata comporta $\omega^{(k)} \wedge \omega^{(k)} = 0$ se $k$ è dispari. Infatti
\begin{flalign}
    \omega^{(k)} \wedge \omega^{(k)} = (-1)^{k^2}\omega^{(k)} \wedge \omega^{(k)} = -\omega^{(k)} \wedge \omega^{(k)} \tag{5.4.7}
\end{flalign}
...
\begin{flalign}
    \omega^{(2)} = \sum_{i = 1}^n dp_i \wedge dq_i \tag{5.4.8}
\end{flalign}
...
\begin{flalign}
    ...
    &= -2\sum_{i < j} dp_i \wedge dp_j \wedge dq_i \wedge dq_j \tag{5.4.9}
\end{flalign}
...
\begin{flalign}
    ...
    &= (-1)^{k - 1}k!\sum_{i_1 < \dots < i_k} dp_{i_1} \wedge \dots \wedge dp_{i_k} \wedge dq_{i_1} \wedge \dots \wedge dq_{i_k} \tag{5.4.10}
\end{flalign}
...
\begin{flalign}
    \omega^{(1)}_v(x) = (v, x) \tag{5.4.11}
\end{flalign}
e la $2$-forma $\omega^{(2)}_v$ definita da
\begin{flalign}
    \omega^{(2)}_v(x_1, x_2) = v \cdot (x_1 \times x_2) \tag{5.4.12}
\end{flalign}
per ogni $x, x_1, x_2 \in \mathbb{R}^3$. Dimostriamo che le mappe 
\begin{flalign}
    v \in \mathbb{R}^3 \mapsto \omega^{(1)}_v \in \Omega^{(1)}(\mathbb{R}^3) \tag{5.4.13} \\
    v \in \mathbb{R}^3 \mapsto \omega^{(2)}_v \in \Omega^{(2)}(\mathbb{R}^3) \tag{5.4.14}
\end{flalign}
sono isomorfismi di spazi lineari. \\

\textbf{Soluzione:} si vede immediatamente che le mappe definite in $(5.4.13)$ e $(5.4.14)$ sono lineari, pertanto preservano la struttura di spazio lineare, ovvero sono omomorfismi. \\
...
\begin{flalign}
    ...
    &= (u - v, x) = 0 \notag
\end{flalign}
per ogni $x \in \mathbb{R}^3$ se e solo se $u = v$. Pertanto la mappa definita in $(5.4.13)$ è iniettiva. Identicamente si dimostra che $v \mapsto \omega^{(2)}_v$ è iniettiva. \\
...
\begin{flalign}
    \omega^{(1)} = adx_1 + bdx_2 + cdx_3 \tag{5.4.15}
\end{flalign}
...
\begin{flalign}
    \omega^{(2)} = Adx_2 \wedge dx_3 + Bdx_1 \wedge dx_3 + Cdx_1 \wedge dx_2 \tag{5.4.17}
\end{flalign}
...
\begin{flalign}
    (f^*\omega^{(k)})(x_1, \dots, x_k) = \omega^{(k)}(f(x_1), \dots, f(x_k)) \tag{5.5.1}
\end{flalign}
...
\textbf{Proposizione 5.5.1:} $f^*$ è un operatore lineare da $\Omega^{(k)}(\mathbb{R}^n)$ a $\Omega^{(k)}(\mathbb{R}^m)$. \\
...
\begin{flalign}
    ...
    &+ \lambda_2(f^*\omega_2)(x_1, \dots, x_k) \tag{5.5.2}
\end{flalign}
per ogni $x_1, \dots, x_k \in \mathbb{R}^m$, ovvero la tesi. \qed \\ \\
\textbf{Proposizione 5.5.2:} siano $f : \mathbb{R}^m \longrightarrow \mathbb{R}^n$ e $g : \mathbb{R}^n \longrightarrow \mathbb{R}^l$, allora $(g \circ f)^* = f^* \circ g^*$. \\
...
\begin{flalign}
    ...
    &= (f^* \circ g^*(\omega))(x_1, \dots, x_k) \tag{5.5.3}
\end{flalign}
per ogni $x_1, \dots, x_k \in \mathbb{R}^m$, ovvero la tesi. \qed \\ \\
\textbf{Proposizione 5.5.3:} $f^*$ preserva il prodotto esterno. \\
...
\begin{flalign}
    ...
    &\cdot \omega^{(l)}(f(x_{j_1}), \dots, f(x_{j_l})) \tag{5.5.4}
\end{flalign}
dove $(i_1, \dots, i_k, j_1, \dots, j_l)$ è una permutazione di $(1, \dots, k + l)$ e $\nu$ è determinato in base al segno della permutazione. Da $(5.5.4)$ si vede immediatamente che
\begin{flalign}
    f^*(\omega^{(k)} \wedge \omega^{(l)}) = (f^*\omega^{(k)}) \wedge (f^*\omega^{(l)}) \tag{5.5.5}
\end{flalign}
ovvero la tesi. \qed
...
\begin{flalign}
    df_x : T_xM \longrightarrow \mathbb{R} \tag{5.6.2}
\end{flalign}
...
\begin{flalign}
    df_x(v) = \frac{d}{dt}f(\gamma(t))|_{t = 0} \tag{5.6.3}
\end{flalign}
\textbf{Esempio:} sia $\alpha$ la curva piana
\begin{flalign}
    t \in \mathbb{R} \mapsto (x(t), y(t)) \in \mathbb{R}^2 \tag{5.6.4}
\end{flalign}
dove $x(t) = \cos t$ e $y(t) = \sin t$. Posto $v$ il vettore tangente ad $\alpha$ per $t = 0$, ovvero 
\begin{flalign}
    v = \frac{d}{dt}\alpha(t)|_{t = 0} = (0, 1) \tag{5.6.5}
\end{flalign}
...
\begin{flalign}
    df : TM \longrightarrow \mathbb{R} \tag{5.6.6}
\end{flalign}
...
\begin{flalign}
    \omega : TM \longrightarrow \mathbb{R} \tag{5.6.7}
\end{flalign}
...
\textbf{Proposizione 5.6.1:} ogni $1$-forma su $\mathbb{R}$ è il differenziale di qualche funzione. \\
...
\begin{flalign}
    \omega = a_1dx_1 + a_2dx_2 \tag{5.6.12}
\end{flalign}
dove $a_1, a_2$ sono date da 
\begin{flalign}
    a_1(x_1, x_2) &= x_1 \tag{5.6.13} \\
    a_2(x_1, x_2) &= x_1x_2 \tag{5.6.14}
\end{flalign}
Notiamo che non esiste una funzione $f$ del piano nella retta tale che 
\begin{flalign}
    a_1 = \frac{\partial f}{\partial x_1} \qquad a_2 = \frac{\partial f}{\partial x_2} \tag{5.6.15}
\end{flalign}
Infatti, possiamo pensare di "costruire" $f$ a partire dalla condizione in $(5.6.13)$, ovvero
\begin{flalign}
    f(x_1, x_2) = \frac{1}{2}x^2_1 + g(x_2) \tag{5.6.16}
\end{flalign}
dove $g$ è una funzione arbitraria della sola $x_2$ da determinare imponendo
\begin{flalign}
    \frac{\partial g}{\partial x_2} = x_1x_2 \tag{5.6.17}
\end{flalign}
Tuttavia, non esiste una funzione $g$ della sola $x_2$ che soddisfa tale condizione. \\ \\
\textbf{Teorema 5.6.1:} una $1$-forma differenziale su $M$ può essere scritta, in ogni intorno coordinato $U$, come combinazione lineare delle $n$-forme differenziali di base ($1$-forme algebriche), ovvero
\begin{flalign}
    \omega = a_1(x)dx_1 + \dots + a_n(x)dx_n \tag{5.6.18}
\end{flalign}
...
\begin{flalign}
    \omega_x = a_1dx_1 + \dots + a_ndx_n \tag{5.6.19}
\end{flalign}
...
\begin{flalign}
   \omega = a_1dx_1 + \dots + a_ndx_n \tag{5.6.20}
\end{flalign}
...
\begin{flalign}
   \omega^{(k)} : TM^k \longrightarrow \mathbb{R} \tag{5.7.1}
\end{flalign}
...
\textbf{Teorema 5.7.1:} data $M$ varietà differenziabile e $(U, \varphi)$ una carta che definisce delle coordinate locali su $M$, ogni $k$-forma differenziale $\omega^{(k)}$ su $M$ può essere scritta come
\begin{flalign}
   \omega^{(k)} = \sum_{i_1 < \dots < i_k} a_{i_1, \dots, i_k}(x)dx_{i_1} \wedge \dots \wedge dx_{i_k} \tag{5.7.2}
\end{flalign}
dove $a_{i_1, \dots, i_k} : M \longrightarrow \mathbb{R}$ è differenziabile per ogni $i_1 < \dots <i_k$. \\

\textbf{Dimostrazione:} la dimostrazione è analoga a quella del \textit{teorema 5.6.1}. Osserviamo che, per definizione, fissato $x \in M$, $\omega^{(k)}_x$ è una $k$-forma esterna su $T_xM$, pertanto può essere scritta come combinazione lineare degli elementi della base $\{dx_{i_1} \wedge \dots \wedge dx_{i_k}\}_{i_1 < \dots < i_k}$. Inoltre, dato che la dipendenza da $x$ deve essere differenziabile, si ottiene la tesi. \qed \\ \\
\textbf{Proposizione 5.7.1:} l'insieme $\Omega^{(k)}(M)$ delle $k$-forme differenziali sulla varietà $M$ è uno spazio vettoriale (di dimensione infinita se $k \leq \dim M$). \\
...
\begin{flalign}
   \omega^{(k)} = \sum_{i_1 < \dots < i_k}a_{i_1, \dots, i_k}(x)dx_{i_1} \wedge \dots \wedge dx_{i_k} \tag{5.7.3}
\end{flalign}
...
\begin{flalign}
   \mathbf{x} = (x_1, x_2, x_3) \qquad \mathbf{y} = (y_1, y_2, y_3) \tag{5.7.4}
\end{flalign}
...
\begin{flalign}
   \omega^{(2)} = X_1dx_2 \wedge dx_3 + X_2dx_1 \wedge dx_3 + X_3dx_1 \wedge dx_2 \tag{5.7.5}
\end{flalign}
...
\begin{flalign}
   dx_i = \frac{\partial x_i}{\partial y_1}dy_1 + \frac{\partial x_i}{\partial y_2}dy_2 + \frac{\partial x_i}{\partial y_3}dy_3 \tag{5.7.6}
\end{flalign}
...
\begin{flalign}
   ...
   &= \left|\frac{\partial(x_i, x_j)}{\partial(y_2, y_3)}\right|dy_2 \wedge dy_3 + \left|\frac{\partial(x_i, x_j)}{\partial(y_1, y_3)}\right|dy_1 \wedge dy_3 + \left|\frac{\partial(x_i, x_j)}{\partial(y_1, y_2)}\right|dy_1 \wedge dy_2 \tag{5.7.7}
\end{flalign}
Segue che 
\begin{flalign}
   \omega^{(2)} = Y_1dy_2 \wedge dy_3 + Y_2dy_1 \wedge dy_3 + Y_3dy_1 \wedge dy_2 \tag{5.7.8}
\end{flalign}
con
\begin{flalign}
   Y_k = \left|\frac{\partial(x_2, x_3)}{\partial(y_i, y_j)}\right|X_1 + \left|\frac{\partial(x_1, x_3)}{\partial(y_i, y_j)}\right|X_2 + \left|\frac{\partial(x_1, x_2)}{\partial(y_i, y_j)}\right|X_3 \tag{5.7.9}
\end{flalign}
...
\textbf{Proposizione 5.8.1:} sia $g : L \longrightarrow M$ un'applicazione differenziabile di varietà differenziabili. Allora $(f \circ g)^* = g^* \circ f^*$. \\

\textbf{Dimostrazione:} data $\omega^{(k)} \in \Omega^{(k)}(N)$ si ha
\begin{flalign}
    (f \circ g)^*\omega = \omega \circ (f \circ g)_* \tag{5.8.6}
\end{flalign}
...
\begin{flalign}
    ...
    &= g^*f^*\omega(v_1, \dots, v_s) \tag{5.8.7}
\end{flalign}
...
\begin{flalign}
    \omega^{(k)} = adx_1 \wedge \dots \wedge dx_k \tag{5.9.1}
\end{flalign}
Pertanto definiamo l'integrale di $\omega^{(k)}$ su $D$ come segue
\begin{flalign}
    \int_D \omega^{(k)} = \int_D a(x) \, dx_1 \dots dx_k \tag{5.9.2}
\end{flalign}
dato che identifichiamo $T\mathbb{R}^k$ con $\mathbb{R}^k$ stesso. \\ \\
Da $(5.9.2)$ segue che anche $\int_D \omega^{(k)}$ dipende da $\omega^{(k)}$ e da $D$ linearmente (per le proprietà dell'integrale di Riemann), ovvero
\begin{flalign}
    \int_{D_1 + D_2} \omega^{(k)}_1 + \omega^{(k)}_2 = \int_{D_1}\omega^{(k)}_1 + \int_{D_2}\omega^{(k)}_2 + \int_{D_1}\omega^{(k)}_2 + \int_{D_2}\omega^{(k)}_1 \tag{5.9.3}
\end{flalign}
per ogni $\omega^{(k)}_1, \omega^{(k)}_2 \in \Omega^{(k)}(\mathbb{R}^k)$ e ogni $D_1, D_2$ poliedri convessi limitati in $\mathbb{R}^k$. \\ \\
\textbf{Proposizione 5.9.1:} siano $D_1, D_2$ domini di integrazione di $\mathbb{R}^k$ e sia $f : D_1 \longrightarrow D_2$ un diffeomorfismo che conserva l'orientazione dell'interno di $D_1$ nell'interno di $D_2$. Data $\omega^{(k)} \in \Omega^{(k)}(\mathbb{R}^k)$ si ha
\begin{flalign}
    \int_{D_1} f^*\omega^{(k)} = \int_{D_2} \omega^{(k)} \tag{5.9.4}
\end{flalign} \\

\textbf{Dimostrazione:} nel sistema di coordinate $y = f(x)$ identificato da $f$ si ha
\begin{flalign}
    \omega^{(k)} = a(y)dy_1 \wedge \dots \wedge dy_k \tag{5.9.5}
\end{flalign}
...
\begin{flalign}
    dy_i = \sum_{j = 1}^k \frac{\partial y_i}{\partial x_j}dx_j \tag{5.9.6}
\end{flalign}
Allora
\begin{flalign}
    ...
    &= \int_{D_1} f^*\omega^{(k)} \tag{5.9.7}
\end{flalign}
ovvero la tesi. \qed \\ \\
\textbf{Corollario 5.9.1:} se $f$ inverte l'orientazione si ha
\begin{align}
    \int_{D_1} f^*\omega^{(k)} = -\int_{D_2} \omega^{(k)} \tag{5.9.8}
\end{align}
...
\begin{align}
    \int_M \omega^{(k)} = \pm\int_{\varphi(U)} (\varphi^{-1})^*\omega^{(k)} \tag{5.9.9}
\end{align}
dove il segno è positivo se e solo se il diffeomorfismo $\varphi$ preserva l'orientamento. Il termine al membro di destra è definito dalla $(5.9.2)$. \\ \\
\textbf{Proposizione 5.9.2:} l'integrale di una $k$-forma differenziale con supporto compatto non dipende dalla scelta della carta locale. \\
...
\begin{align}
    ...
    &= \int_{\varphi(U)}(\varphi^{-1})^*\omega^{(k)} \tag{5.9.10}
\end{align}
ovvero la tesi. \qed \\ \\
...
\begin{align}
    \int_M \omega^{(k)} = \sum_\alpha \int_M \psi_\alpha \omega^{(k)} \tag{5.9.11}
\end{align}
Analogamente, si può dimostrare che la definizione $(5.9.11)$ non dipende dalla scelta della partizione dell'unità. \\ \\
\textbf{Proposizione (proprietà dell'integrale di forme differenziali):} siano $M, N$ $k$-varietà differenziabili e siano $\omega^{(k)}, \eta^{(k)} \in \Omega(M)$ a supporto compatto. Allora: \\
\textbf{1.} dati $\alpha, \beta \in \mathbb{R}$ si ha
\begin{align}
    \int_M (\alpha\omega^{(k)} + \beta\eta^{(k)}) = \alpha\int_M \omega^{(k)} + \beta\int_M \eta^{(k)} \tag{5.9.12}
\end{align}
\textbf{2.} indichiamo con $-M$ la varietà $M$ con orientazione opposta. Allora
\begin{align}
    \int_{-M} \omega^{(k)} = -\int_M \omega^{(k)} \tag{5.9.13}
\end{align}
\textbf{3.} se $F : N \longrightarrow M$ è un diffeomorfismo che preserva $(+)$ o inverte $(-)$ l'orientamento si ha
\begin{align}
    \int_M \omega^{(k)} = \pm\int_{N} F^*\omega^{(k)} \tag{5.9.14}
\end{align} \\

\textbf{Dimostrazione:} $(1)$ segue immediatamente dalla linearità del pushback e dalla linearità dell'integrale di Riemann in $\mathbb{R}^n$. \\
...
\newpage
\subsection{Derivata esterna}
Data una generica $1$-forma 
\begin{align}
    \omega = \sum_{i = 1}^k \omega_i dx_i \tag{5.10.1}
\end{align}
...
\begin{align}
    \frac{\partial \omega_i}{\partial x_j} - \frac{\partial \omega_j}{\partial x_i} = 0 \tag{5.10.2}
\end{align}
per ogni $i, j = 1, \dots, k$ e ogni sistema di coordinate locale, conseguenza del \textit{teorema di Schwartz} (di inversione dell'ordine di derivazione). \\
Possiamo definire la $2$-forma differenziale $d\omega$ come segue
\begin{align}
    d\omega = \sum_{i < j} \left(\frac{\partial \omega_i}{\partial x_j} - \frac{\partial \omega_j}{\partial x_i}\right)dx_i \wedge dx_j \tag{5.10.3}
\end{align}
...
\begin{align}
    ...
    &= \sum_{i_1 < \dots < i_k}\sum_{j = 1}^n \frac{\partial \omega_{i_1, \dots, i_k}}{\partial x_j}dx_j \wedge dx_{i_1} \wedge \dots \wedge dx_{i_k} \tag{5.10.4}
\end{align}
...
\begin{align}
    ...
    &= \sum_{i < j}\frac{\partial \omega_j}{\partial x_i}dx_i \wedge dx_j \tag{5.10.5}
\end{align}
per antilinearità delle forme differenziali. \\ \\
\textbf{Proposizione (proprietà della derivata esterna in $\mathbb{R}^n$):} l'operatore di derivata esterna soddisfa le seguenti proprietà: \\
\textbf{1.} $d$ è lineare. \\
...
\begin{align}
    d(\omega \wedge \eta) = d\omega \wedge \eta + (-1)^k\omega \wedge d\eta \tag{5.10.6}
\end{align}
...
\begin{align}
    d(F^*\omega^{(k)}) = F^*d\omega^{(k)} \tag{5.10.7}
\end{align} \\

\textbf{Dimostrazione:} $(1)$ la linearità dell'operatore derivata esterne segue immediatamente dalla struttura di spazio lineare definita sull'insieme delle $k$-forme differenziabili in $\mathbb{R}^k$ per ogni $k < n$ e dalla linearità dell'operatore derivata parziale in $\mathbb{R}^n$. \\
$(2)$ Date
\begin{align*}
    \omega &= \sum_{i_1 < \dots < i_k} u_{i_1, \dots, i_k} \, dx_{i_1} \wedge \dots \wedge dx_{i_k} \quad \eta = \sum_{j_1 < \dots < j_l} v_{j_1, \dots, j_k} \, dx_{j_1} \wedge \dots \wedge dx_{j_l} \tag{5.10.8}
\end{align*}
si ha che
\begin{align*}
    d(\omega \wedge \eta) &= d(uv \, dx_I \wedge dx_J) \tag{5.10.9}
\end{align*}
per linearità (compattando la notazione degli indici). In particolare
\begin{align*}
    d(uv \, dx_I \wedge dx_J) &= (vdu + udv) \wedge dx_I \wedge dx_J \\
    &= (du \wedge dx_I) \wedge (vdx_J) + (-1)^k(udx_I) \wedge (dv \wedge dx_J) \tag{5.10.10}
\end{align*}
dove si è usata la multilinearità e il fatto che $udv \wedge dx_I \wedge dx_J = (-1)^k udx_I \wedge (dv \wedge dx_J)$. \\
$(3)$ Segue dalla linearità dell'operatore di derivata parziale in $\mathbb{R}^n$ e dal \textit{teorema di Schwartz} (di inversione dell'ordine di derivazione), infatti
\begin{align}
    \sum_{i, j}\frac{\partial}{\partial x_i}\left(\frac{\partial \omega_{i_1, \dots, i_k}}{\partial x_j}\right)dx_i \wedge dx_j &= \sum_{i < j}\overbrace{\left(\frac{\partial \omega_{i_1, \dots, i_k}}{\partial x_i\partial x_j} - \frac{\partial \omega_{i_1, \dots, i_k}}{\partial x_j \partial x_i}\right)}^{0}dx_i \wedge dx_j \tag{5.10.11}
\end{align}
...
\begin{align}
    d(F^*\omega^{(k)}) = F^*d\omega^{(k)} \tag{5.10.12}
\end{align}
ovvero $d$ commuta con l'operazione di pullback. \qed \\ \\
...
\textbf{Teorema (esistenza e unicità della differenziazione esterna):} sia $M$ una varietà differenziabile, per ogni $k \in \mathbb{N}$ esiste unico
\begin{align*}
    d : \Omega^{(k)}(M) \longrightarrow \Omega^{(k + 1)}(M) \tag{5.10.13}
\end{align*}
...
\begin{align*}
    d(\omega \wedge \eta) = d\omega \wedge \eta + (-1)^k\omega \wedge d\eta \tag{5.10.14}
\end{align*}
\textbf{3.} $d \circ d = 0$. \\
\textbf{4.} per ogni $f \in \Omega^{(0)}(M) = C^\infty(M)$ si ha che $df$ è il suo differenziale, dato da $df(X) = Xf$ ($X$ campo vettoriale su $M$). \\
Definiamo $d$ per ogni carta locale come in $5.10.4$. \\

\textbf{Dimostrazione:} date $(U, \varphi)$ una carta locale di $M$ e $\omega$ una $k$-forma differenziale su $M$, definiamo la derivata esterna di $\omega$ su $U$ come segue
\begin{align*}
    d\omega = \varphi^*d(\varphi_*\omega) \tag{5.10.15}
\end{align*}
...
\begin{align*}
    (\varphi \circ \psi^{-1})^*d(\varphi_*\omega) = d((\varphi \circ \psi^{-1})^*\varphi_*\omega) = d(\psi_*\varphi^*\varphi_*\omega) = d(\psi_*\omega) \tag{5.10.16}
\end{align*}
...
\begin{align*}
    d(\psi\eta) = d\psi \wedge \eta + \psi d\eta = 0 \tag{5.10.17}
\end{align*}
Calcolando in $p$ si ottiene
\begin{align*}
    0 = d(\psi\eta)_p = \psi(p)d\eta_p = d\eta_p = d\omega_1|_p - d\omega_2|_p \tag{5.10.18}
\end{align*}
ovvero $d\omega_1|_p = d\omega_2|_p$. \\
A questo punto, data $\omega \in \Omega^{(k)}(M)$ arbitraria, in $(U, \varphi)$ possiamo scrivere $\omega$ in termini coordinate locali
\begin{align*}
    \omega = \sum_I \omega_I dx_I \tag{5.10.19}
\end{align*}
Questo significa che $d\omega$ è definita tramite $5.10.14$. Dato $\widetilde{d}$, altro operatore di differenziazione esterna che soddisfa $(1)-(4)$, possiamo costruire, come prima fatto con una funzione a scalino, delle funzioni $\widetilde{\omega}_I$ e $\widetilde{x}_I$ che sono identiche a $\omega_I$ e $x_I$ in un intorno di $p$ in $U$. Segue che $d = \widetilde{d}$ in $p$ e quindi, dall'arbitrarietà di $p$, si ottiene la tesi. \qed \\ \\
\textbf{Proposizione 5.10.1:} siano $M, N$ varietà differenziabili e $F \in C^\infty(M, N)$. Allora il pullback tramite $F$ commuta con la derivata esterna. \\

\textbf{Dimostrazione:} date $\omega \in \Omega^{(k)}(N)$ e $(U, \varphi)$ e $(V, \psi)$ rispettiamente carte locali di $M$ e $N$, si ha che
\begin{align*}
    ...
    &= d(F^*\omega) \tag{5.10.20}
\end{align*}
dove si è usato che $\phi^*\phi_* = 1 = \phi_*\phi^*$ ($1$ inteso come identità). \qed \\ \\
...
\newpage
\subsection{Derivata di Lie di forme differenziali}
...
\begin{align*}
    \mathcal{L}_Xf = \left.\frac{d}{dt}\right|_{t = 0}f(\varphi_t(p)) \tag{5.11.1}
\end{align*}
ovvero la derivata di $f$ lungo le curve integrali di $X$. \\
Analogamente, per una $k$-forma differenziale su $M$
\begin{align*}
    \mathcal{L}_X\omega = \left.\frac{d}{dt}\right|_{t = 0}(\varphi^*_t\omega) \tag{5.11.2}
\end{align*}
...
\textbf{Proposizione 5.11.1:} siano $M$ una varietà differenziabile e $X$ un campo vettoriale regolare su $M$, date $\omega$ un $k$-forma su $M$ e $\eta$ una $l$-forma su $M$ si ha che
\begin{align*}
    \mathcal{L}_X(\omega \wedge \eta) = \mathcal{L}_X\omega \wedge \eta + \omega \wedge \mathcal{L}_X\eta \tag{5.11.3}
\end{align*}
ovvero $\mathcal{L}_X$ soddisfa la regola del prodotto per il prodotto esterno. \\

\textbf{Dimostrazione:} l'operazione di pullback si distribuisce sul prodotto esterno, pertanto
\begin{align*}
    \varphi^*_t(\omega \wedge \eta) = \varphi^*_t\omega \wedge \varphi^*_t\eta \tag{5.11.4}
\end{align*}
...
\begin{align*}
    ...
    &= \mathcal{L}_X\omega \wedge \eta + \omega \wedge \mathcal{L}_X \eta \tag{5.11.5}
\end{align*}
ovvero la tesi. \qed \\ \\
\textbf{Definizione:} sia $M$ una varietà differenziabile e sia $X$ un campo vettoriale su $M$, data $\omega$ una $k$-forma su $M$ definiamo la contrazione
\begin{align*}
    \iota_X : \omega \in \Omega^{(k)}(M) \mapsto \omega_X \in \Omega^{(k - 1)}(M) \tag{5.11.6}
\end{align*}
dove $\omega_X$ è la $k - 1$-forma su $M$ definita da
\begin{align*}
    \omega_X : (p, v_1, \dots, v_{k - 1}) \mapsto \omega_p(X_p, v_1, \dots, v_{k - 1}) \tag{5.11.7}
\end{align*}
ovvero la prima entrata della forma è fissata a $X$. \\
Per una $1$-forma di base $dx_i$ si ha
\begin{align*}
    \iota_X(dx_i) = dx_i(X) = dx_i\left(\sum_j X_j\frac{\partial}{\partial x_j}\right) = X_i\delta_{ij} \tag{5.11.8}
\end{align*}
dove si è scomposto $X$ in termine della base delle derivazioni (vettori base dello spazio tangente). Osserviamo che
\begin{align*}
    \iota_X(dx_{i_1} \wedge \dots \wedge dx_{i_k}) = \sum_{r = 1}^k (-1)^{r - 1}X_{i_r}dx_{i_1} \wedge \dots \cancel{dx_{i_r}} \dots \wedge dx_{i_k} \tag{5.11.9}
\end{align*}
...
\textbf{Teorema (formula magica di Cartan):} siano $M$ una varietà differenziabile, $X$ un campo vettoriale su $M$ e $\omega$ una $k$-forma differenziale su $M$, allora
\begin{align*}
    \mathcal{L}_X\omega = \iota_X(d\omega) + d(\iota_X \omega) \tag{5.11.10}
\end{align*} \\

\textbf{Dimostrazione:} ragioniamo per induzione. Data $f$ una $0$-forma su $M$ si ha
\begin{align*}
    \iota_X(df) + d(\iota_X f) = \iota_X df = df(X) = \mathcal{L}_X f \tag{5.11.11}
\end{align*}
...
\begin{align*}
    \mathcal{L}_X \omega = \sum_I \mathcal{L}_X dx_{i_1} \wedge \eta + dx_{i_1} \wedge \mathcal{L}_X \eta \tag{5.11.12}
\end{align*}
Per ipotesi induttiva si ottiene
\begin{align*}
    \mathcal{L}_X \omega &= \sum_I (\cancel{\iota_X(d^2x_{i_1})} + dX_{i_1}) \wedge \eta + dx_{i_1} \wedge (\iota_X(d\eta) + d(\iota_X \eta)) \\
    &= \sum_I dX_{i_1} \wedge \eta + dx_{i_1} \wedge (\iota_X(d\eta) + d(\iota_X\eta)) \tag{5.11.13}
\end{align*}
Notiamo che
\begin{align*}
...
Pertanto
\begin{align*}
    \iota_X(d\omega) = \sum_I -X_{i_1}d\eta + dx_{i_1} \wedge \iota_X(d\eta) \tag{5.11.14}
\end{align*}
Mentre
\begin{align*}
    d(\iota_X \omega) &= d\left(\sum_I X_{i_1}\eta - dx_{i_1} \wedge \iota_X\eta\right) \\
    &= \sum_I dX_{i_1} \wedge \eta + X_{i_1}d\eta + dx_{i_1} \wedge d(\iota_X \eta) \tag{5.11.15}
\end{align*}
Sommando $(5.11.14)$ e $(5.11.15)$ si cancellano i termini $X_{i_1}d\eta$ e si ottiene
\begin{align}
    \iota_X(d\omega) + d(\iota_X\omega) = \sum_I dX_{i_1} \wedge \eta + dx_{i_1} \wedge (\iota_X(d\eta) + d(\iota_X\eta)) = \mathcal{L}_X\omega \tag{5.11.16}
\end{align}
confrontando con $(5.11.13)$. \qed \\ \\
\textbf{Corollario 5.11.1:} la derivata di Lie commuta con il differenziale esterno. \\

\textbf{Dimostrazione:} si ha
\begin{align*}
    \mathcal{L}_X(d\omega) = \iota_X(\cancel{d^2\omega}) + d(\iota_X d\omega) \tag{5.11.17}
\end{align*}
Mentre
\begin{align*}
    d\left(\mathcal{L}_X\omega\right) = d(\iota_X d\omega) + \cancel{d^2(\iota_X \omega)} \tag{5.11.18}
\end{align*}
Confrontando si ottiene la tesi. \qed
\end{document}
