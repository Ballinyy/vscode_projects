...
\title{Introduzione alla topologia e geometria differenziale}
...
($\Leftarrow$) supponiamo ora che per ogni $x \in A$ esiste $V_x \in \mathcal{N}(x)$ tale che $V_x \subseteq A$, allora per definizione di intorno esiste $W_x \subseteq V_x$ tale che $W_x \in \tau$ per ogni $x \in A$, dunque $A = \bigcup_{x \in A} W_x$, unione di aperti, pertanto $A \in \tau$. \\ \\ \\
...
\end{align}
poiché per ipotesi $B_h \cap B_k \in \tau_\mathcal{B}$ per ogni $(h, k) \in H \times K$, si ottiene la tesi. \\ \\
...
\begin{gather}
    \mathcal{B} := \{ D_{1/n}(\mathbf{x}) := n \in \mathbb{N}, \mathbf{x} \in \mathbf{Q}^n \} \notag
\end{gather}
...
\textbf{Dimostrazione:} sia $x \in D(A)$, supponiamo per assurdo che $x \in X \setminus \overline{A}$, poiché $\overline{A}$ è chiuso si ha che $X \setminus \overline{A}$ è aperto, pertanto esiste $V \in \mathcal{N}(x)$ tale che $V \subset X \setminus \overline{A}$, ciò comporta che $\overline{A} \cap (V \setminus \{x_0 \} = \emptyset$ e poiché $\overline{A} \supseteq A$ si ottiene $A \cap (V \setminus \{x_0 \} = \emptyset$, ma è un assurdo. \\ \\ \\
...
\textbf{Dimostrazione:} si verifica immediatamente che gli aperti di $X$ sono unione degli aperti di $\mathcal{B}$. \\ \\ \\
...
\end{gather}
dunque $Y$ è uno spazio di Hausdorff. \\ \\ \\
\textbf{Proposizione 1.2.2:} se $X$ è uno spazio di Hausdorff i punti sono chiusi. \\

\textbf{Dimostrazione:} sia $x \in X$, consideriamo l'insieme $X \setminus \{ x \}$ e mostriamo che è aperto. Per ogni $y \in X \setminus \{ x \}$ si ha, per ipotesi, che esistono $V \in \mathcal{N}(x)$ e $W \in \mathcal{N}(y)$ tali che $V \cap W = \emptyset$, allora si ha che $W \subseteq X \setminus V \subseteq X \setminus \{ x \}$. \\ \\ \\
\textbf{Proposizione 1.2.3:} sia $X$ uno spazio di Hausdorff, allora se il limite di una successione esiste è unico. \\

\textbf{Dimostrazione:} sia $\{ x_n \}_{n \in \mathbb{N}} \subseteq X$ una successione, supponiamo per assurdo che essa converga in due punti $y_1$ e $y_2$ di $X$. Per ipotesi si ha che esistono $V \in \mathcal{N}(y_1)$ e $W \in \mathcal{N}(y_2)$ tali che $V \cap W = \emptyset$. Per definizione di limite esiste $M \in \mathbb{N}$ tale che $x_n \in V$ e $x_n \in W$ per ogni $n \geq M$, ma ciò è un assurdo. \\ \\ \\
...
\end{align}
Concludiamo che anche $X$ è uno spazio di Hausdorff. \\ \\ \\
...
\textbf{Dimostrazione:} segue direttamente dal fatto che uno spazio regolare è $T_1$, dunque i punti sono sottoinsiemi chiusi. \\ \\ \\
...
\textbf{Dimostrazione:} segue direttamente dal fatto che uno spazio normale è $T_1$, dunque i punti sono sottoinsiemi chiusi. \\ \\ \\
...
\end{align}
per la disuguaglianza triangolare, dunque $D_{\frac{r}{3}}(x_0) \subset W_1$, poiché ciò vale comunque scelto $x_0 \in W_1$ si ha che $W_1$ è aperto. Analogamente si dimostra per $W_2$, dunque si ottiene la tesi. \\ \\ \\
...
\textbf{Dimostrazione:} supponiamo che esistono $A, B \in X$ aperti non vuoti e disgiunti tali che $X = A \cup B$. Consideriamo due punti $p \in A$ e $q \in B$, per ipotesi si ha che esiste un sottoinsieme $K$ connesso tale $p, q \in K$, inoltre si ha che $K = K \cap (A \cup B)$ e $(K \cap A) \cap (K \cap B) = \emptyset$, ma ciò è un assurdo perché $K$ è connesso per ipotesi, pertanto si ottiene la tesi. \\ \\ \\
...
\textbf{Dimostrazione:} sia $M$ un sottoinsieme non vuoto, aperto e chiuso di $f(Z)$. Per ipotesi di continuità si ha che $f^{-1}(M)$ è un sottoinsieme non vuoto, aperto e chiuso di $X$ tale che $f^{-1}(M) \cap Z = Z$, perché $Z$ è connesso. Dunque $f^{-1}(M) \supseteq Z$ e ciò comporta che $M = f(f^{-1}(M)) = f(Z)$, la tesi segue.  \\ \\
...
\textbf{Dimostrazione:} $C(p)$ è connesso poiché unione di insiemi connessi. Supponiamo ora che esiste $Z \supset C(p)$ connesso, allora esiste $q \in Z$ tale che $q \not\in C(p)$ e $p, q$ sono connessi, ciò comporterebbe, per definizione, $q \in C(p)$, ma ciò è un assurdo. \\ \\ Notiamo quindi che $C(p)$ è un chiuso per la \textit{proposizione 1.3.1}. \\ \\ \\
...
\end{gather}
inoltre si ha $q, p \in G$, pertanto $G \subset C(p)$ e $C(p) \cap B \neq \emptyset$, dunque, essendo $B$ un aperto qualsiasi, si ha che $C(p)$ è denso ed è chiuso per la \textit{proposizione 1.3.6}, otteniamo allora che $C(p) = X$, cioè $X$ è connesso.
\subsubsection{Esercizi}
...
\textbf{3.} Sia $X$ uno spazio topologico $T_2$ e sia $E \subset X$ connesso. Dimostrare se è sempre vero che $|E| \geq 2 \implies |E| = +\infty$. \\
...
\end{gather}
$\beta_s$ è un arco in $X$ di estremi $p$ e $\alpha(s)$, quindi $\alpha(s) \in C_a(p)$ per ogni $s \in \mathbf{I}$, pertanto $\alpha(\mathbf{I}) \subset C_a(p)$. \\ \\ \\
...
\end{gather}
$\alpha$ è un arco in $X$ tale che $\alpha(0) = p$ e $\alpha(1) = q$, quindi $X$ è connesso per archi.
...
\end{gather}
dunque $X$ è sia chiuso che aperto in $[a, b]$, tuttavia l'unico intervallo chiuso e aperto in $[a, b]$ è $[a, b]$ stesso, perciò $X = [a, b]$, la tesi segue. \\ \\ \\
...
\end{gather}
è un intorno aperto di $p$ contenuto in $X \setminus K$. Poiché ciò vale per ogni $p \in X \setminus K$ si ha che $X \setminus K$ è aperto, quindi $K$ è chiuso. \\ \\ \\
...
\end{gather}
poiché $K$ è chiuso si ha $a, b \in K$, allora $K \subseteq [a, b]$ che è compatto perché è un intervallo chiuso di $\mathbb{R}$, inoltre $K$ è chiuso in $[a, b]$, pertanto $K$ è un sottoinsieme chiuso di uno spazio compatto, per la \textit{proposizione 1.5.1} $K$ è compatto. \\ \\ \\
...
$b)$ Se la successione ha infiniti numeri uguali, essi formano una sottosuccessione convergente. Se la successione ha infiniti punti distinti, allora possiede un punto di accumulazione in $X$ e, per la \textit{proposizione 1.2.7}, ammette una sottosuccessione convergente. \\ \\ \\
...
\textbf{Dimostrazione:} poiché $\{ x_n \}$ è limitata esiste $M > 0$ tale che $\{ x_n \} \subset [-M, M]$, ma $[-M, M]$ è un sottoinsieme compatto di $\mathbb{R}$, pertanto, per la \textit{proposizione 1.5.2}, $\{ x_n \}$ ammette una sottosuccessione convergente. \\ \\ \\
...
\end{gather}
(contenuto poiché $f$ potrebbe non essere suriettiva), ovvero $\{ U_1, \dots, U_n\}$ è un ricoprimento aperto finito di $f(X)$. $f(X)$ è compatto. \\ \\
...
\textbf{Dimostrazione:} dalla \textit{proposizione 1.5.3} segue che $f(X)$ è compatto, pertanto per il \textit{corollario 1.5.1} è chiuso e limitato, dunque $f(X)$ contiene il suo estremo superiore ed il suo estremo inferiore. \\ \\ \\
...
\end{gather}
ovvero $A_1 \cup \dots \cup A_m = X$, pertanto $\mathcal{A}$ ammette un sottoricoprimento finito, quindi $X$ è compatto. \\ \\ \\
...
Sia $K$ chiuso e limitato, allora $K \subset \mathbf{I}_1 \times \dots \times \mathbf{I}_1$ ($n$ copie di $\mathbf{I}_1$) che è compatto per il \textit{teorema di Tichonov}, inoltre $K$ è chiuso in $\mathbf{I}_1 \times \dots \times \mathbf{I}_1$ poiché è chiuso in $\mathbb{R}^n$, pertanto $K$ è un sottoinsieme chiuso di uno spazio compatto, segue che $K$ è compatto.
...
\textbf{4.} Sia $X$ uno spazio metrizzabile, siano $A, B$ sottoinsieme chiusi, non vuoti e disgiunti di $X$. Posto $d(A, B) := \inf \{ d(a, b) : a \in A, b \in B \}$, dimostrare che se $A$ (oppure $B$) è compatto si ha $d(A,B) = 0$ se e solo se $A \cap B \neq \emptyset$. \\

\textbf{Dimostrazione:} fissiamo $A$ compatto. $(\Rightarrow)$ Sia $d(A, B) = 0$, poniamo
\begin{gather}
    d_B(x) = \inf \{ d(x, b) := b \in B \} \notag
\end{gather}
Poiché $d_B$ è continua si ha che $d_B(A) \subset \mathbb{R}$ è compatto, ovvero chiuso e limitato, pertanto
\begin{gather}
    m := \min d_B(A) \in d_B(A) \notag \implies \overline{a} := d^{-1}_B(m) \in A \notag
\end{gather}
Inoltre
\begin{gather}
    d_B(\overline{a}) = \inf \{ d(\overline{a}, b) : b \in B \} = d(A,B) = 0 \notag
\end{gather}
Mostriamo ora che $\overline{a} \in B$. Supponiamo $A \cap B \neq \emptyset$, allora $A \subset X \setminus B$ che è un aperto, pertanto esiste $\delta > 0$ tale che $D_\delta(\overline{a}) \subset X \setminus B$, cioè $d_B(\overline{a}) > \delta$, ma ciò è un assurdo. \\ \\
...
\end{align}
notiamo che in questo caso $K_i$ è chiuso nella topologia relativa ed è compatto poiché chiuso e limitato in $\mathbb{R}^n$. \\ \\
...
\textbf{Dimostrazione:} sia $x \in X$, allora esiste $U \in \mathcal{U}$ tale che $x \in U$, inoltre, per ipotesi di compattezza locale, esiste $K$ compatto tale che $x \in K$. Consideriamo l'aperto $\mathring{K} \cap U$, per ipotesi $X$ possiede una base numerabile $\mathcal{B}$ di aperti, pertanto esiste $B_i \in \mathcal{B}$ tale che $x \in B_i \subset \mathring{K} \cap U$. Inoltre $B_i$ è relativamente compatto, infatti $\overline{B}_i \subset K$, ovvero la chiusura di $B_i$ è contenuta in $K$ che è compatto. Dunque per ogni $x \in X$ e ogni $U \in \mathcal{U}$ intorno di $x$ esiste $B_i \in \mathcal{B}$ relativamente compatto tale che $x \in B_i \subset U$, allora la famiglia $\{B_i : i \in \mathbb{N}\}$ è un raffinamento di $\mathcal{U}$.
...
\textbf{Definizione A.2:} $G$ è detti \textit{gruppo} se è dotato di un'operazione binaria \\ $\cdot : G \times G \longrightarrow G$ tale che: \\
...
\textbf{Dimostrazione:} segue immediatamente dalla definizione di gruppo. \\ \\ \\
...
\end{gather}
Dunque, $f \cdot g (i) = f(g(i)) = i$ e $g \cdot f (i) = g(f(i)) = i$ per definizione, pertanto esiste l'inverso per ogni elemento di $S_n$. \\ \\ \\
...
\textbf{Dimostrazione:} vale $\det \mathbf{I}_n = \det (AA^t) = (\det A)^2$. \\ \\ \\
...
\end{gather}
ovvero $g(x) = x$ per ogni $x \in \mathbb{R}^n$, dunque $g = \mathbf{I}_n$. \\ \\ \\
...
\end{gather}
per ogni $v \in r$. Pertanto $A$ è un'isometria che fissa l'origine e lascia invariate $r$ e $r_\perp$, ovvero $A$ è una rotazione attorno a $r$. \\ \\ \\
...
Infine si ha che $\mathbf{SO}_n(\mathbb{R}) = \mathbf{SL}_n(\mathbb{R}) \cap \mathbf{O}_n(\mathbb{R})$, dunque intersezione di due chiusi e quindi chiuso. \\ \\ \\
...
\begin{gather}
    \sum_{h, i} a^2_{ih} = n \notag
\end{gather}
cioè $\mathbf{O}_n(\mathbb{R}) \subset \overline{D_{\sqrt{n}}(0)}$, allora $\mathbf{O}_n(\mathbb{R})$ è un sottoinsieme chiuso e limitato di $\mathbb{R}^{n^2}$ e per il \textit{corollario 1.5.3} è compatto. Inoltre $\mathbf{SO}_n(\mathbb{R})$ è un sottoinsieme chiuso di un insieme compatto e pertanto compatto.
...
\begin{gather}
    ...
    \end{pmatrix} \notag
\end{gather}
per ogni $\rho > 0$ esiste $\Tilde{k} \in \mathbb{N}$ tale che
\begin{gather}
...
\end{gather}
per ogni $k \in \mathbb{N}$. Inoltre, analogamente all'esercizio precedente, si ha che per ogni $\rho > 0$ esiste $\Tilde{k} \in \mathbb{N}$ tale che
\begin{gather}
...
\end{gather}
Infine si ha che la matrice nulla appartiene a $\mathfrak{g}(G)$ poiché la funzione costante $\delta(t) = \mathbf{I}_n$ per ogni $t \in (-\varepsilon, \varepsilon)$ è differenziabile e $\delta'(0) = 0 \in \mathfrak{g}(G)$. \\ \\ \\
...
Inoltre la funzione $\det$ è continua dunque, per il teorema della permanenza del segno, esiste $\varepsilon > 0$ tale che $\det \gamma(t) > 0$ per ogni $t \in (-\varepsilon, \varepsilon)$, pertanto la tesi segue e $\dim \mathbf{GL}_n(\Bbb{R}) = n^2$. \\ \\ \\
...
\end{gather}
dove il penultimo passaggio si ottiene ragionando per induzione. \\ \\ \\
...
\end{gather}
dove sono stati scegli come indipendenti i primi $n - 1$ elementi della diagonale senza perdita di generalità. \\ \\ \\
...
Notiamo che la $(3)$ è equivalente a dire $x \perp xA$ per ogni $A$ antisimmetrica. \\ \\ \\
...
\begin{gather}
    X + X^t = 0 \notag
\end{gather}
ciò comporta anche che $X$ è invertibile e che $\det X = 1$, pertanto la tesi segue. \\ \\ \\
...
\textbf{Dimostrazione:} si ha che $1 + 2 + \dots (n - 1) = \begin{pmatrix}
    n \\
    2
\end{pmatrix}$, ovvero il numero delle componenti indipendenti. \\ \\ \\
...
\end{gather}
pertanto $\{ W + a_{k + 1}, \dots, W + a_n \}$ è un insieme di vettori linearmente indipendenti che genera $V/W$, ovvero $\dim V/W = n - k$.
...
\begin{gather}
    \Tilde{f} : \mathbb{R}^{V \times W} \longrightarrow U \notag
\end{gather}
definita ponendo
\begin{gather}
...
\end{gather}
ovvero $\dim (V \otimes W) = n \cdot m$. \\ \\
...
\end{gather}
dunque $\Phi$ è un isomorfismo e $V^* \otimes V \cong \mathrm{End}(V)$.
...
Se $X$ è una varietà topologica connessa e $\dim (p) = n$ per qualche $p \in X$, si ha che $\dim (p) = n$ per ogni $p \in X$, dunque poniamo $\dim X = n$. \\ \\
...
\end{gather}
con $\lambda_i = L(x_i)$, pertanto la tesi segue. \\ \\ \\
