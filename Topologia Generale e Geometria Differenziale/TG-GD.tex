...
\title{Topologia Generale e Geometria Differenziale}
...
$(\leftarrow)$ Sia $(X, d)$ completo e totalmente limitato. Dimostriamo che $X$ è compatto per successioni e dal \textit{teorema 1.6.1} seguirà la tesi. Fissiamo $\varepsilon > 0$, per ipotesi esiste un numero finito di punti $\{x_1, \dots, x_n\} \subset X$ tale che 
\begin{align}
    X = \overline{D_\varepsilon(x_1)} \cup \dots \cup \overline{D_\varepsilon(x_n)} \notag
\end{align}
Sia $\{x_n\}_{n \in \mathbf{N}}$ una successione di punti in $X$. Se $\{x_n\}_{n \in \mathbf{N}}$ ha un numero finito di punti distinti esiste certamente una sottosuccessione convergente. Supponiamo allora che $\{x_n\}_{n \in \mathbf{N}}$ abbia un numero infinito di punti distinti e costruiamo per induzione una sottosuccessione convergente. 
