...
\textbf{Definizione 1.1.14:} siano $X$ uno spazio topologico, $\{ x_n \}_{n \in \mathbb{N}} \subseteq X$ e $y \in X$. Si dice che $y$ è un limite per $\{ x_n \}_{n \in \mathbb{N}}$ per $n \rightarrow \infty$ se per ogni $V \in \mathcal{N}(y)$ esiste $M \in \mathbb{N}$ tale che $x_n \in V$ per ogni $n \geq M$, in questo caso scriviamo $x_n \rightarrow y$, per $n \rightarrow \infty$. \\ \\
...
\textbf{Definizione 1.1.16:} sia $X$ uno spazio topologico, $X$ è detto \textit{separabile} se esiste $Y \subset X$ denso e numerabile (numerabile se esiste biezione con $\mathbb{N}$).
...
Tuttavia $\det : \mathbb{M}_n \longrightarrow \mathbb{R}$ è continua, dunque per la \textit{proposizione 1.5.3} si ha che $\det(G)$ è compatto, ma ciò è un assurdo. \\ \\
...
\textbf{Definizione 1.6.7:} sia $X$ uno spazio metrico, una successione $\{x_n\}_{n \in \mathbb{N}}$ è detta di Cauchy se per ogni $\varepsilon > 0$ esiste $N_\varepsilon \in \mathbb{N}$ tale che $d(x_n, x_m) < \varepsilon$ per ogni $n, m > N_\varepsilon$. \\ \\ \\
...
\textbf{Definizione 2.2.1:} sia $n \in \mathbb{N}$, poniamo
\begin{gather}
...
\textbf{Definizione 2.2.6:} $f : \mathbb{R}^n \longrightarrow \mathbb{R}^n$ si dice \textit{isometria} se $\| x - y \| = \| f(x) - f(y) \|$ per ogni $x, y \in \mathbb{R}^n$. \\ \\ \\
...
\textbf{Definizione 2.2.7:} una base ordinata ortonormale $\{ v_1, v_2, v_3 \}$ di $\mathbb{R}^3$ è detta \textit{destra} se $v_3 = v_1 \wedge v_2$. \\ \\ \\
...
\textbf{Definizione 2.2.8:} sia $X \subset \mathbb{R}^n$, il \textit{gruppo delle simmetrie di $X$} è
\begin{gather}
...
\end{gather} \\ \\
\textbf{Definizione 2.2.9:} sia $m \in \mathbb{N}$, $m \geq 3$, il \textit{gruppo diedrale} di ordine $2m$ è
\begin{gather}
...
\textbf{Dimostrazione:} omessa. \qed \\ \\ \\
\textbf{Definizione 2.2.11:} sia $X$ un poliedro in $\mathbb{R}^3$. $X$ è detto \textit{regolare} se valgono: \\
...
\textbf{Definizione 2.3.1:} un \textit{gruppo di matrici} è un sottogruppo $G$ di $\mathbf{GL}_n(\mathbb{R})$ tale che $G$ è chiuso in $\mathbf{GL}_n(\mathbb{R})$ (topologia relativa). \\ \\
...
\textbf{Definizione 2.4.1:} sia $X \subseteq \mathbb{R}^n$ e sia $p \in X$. Lo \textit{spazio tangente} ad $X$ in $p$ è
\begin{gather}
...
\begin{equation} \gamma (t) =
    ...
    \end{pmatrix} \notag
\end{equation}
dove $X \in \mathbb{M}_n$ e $v(t) \in C^1((-\varepsilon, \varepsilon);\mathbb{R}^n)$ tale che $v(0) = \vec{0}$. Per come è stato definito $Isom\, \mathbb{R}^n$ deve valere che $e^{tX} \in \mathbf{O}_n(\mathbb{R})$ per ogni $t \in (-\varepsilon, \varepsilon)$, cioè è sufficiente che:
\begin{gather}
...
\end{equation}
\\ \\ \\
\textbf{2.} sia $G := \{ A \in \mathbb{M}_n : \det A \in 2^{\mathbb{Z}}\}$, determinare $\mathfrak{g}(G)$. \\
...
\textbf{Definizione 4.1.3:} sia $X$ una varietà differenziabile con atlante $\{ U_\mu, f_\mu \}_{\mu \in M}$, si ha che $f : X \longrightarrow \mathbb{R}$ è differenziabile se $f \cdot f^{-1}_\mu$ è differenziabile per ogni $\mu \in M$. \\ \\
