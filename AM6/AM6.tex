...
\textbf{Dimostrazione:} possiamo assumere senza perdità di generalità che $[a, b] = [0, 1]$, dato che la densità è una proprietà topologica. Inoltre data $f \in C([0, 1])$ possiamo assumere che $f(0) = 0 = f(1)$, infatti se così non fosse basterebbe definire
\begin{align}
...
Vogliamo costruire una successione di funzioni polinomiali $\{q_n\}_{n \in \mathbf{N}}$ che approssima la delta di Dirac. Data $f \in C([0, 1], \mathbf{C})$, per ogni $n \in \mathbf{N}$ poniamo 
\begin{align}
    p_n(x) = \int_0^1 q_n(t - x)f(t) \, dt \notag
\end{align}
con $x \in [0, 1]$. Notiamo che se $q_n \in Pol([0, 1])$ allora anche $p_n \in Pol([0, 1])$. Scegliamo 
\begin{align}
    q_n(x) = c_n(1 - x^2)^n \notag
\end{align}
dove $c_n$ è una costante che garantisce la normalizzazione su $[-1, 1]$. Ponendo $s = t - x$ si ha 
\begin{align}
    p_n(x) = \int_{-1}^1 q_n(s)f(s + x) \, ds \notag
\end{align}
Allora 
\begin{align}
    |p_n(x) - f(x)| &= \left|\int_{-1}^1 q_n(s)(f(s + x) - f(x)) \, ds\right| \notag \\
    &\leq \int_{-1}^1 q_n(s)|f(s + x) - f(x)| \, ds \notag
\end{align}
Poiché $f$ è continua su un compatto ($[0, 1]$) segue dal \textit{teorema di Heine-Cantor} che per ogni $\varepsilon > 0$ esiste $\delta > 0$ tale che se $|x - y| < \delta$ si ha $|f(x) - f(y)| < \varepsilon$ per ogni $x, y \in [0, 1]$.
