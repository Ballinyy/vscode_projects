...
\textbf{Teorema (di Stone-Weierstrass):} sia $Pol([0, 1])$ lo spazio delle funzioni polinomiali a coefficienti complessi definite sull'intervallo $[0, 1]$. Si ha che $Pol([0, 1])$ è denso (in norma $\|\cdot\|_\infty$) in $C([0, 1], \mathbf{C})$. \\

\textbf{Dimostrazione:} vogliamo costruire una successione di polinomi $\{q_n\}_{n \in \mathbf{N}}$ che approssima la delta di Dirac. Data $f \in C([0, 1], \mathbf{C})$ poniamo 
\begin{align}
    p_n(x) = \int_0^1 q_n(t - x)f(t) \, dt \notag
\end{align}
Notiamo che se $q_n \in Pol([0, 1])$ allora anche $p_n \in Pol([0, 1])$.
