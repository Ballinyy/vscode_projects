...
\begin{align}
    \lim_{n \rightarrow \infty}x_n = x_1 + \lim_{n \rightarrow \infty}\sum_{i = 1}^n(x_{i + 1} - x_i) < \infty \notag
\end{align}
...
\begin{align}
    &a. \quad l^1(\mathbf{N}) := \{\mathbf{x} = \{x_n\}_{n \in \mathbf{N}} : \sum_{i = 1}^\infty |x_i| < \infty\} \text{ rispetto alla norma }\|\cdot\|_1 \notag \\
    &b. \quad l^2(\mathbf{N}) := \{\mathbf{x} = \{x_n\}_{n \in \mathbf{N}} : \sum_{i = 1}^\infty |x_i|^2 < \infty\} \text{ rispetto alla norma }\|\cdot\|_2 \notag \\
...
\end{align}
per ogni $n, m > N_\varepsilon$, dunque per ogni $k \in \mathbf{N}$ si ha che $\{x^k_n : n \in \mathbf{N}\}$ è una successione di Cauchy a termini complessi. Per la \textit{proposizione 1.2.4}, per ogni $k \in \mathbf{N}$ esiste $x^k \in \mathbf{C}$ ed esiste $N_l \in \mathbf{N}$ tale che
\begin{align}
    |x^k_n - x^k| < \varepsilon2^{-l} \notag
\end{align}
per ogni $n > N_l$. Sia $\mathbf{x} = \{x^k : k \in \mathbf{N}\}$, allora
\begin{align}
    \|\mathbf{x}_n - \mathbf{x}\|_1 &= \sum_{k = 1}^\infty|x^k_n - x^k| \notag \\
    &< \varepsilon\sum_{l = 1}^\infty 2^{-l} = \varepsilon \notag
\end{align}
per ogni $n > N_l$. Inoltre, fissato $n > N_l$ si ha
\begin{align}
    \|\mathbf{x}\|_1 \leq \|\mathbf{x} - \mathbf{x}_n\|_1 + \|\mathbf{x}_n\|_1 < \infty \notag
\end{align} 
ovvero la tesi. \\ \\
...
\end{align}
per ogni $n, m > N-\varepsilon$. Allora per ogni $k \in \mathbf{N}$ si ha
\begin{align}
    |x^k_n - x^k_m| < \varepsilon \notag
\end{align}
