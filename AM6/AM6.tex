...
\begin{align}
    ...
    &= ((T^{-1})^*T^*x, y) \notag
\end{align}
per ogni $x, y \in \mathcal{H}$, pertanto $(T^{-1})^*T^* = \mathbf{1}$. Con un ragionamento analogo si ottiene $T^*(T^{-1})^* = \mathbf{1}$. Pertanto concludiamo che $(T^*)^{-1} = (T^{-1})^*$. \\ \\
...
\end{align}
La tesi segue immediatamente dal fatto che $T^*$ è invertibile (bilatelarmente) e quindi iniettivo, pertanto $T^*x = 0$ se e solo se $x = 0$, inoltre la funzione 
\begin{align}
...
\end{align}
Essendo $\mathbf{S}^1$ compatto, per il \textit{teorema di Weierstrass}, la funzione $x \mapsto \|T^*x\|$ ammette massimo e minimo in $\mathbf{S}^1$, segue che esiste $x_0 \in \mathbf{S}^1$ tale che $\|T^*x_0\| \leq \|T^*x\|$ per ogni $x \in \mathbf{S}^1$, ovvero 
\begin{align}
    \inf_{x \in \mathbf{S}^1}\|T^*x\| = \|T^*x_0\| \notag
\end{align}
quindi $T^*$ lontano da zero. Analogamente si può dimostrare per $T$. \\ \\
$(3 \rightarrow 4)$
