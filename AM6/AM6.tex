...
\end{equation}
L'unione di questi $k_1 + k_2 + \dots + k_n$ iperpiani è detto \textbf{reticolato} su $\mathbf{R}^n$. Un reticolato $\mathcal{P}$ suddivide $\mathbf{R}^n$ in un numero finito di intervalli chiusi e limitati, detti \textit{intervalli del reticolato}, più un numero finito di insiemi illimitati. \\ \\
\textbf{Definizione:} un insieme $X$ è detto \textbf{plurintervallo} di $\mathbf{R}^n$ se esiste un reticolato $\mathcal{P}$ tale che $X$ è unione finita di \textit{intervalli} di $\mathcal{P}$ (anche non tutti). Allora se $I_1, I_2, \dots, I_m$ sono intervalli di $\mathcal{P}$ si ha che:
\begin{equation}
...
\textit{Proposizione 1.4.1:} sia $E \subset \mathbf{R}^n$ un insieme misurabile, allora $\underline{m}(E) = \overline{m}(E) = \\ = \lim_{r \rightarrow \infty} m(E \cap B_r(0))$. \\
