...
\newpage
\section*{Appendice}
\subsection*{Disuaguaglianze fondamentali}
\textbf{Lemma:} siano $x, y \geq 0$ tali che almeno uno dei due è non nullo e sia $\alpha \in (0, 1)$, allora 
\begin{align}
...
\end{align}
che è la tesi. \qed \\ \\
\textbf{Lemma:} sia $(X, \mathcal{M}, \mu)$ uno spazio di misura e siano $f, g : X \longrightarrow [0, +\infty]$ funzioni positive integrabili, fissato $\alpha \in (0, 1)$ si ha 
\begin{align}
    \int_X f^\alpha g^{1 - \alpha} \, d\mu \leq \left(\int_X f \, d\mu\right)^\alpha\left(\int_X g \, d\mu\right)^{1 - \alpha} \notag
\end{align} \\

\textbf{Dimostrazione:} se $f$ (0 $g$) è nulla quasi ovunque in $X$ la disuguaglianza è ovvia. Analogamente se $\int f$ (o $\int g$) è infinito la disuguaglianza è ovvia. Supponiamo allora 
\begin{align}
...
\end{align}
\textbf{Definizione:} sia $p \in \mathbf{N}$, esiste unico $q \in \mathbf{N}$ tale che 
\begin{align}
    \frac{1}{p} + \frac{1}{q} = 1 \notag
\end{align}
$q$ è detto \textit{Holder coniugato di} $p$. \\ \\
\textbf{Proposizione (disuaguaglianza di Holder):} siano $p, q \in \mathbf{N}$ Holder coniugati, sia $(X, \mathcal{M}, \mu)$ uno spazio di misura e siano $f, q : X \longrightarrow \mathbf{C}$ funzioni integrabili, allora si ha 
\begin{align}
